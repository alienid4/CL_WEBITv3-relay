...
# 第三方壓縮過的函式庫：壓縮後的變數名跟長字串會踩到 token=「xxx」這種正則，
# 但那是它自己的程式碼不是我們的金鑰（2026-09-21 xlsx.full.min.js）。
# 放行的是**確切路徑**，不是整個 vendor 目錄——整目錄放行等於後門，
# 任何人把帶金鑰的檔丟進 vendor/ 就永遠掃不到。換版本要重新放行。
SECRET_ALLOW_PATHS |= {
    "APP/asset-module/frontend/public/patch-app/assets/vendor/xlsx.full.min.js",
    "APP/asset-module/frontend/public/patch-app/assets/vendor/chart.umd.min.js",
}
...
BIG_FILE_ALLOW = {
    ...
    # 公司主機離線、patch 要自帶納管／SAN 收集用的 paramiko 相依套件（2026-09-11 v1.94.2）。
    # 這些是 PyPI 公開的二進位 wheel，不是資料；只放行確切檔名，換版本要重新放行。
    ".project/patch_wheels/pynacl-1.6.2-cp38-abi3-manylinux2014_x86_64.manylinux_2_17_x86_64.whl",
    # cryptography 是 paramiko 的必要相依（2026-09-14）。後端 venv 是 Python 3.11，用 cp311 輪子。
    ".project/patch_wheels/cryptography-50.0.1-cp311-abi3-manylinux2014_x86_64.manylinux_2_17_x86_64.whl",
    # 弱點彙總（CL_Patch）搬上系統版需要的 Excel 解析函式庫（2026-09-21）。
    # 公司主機離線，不能從 CDN 拉；這是 SheetJS 公開發行的實作，不是資料。
    # 另一支 chart.umd.min.js 只有 201KB，不需要放行。
    "APP/asset-module/frontend/public/patch-app/assets/vendor/xlsx.full.min.js",
}
try:
    idx = subprocess.run(
...
# 本次 commit 只碰文件/資料檔（沒動程式碼）時跳過測試，省下白等——測試結果不會變。
# 保守閘門：只有低風險（small，含手動 commit）才跳；medium 以上或 REQUIRE_TESTS=1 一律照跑。
#
# 測試閘門分兩段（2026-09-10 使用者定案「方案 A」，見 CLAUDE.md「測試閘門分兩段」）：
#   互動式 commit（small 風險）＝只跑「改到的檔」對應的測試當冒煙測試 → commit 幾秒過；
#   全套（106 檔）搬到 deploy.sh 部署 221 前擋——沒過全套的碼進不了正式機。
#   medium 以上切片 / REQUIRE_TESTS=1 / 手動單獨跑，一律仍跑全套（自動化風險切片不放寬）。
test_dir = ROOT / "tests"
...
#: 冒煙最多跑幾個測試檔。超過就退回「同名」那套——commit 要幾秒過，
#: 不能因為改到 api.py 這種全站都 import 的檔就等三分鐘。
#: 漏掉的由 deploy.sh 的全套關卡擋（它已經證明有效：2026-09-23 攔下 v1.342.0）。
SMOKE_MAX = 25


def _smoke_targets() -> list[str] | None:
    """staged 的程式碼檔 -> 要跑哪些測試檔。

    []   ＝有改碼但對不到任何測試（例如純前端 .vue）-> commit 放行，交給 deploy.sh
    None ＝沒 staged（手動單獨跑）-> 呼叫端跑全套，維持舊行為

    ## 為什麼不只看「同名」（2026-09-23）

    原本只對同名：`manage_state.py` -> `test_manage_state.py`。
    那天改了 `manage_state._ssh_runner` 的回傳介面（開始讀 returncode／stderr），
    同名那支補好了、閘門全綠，**但 `test_collect_account.py` 裡也有三個手捏的
    假 CompletedProcess**——它用 manage_state，卻不叫這個名字，於是完全沒被跑到。
    結果是 221 部署前跑全套才擋下來，繞了一圈。

    所以再加一條：**測試檔內容有提到這個模組名，也算它的消費者。**
    改回傳結構時真正該找的就是這種「假造它的地方」，而那多半不同名。

    數量有上限（SMOKE_MAX）：`api.py` 這種全站都 import 的檔會命中一大片，
    commit 就不是幾秒的事了。超過就退回同名那套，缺口由部署前的全套關卡補。
    """
    staged = staged_files()
    if not staged:
        return None
    ...
    for f in staged:
        pf = Path(f)
        if pf.suffix.lower() not in CODE_EXT:
            continue
        if pf.name.startswith("test_") and pf.suffix == ".py":
            want.add(pf.name)
            continue
        want.add(f"test_{pf.stem}.py")
        if pf.suffix == ".py":
            mods.add(pf.stem)

    by_name = {p for p in test_files if p.name in want}
    by_use: set = set()
    for t in test_files:
        if t in by_name:
            continue
        try:
            ...
            continue
        if any(m in body for m in mods):
            by_use.add(t)

    hit = by_name | by_use
    if len(hit) > SMOKE_MAX:
        hit = by_name          # 太多就退回同名，交給部署前的全套關卡
    return sorted(p.relative_to(ROOT).as_posix() for p in hit)


def _pytest_summary(r: subprocess.CompletedProcess) -> str:
    """摘要行 ＋ **失敗的測試名稱**。

    2026-09-22：閘門報「1 failed」但只留摘要，沒有名字——追不下去的 FAIL
    等於沒報。重跑一次又過了（偶發），更沒線索。所以把 FAILED/ERROR 那幾行
    一起帶出來，偶發也查得到是哪一條。
    """
    out = (r.stdout + r.stderr).strip().splitlines()
    head = next((ln.strip() for ln in reversed(out)
                 if "passed" in ln or "failed" in ln or "error" in ln),
                out[-1].strip() if out else "")
    bad = [ln.strip() for ln in out
           if ln.startswith(("FAILED", "ERROR")) and "::" in ln][:10]
    return head + ("" if not bad else "｜失敗：" + "；".join(bad))


use_smoke = bool(test_files) and not skip_tests and SLICE_RISK == "small" and not REQUIRE_TESTS
def _parallel_args() -> list[str]:
    """有裝 pytest-xdist 才平行跑（2026-09-17 使用者：「這兩天 commit 都很久 WHY」）。

    實測：全套序列 10 分 21 秒 → 平行（-n auto）3 分 42 秒，1392 項全過、沒有互相干擾。

    ⚠️ **不可以寫死 `-n auto`**：沒裝 xdist 的機器（公司機、乾淨的新環境）會直接
    「unrecognized arguments: -n」而讓整個部署失敗——為了快而讓部署掛掉是壞交易。
    所以先偵測，沒有就照舊序列跑。
    """
    try:
...
smoke = _smoke_targets() if use_smoke else None

if skip_tests:
    rec("驗收測試（本次無程式異動，跳過）", True,
        "只 staged 文件/資料檔，未碰 " + "/".join(sorted(CODE_EXT)) + "；測試結果不受影響")
elif use_smoke and smoke is not None:
    # 方案 A：只跑改到的檔對應的測試（全套在 deploy.sh 部署前擋）
    if smoke:
        # 冒煙只有幾個檔，平行的行程啟動成本反而划不來——這裡不平行
        r = run([sys.executable, "-m", "pytest", "-q", *smoke])
        rec("驗收測試（改到的檔｜全套在部署前跑）", r.returncode == 0,
            f"{_pytest_summary(r)}｜{len(smoke)} 檔：" + ", ".join(Path(t).name for t in smoke))
    else:
        rec("驗收測試（改到的檔對不到測試，冒煙略過）", True,
            "staged 的程式碼沒有對應 test_*.py；全套在 deploy.sh 部署 221 前擋。")
elif test_files:
    # 全套：medium+ 切片 / REQUIRE_TESTS / 手動單獨跑
    r = run([sys.executable, "-m", "pytest", "-q", *PAR])
    rec("驗收測試通過（全套）" + ("｜平行" if PAR else "｜序列（沒裝 pytest-xdist）"),
        r.returncode == 0, _pytest_summary(r))
