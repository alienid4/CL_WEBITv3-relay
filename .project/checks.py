...
def _smoke_targets() -> list[str] | None:
    # 把 staged 的程式碼檔對應到要跑的測試檔（同名 test_<stem>.py，本專案命名慣例）。
    # []   ＝有改碼但對不到任何測試（例如純前端 .vue）→ commit 放行，交給 deploy.sh 全套關卡。
    ...
    for f in staged:
        ...
        if pf.suffix.lower() not in CODE_EXT:
            continue
        want.add(pf.name if (pf.name.startswith("test_") and pf.suffix == ".py")
                 else f"test_{pf.stem}.py")
    return sorted(p.relative_to(ROOT).as_posix() for p in test_files if p.name in want)


def _pytest_summary(r: subprocess.CompletedProcess) -> str:
    out = (r.stdout + r.stderr).strip().splitlines()
    return next((ln.strip() for ln in reversed(out)
