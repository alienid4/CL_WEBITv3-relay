...
def test_資產可編輯_主鍵不可改_亂欄位被擋(tmp_path, monkeypatch):
    ...
    try:
        ...
        # ⑤ 改不存在的序號（且帶 asset_status）→ 404，不可以噴 500
        #    （2026-09-19 抓到：舊碼 fetchone()[0] 遇 None 會 TypeError→500）
        r = client.put("/api/assets/NOPE-999", json={"fields": {"asset_status": "停用"}})
        assert r.status_code == 404, r.text

        # 查無此資產（只改 hostname 也要 404）
...
def test_資產查詢預設排退役_跟首頁頭條同口徑():
    """/api/assets 預設就排退役（跟 composition／首頁頭條同口徑）——所以首頁組成長條
    （composition 排退役）點下去到 /assets 不會多出退役那幾筆。2026-09-19 驗這個前提還在，
    避免哪天有人拿掉預設排退役、讓下鑽又比檯面多。"""
    with tempfile.TemporaryDirectory() as tmp:
        client, db_path = _client(tmp)
        try:
            _insert_hardware(db_path, "A-1", "w1", "10.0.0.1", os="Windows Server 2019")
            _insert_hardware(db_path, "A-2", "w2", "10.0.0.2", os="Windows Server 2019")
            _insert_hardware(db_path, "R-1", "w3", "10.0.0.3", os="Windows Server 2019",
                             asset_status="報廢")
            rows = client.get("/api/assets").json()
            assert {r["asset_serial"] for r in rows} == {"A-1", "A-2"}, "預設要排掉報廢那筆"
            # 明確要看某狀態時才顯示退役（尊重使用者的 filter）
            ret = client.get("/api/assets",
                             params={"filter_field": "asset_status", "filter_value": "報廢"}).json()
            assert {r["asset_serial"] for r in ret} == {"R-1"}
        finally:
            api.app.dependency_overrides.clear()
            api._COLUMN_CACHE.clear()
...
def test_手動新增資產_擋帳外前綴序號():
    """手動新增用 DYN-/VC-/AUTO- 開頭序號要被擋（那是帳外保留前綴，會被統計當帳外）。"""
    with tempfile.TemporaryDirectory() as tmp:
        client, db_path = _client(tmp)
        try:
            for sn in ("DYN-手動", "VC-x", "AUTO-1"):
                resp = client.post("/api/assets/manual", json={"fields": {"asset_serial": sn}})
                assert resp.status_code == 400, f"{sn} 應被擋：{resp.text}"
            # 正常序號照樣可建
            ok = client.post("/api/assets/manual", json={"fields": {"asset_serial": "HW-OK-1"}})
...
def _dump_recipient(client, tmp_path):
    """匯出一律加密（決策 SEC5），所以測試要先設一把收件人公鑰。回私鑰路徑。

    沒設公鑰的話端點會 400——那是刻意的：寧可擋下來，也不要給一份
    沒有保護的檔案，因為那份會被 email 寄出去。
    """
    import export_crypto

    priv = str(tmp_path / "recipient.key")
    info = export_crypto.generate_keypair(priv)
    r = client.put("/api/export-key/recipient", json={"public_key": info["public_key"]})
    assert r.status_code == 200, r.text
    # 2026-09-18 起加密是開關、預設關；這些測試驗的是加密那條路，所以打開
    r = client.put("/api/export-key/encryption", json={"enabled": True})
    assert r.status_code == 200, r.text
    return priv
...
def test_匯出全部資料_二進位格式可以重新打開且資料完整(tmp_path, monkeypatch):
    ...
    try:
        ...
        priv = _dump_recipient(client, tmp_path)
        resp = client.get("/api/backup/dump", params={"fmt": "binary"})
        assert resp.status_code == 200
        assert resp.headers["content-type"] == "application/octet-stream"
        assert ".db.enc" in resp.headers["content-disposition"]

        # 決策 SEC5：匯出一律加密。解開之後要驗的東西跟以前一樣——
        # 整庫完整、資料真的在裡面。加密只是外殼，不是驗證的替代品。
        import export_crypto

        assert export_crypto.looks_encrypted(resp.content), "匯出沒有加密"
        assert b"dump-host-1" not in resp.content, "主機名直接出現在匯出檔裡"
        out_path = tmp_path / "downloaded.db"
        out_path.write_bytes(export_crypto.decrypt(resp.content, priv))
...
def test_匯出全部資料_文字格式含完整SQL(tmp_path, monkeypatch):
    ...
    try:
        ...
        assert resp.status_code == 200
        assert ".sql.enc" in resp.headers["content-disposition"]

        # SQL 這條路以前走串流、**不經過加密**，是最容易漏掉的一條。
        # 漏掉的話使用者以為匯出加密了，實際寄出去的是純文字 SQL。
        import export_crypto

        assert export_crypto.looks_encrypted(resp.content), "SQL 匯出繞過了加密"
        assert b"CREATE TABLE" not in resp.content
        text = export_crypto.decrypt(resp.content, priv).decode("utf-8")
