...
def test_vInfo整列原始欄位都存進payload_不只挑過的():
    """2026-09-08 使用者定案「不要只存你自己想知道的」：vInfo 除了挑進 hardware 的
    核心欄位，其餘欄位（例 Annotation／CPUs）也要一併存進 source_record.payload 的 raw，
    之後要用哪欄再挑。"""
    import json
    with tempfile.TemporaryDirectory() as tmp:
        conn = _conn(tmp)
        try:
            ...
            ws = wb.active
            ws.title = "vInfo"
            ws.append(["VM", "DNS Name", "VM UUID", "Annotation", "CPUs", "VI SDK Server"])
            ws.append(["web01", "web01.corp.local",
                       "42309999-1111-2222-3333-444455556666",
                       "業務備註XYZ", 8, "vcenter-x.corp.local"])
            p = Path(tmp) / "rv_raw.xlsx"
            wb.save(p)
            rv.import_rvtools(p, conn)
            ...
            # 挑過的核心欄照舊
            assert payload["hostname"] == "web01.corp.local"
            assert payload["vi_sdk_server"] == "vcenter-x.corp.local"
            # 沒挑進 hardware 的欄，也一欄不漏地存在 raw
            assert payload["raw"]["Annotation"] == "業務備註XYZ"
            assert payload["raw"]["CPUs"] == 8
        finally:
            conn.close()
