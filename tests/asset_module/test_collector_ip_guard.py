...
def test_串流讀取一定要有逾時看門狗():
    ...
    # 2026-09-11 換成 paramiko 後改成行為測試：遠端永遠不吐東西也不結束，
    # 一定要在時限內回來、關掉連線、講出人話。
    import socket
    import time
    ...
    class _Client:
        ...
        def get_transport(self):
            class _T:
                def open_session(self, timeout=None): return _HangChan()
            return _T()

        def close(self): _Client.closed = True

    import pytest
    mp = pytest.MonkeyPatch()
    try:
        mp.setattr(eng, "_ssh_connect", lambda *a, **k: _Client())
        mp.setattr(eng, "_STREAM_GRACE", 0)
        t0 = time.monotonic()
        r = eng._ssh_executor("10.0.0.9", "u", "x", "linux", "echo\n", "10.0.0.221",
                              timeout=1, as_root=True)
    finally:
        mp.undo()
    assert time.monotonic() - t0 < 5, "串流迴圈沒有時限，會永遠卡住"
    assert _Client.closed, "逾時後沒關連線"
    # 逾時要講得出人話，不能只是靜靜回一個失敗
    assert not r.ok and "強制中止" in r.output
...
# ===== 明顯錯的參數要在「連線之前」擋下（2026-09-17）=====
#
# 使用者問「這兩天 commit 都很久 WHY」。量測發現兩支 onboard 測試各空等 20 秒——
# 它們在等真實 SSH 逾時，因為參數檢查寫在 build_script 裡（也就是**連完線之後**）。
#
# 這不只是測試慢：使用者打錯收集器位址或帳號名，要等 20 秒的連線逾時才被告知，
# 而那趟連線從一開始就注定產出壞腳本。所以把檢查往前搬。
#
# 這兩條測試用「有沒有碰到網路」來守：只要有人把檢查搬回連線之後，probe 就會被呼叫。


def test_佔位收集器位址在連線前就擋下(monkeypatch):
    called = []
    monkeypatch.setattr(eng, "probe_target",
                        lambda *a, **k: called.append(a) or {})
    r = eng.onboard("10.0.0.9", "linux", "sysctl", "x", "YOUR_SERVER_IP", pubkey=PUBKEY)
    assert r.ok is False and "ASSET_COLLECTOR_IP" in r.message
    assert called == [], "參數已經知道是錯的，不可以還先連出去等逾時"


def test_空的收集器位址也在連線前擋下(monkeypatch):
    called = []
    monkeypatch.setattr(eng, "probe_target",
                        lambda *a, **k: called.append(a) or {})
    r = eng.onboard("10.0.0.9", "linux", "sysctl", "x", "", pubkey=PUBKEY)
    assert r.ok is False and called == []


def test_位址正常時照樣會去探測目標(monkeypatch):
    called = []

    def fake_probe(*a, **k):
        called.append(a)
        return {"os": "linux", "uid": 0, "has_sudo": True}

    monkeypatch.setattr(eng, "probe_target", fake_probe)
    monkeypatch.setattr(eng, "_ssh_executor",
                        lambda **kw: eng.OnboardResult(True, "done", "ok"))
    eng.onboard("10.0.0.9", "linux", "root", "x", "10.92.198.1", pubkey=PUBKEY)
    assert called, "位址正常就該照原本流程走——不可以把探測一起擋掉"
