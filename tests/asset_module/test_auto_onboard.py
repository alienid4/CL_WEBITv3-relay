...
def test_沒授權網段_一律不碰任何主機():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            r = ao.run_auto_onboard(conn, "10.0.0.221",
...
def test_Linux未納管_用庫裡憑證納管成功_並留auto稽核():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            ao.save_segment(conn, "192.168.1.")
            r = ao.run_auto_onboard(conn, "10.0.0.221", pubkey=PUBKEY,
...
def test_密碼絕不進稽核任何欄位():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            ao.save_segment(conn, "192.168.1.")
            ao.run_auto_onboard(conn, "10.0.0.221", pubkey=PUBKEY,
                                executor=_ok_executor, cred_lookup=_fake_ssh_cred())
            row = conn.execute(
...
def test_憑證解不開_記為失敗():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            r = ao.run_auto_onboard(conn, "10.0.0.221", executor=_ok_executor,
