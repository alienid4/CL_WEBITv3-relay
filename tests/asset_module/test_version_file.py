...
def test_api回得出真版號而不是問號():
    ...
    2026-09-21 改成走 TestClient：原本直接呼叫函式，FastAPI 的 Depends 預設值
    不會被解開，所以這支端點一加入 DB 相依就會假失敗——而且更糟的是，
    直接呼叫永遠測不到「登入被關掉時要回 true」那倍保險是不是真的有效。
    """
    import api
    from fastapi.testclient import TestClient

    with TestClient(api.app) as c:
        info = c.get("/api/version").json()
