...
def _set_recipient(client, tmp):
    ...
    # 2026-09-18 起加密改成開關、預設關；這個 helper 代表「要加密」的情境，所以一併打開
...
def test_開了加密卻沒設公鑰_不准匯出():
    """開著「匯出時加密」時，寧可擋下來也不要給一份沒有保護的檔案。"""
    with tempfile.TemporaryDirectory() as tmp:
        client, _ = _client(tmp)
        try:
            assert client.put("/api/export-key/encryption", json={"enabled": True}).status_code == 200
...
def test_預設不加密_可直接匯出_而且留紀錄():
    """2026-09-18 使用者：「我自己都會忘，所以我要拿掉，以後功能關閉即可」。
    預設關：直接給原始檔；但每次都記一筆操作紀錄，事後查得到誰拿走了整份資料。"""
    with tempfile.TemporaryDirectory() as tmp:
        client, _ = _client(tmp)
        try:
            st = client.get("/api/export-key").json()
            assert st["encryption_enabled"] is False and st["can_export"] is True
            r = client.get("/api/backup/dump")
            assert r.status_code == 200, r.text
            assert r.content[:16] == b"SQLite format 3\x00", "不加密就是原始 SQLite 檔"
            assert not r.headers["content-disposition"].endswith('.enc"')
            conn = db.get_connection(Path(tmp) / "t.db")
            try:   # 一定要關：Windows 上沒關的連線會讓暫存目錄刪不掉
                n = conn.execute("SELECT COUNT(*) FROM activity_log WHERE action = 'export_plain'").fetchone()[0]
            finally:
                conn.close()
            assert n == 1, "未加密匯出要留一筆紀錄"
        finally:
            api.app.dependency_overrides.clear()
            api.get_db_path = _ORIG_GET_DB_PATH
...
def test_狀態端點分得出來源端與收件端():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            st = client.get("/api/export-key").json()
            # 預設不加密（2026-09-18）→ 不需要公鑰就能匯出
            assert st["encryption_enabled"] is False and st["can_export"] is True
            # 打開加密、還沒設公鑰 → 不能匯出
            assert client.put("/api/export-key/encryption", json={"enabled": True}).status_code == 200
            st = client.get("/api/export-key").json()
            assert st["can_export"] is False, "開了加密、還沒設公鑰卻說可以匯出"
