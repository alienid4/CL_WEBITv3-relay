...
def test_狀態摘要卡下鑽等於各平台該欄加總_且不含不列入的(tmp_path):
    """表1上方的狀態摘要卡（已EOS／一年內EOS…）＝各平台列同一欄加總。
    點卡片下鑽帶 registered_only，撈到的筆數必須等於那個加總——不能把
    『不列入本報告』（網路設備等，也各有 eos_status）混進來（天條：加總＝下鑽同一份口徑）。"""
    conn = _conn(tmp_path)
    try:
        _seed(conn)
        t1 = system_report.platform_lifecycle(conn)
        for st in system_report.STATUS_ORDER:
            card = sum(row[st] for row in t1["rows"])
            got = system_report.drill_platform(conn, status=st, registered_only=True)
            assert len(got) == card, st
            # 撈到的每一台都必須是列入本報告的（platform 有值）
            assert all(a["platform"] is not None for a in got), st
    finally:
        conn.close()
