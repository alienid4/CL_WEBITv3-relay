...
# ===== Bug 7：對帳「搜不到」把退役＋帳外算進去，比「在管」還大 =====

def test_搜不到不含退役與帳外_不可比在管大():
    """2026-09-17 使用者在公司機看到「搜不到 7823 > 在管 3768」。

    真因：對帳的 total_ica_count 用整張 hardware 的原始筆數（含退役、帳外 DYN-/VC-/AUTO-），
    而頭條「在管」是去退役、去帳外。同一頁兩套口徑，「搜不到＝登記−一致」就把
    退役機（本來就不會回應）和帳外算進去，撐到比在管總數還大。

    守的是：對帳的『登記 universe』要跟『在管（registered_total）』同口徑，
    所以 total_ica_count 只算 2 台（在管），搜不到不可能比它大。
    """
    with tempfile.TemporaryDirectory() as tmp:
        client, db_path = _client(tmp)
        try:
            ...
            try:
                # 2 台正式在管
                db.insert_hardware(conn, asset_serial="A-1", hostname="reg-1",
                                   ip="10.99.0.1", environment="正式", asset_status="使用中")
                db.insert_hardware(conn, asset_serial="A-2", hostname="reg-2",
                                   ip="192.168.1.2", environment="正式", asset_status="使用中")
                # 1 台退役（不該進在管、也不該算成搜不到——它是「已退役」那格）
                db.insert_hardware(conn, asset_serial="A-3", hostname="old-1",
                                   ip="192.168.1.3", environment="正式", asset_status="報廢")
                # 1 台帳外（DYN-：掃到但沒登記，屬「未登記」不屬「登記卻搜不到」）
                db.insert_hardware(conn, asset_serial="DYN-9", hostname="ghost-1",
                                   ip="192.168.1.9", environment="正式", asset_status="使用中")
                # 沒有任何掃描 → 一致 0、搜不到＝全部登記
            finally:
                conn.close()

            s = client.get("/api/dashboard/stats", params={"environment": "正式"}).json()
            comp = client.get("/api/dashboard/composition").json()
            registered = comp["registered_total"]
            ica_only = s["total_ica_count"] - s["total_overlap_count"]

            assert s["total_ica_count"] == registered == 2, (
                f"對帳登記數 {s['total_ica_count']} 要＝在管 {registered}＝2（排退役＋帳外）")
            assert ica_only <= registered, f"搜不到 {ica_only} 不可能比在管 {registered} 大"
            assert ica_only == 2, "2 台在管、沒掃到 → 搜不到 2（退役/帳外不算在內）"
        finally:
            api.app.dependency_overrides.clear()
