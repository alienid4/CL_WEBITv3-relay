...
def test_預設收集身分是唯讀帳號():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            # 定案用專用唯讀帳號，不共用 sysinfra（理由見 manage_state 的說明：
            # 共用管理帳號會讓登入紀錄被機器流量塞滿，稽核時人跟系統分不開）
...
def test_設定影響遠端ssh登入帳號(monkeypatch):
    ...
    def fake_run(cmd, **kw):
        ...
        # 而且 Windows 閘門會 skip 這些 POSIX 測試，本機看不到——
        # 真的踩到是在 221 部署前跑全套的時候。用本尊就不會有這種漂移。
...
def test_唯讀帳號收本機也走ssh不走本機捷徑(monkeypatch):
    """2026-09-11 使用者選 3a：收集器收自己也用收集帳號 SSH 進來。

    以前的本機捷徑是以「跑服務的帳號」執行（221 是 sysctl，沒有 sudo），
    盤點結果跟別台用 webit3scan 收的不一樣；服務帳號之後還會換成 sysinfra。
    這條測試反過來盯住：本機位址也不可以退回 bash 直跑。"""
    import subprocess
    ...
    runner = manage_state._runner_for(
        "203.0.113.1", "/tmp/key", account=manage_state.READONLY_ACCOUNT)
    runner("203.0.113.1", "id")
    assert captured["cmd"][0] == "ssh", "收本機退回了本機捷徑"
    assert f"{manage_state.READONLY_ACCOUNT}@203.0.113.1" in captured["cmd"]
