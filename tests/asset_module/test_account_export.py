...
使用者 2026-09-03 給了公司現行的 18 欄 Excel 格式，要兩種匯出：
...
def test_department_是處加部串起來():
    """2026-09-03 用真實資料查證：inventory_division 3643 筆全是「資訊管理處」、
...
def test_以主機為主_一台多系統_帳號只列一次_系統併在同一格(tmp_path):
    """2026-09-18 使用者：「以 hostname 為主」。以前 hardware.ip = host_account.ip 直接 JOIN，
    一台 3 個系統 → 同一個帳號 3 列。改成一個帳號一列，多個系統併在同一格；
    報廢的那筆登記不算這台的系統。"""
    import db as _db
    p = tmp_path / "t.db"
    _db.init_db(p)
    conn = _db.get_connection(p)
    for sn, api, st in (("A-1", "N-001", "使用中"), ("A-2", "N-002", "使用中"),
                        ("A-3", "N-003", "使用中"), ("A-9", "N-999", "報廢")):
        _db.insert_hardware(conn, asset_serial=sn, hostname="web1", ip="192.0.2.10",
                            asset_status=st, api_id=api, custodian="李小華")
    conn.execute("INSERT INTO host_account (ip, username, uid, kind, can_login, source, first_seen, last_seen) "
                 "VALUES ('192.0.2.10','root',0,'default',1,'ssh','x','x')")
    conn.commit()
    rows, _ = ae.standard_rows(conn)
    assert len(rows) == 1, f"一個帳號只能一列，實際 {len(rows)} 列"
    assert rows[0]["system_id"] == "N-001、N-002、N-003", "多系統併在同一格、報廢的不算"
    assert rows[0]["hostname"] == "web1" and rows[0]["owner"] == "李小華"
    full, _ = ae.full_rows(conn)
    assert len(full) == 1 and full[0]["api_id"] == "N-001、N-002、N-003"
    conn.close()
