...
def test_stage_issues_key_and_returns_files():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            # 2026-08-28（決策 SEC4）：投放目錄只收資料，端點就不該交出可執行內容。
            # 舊版把這兩支腳本一起交出去，配上 root 排程到 /tmp 撿東西執行，
            # 合起來就是本機提權。現在它們隨 bootstrap 固定佈在目標主機上，
            # 而且投放目錄真的出現 .sh 的話 install.sh 會把整個任務當攻擊跡象拒絕——
            # 端點若還交出去，等於誘導人做出會讓納管安靜失敗的事。
            assert "push_agent.sh" not in body["files"]
            assert "install.sh" not in body["files"]
            assert body["upload_dir"].startswith("/var/lib/webit3-agent/incoming/")
            assert "/tmp" not in body["upload_dir"]
