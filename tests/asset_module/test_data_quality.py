...
def test_人名合併預覽不寫入實際合併才寫():
    """預覽只回筆數不改資料；apply 才把三處錯名併成正確名（2026-09-19）。"""
    import pytest
    with tempfile.TemporaryDirectory() as tmp:
        db_path = Path(tmp) / "t.db"
        db.init_db(db_path)
        _insert_hardware(db_path, "A-1", "h1", "10.99.0.1", user_name="李太名", custodian="李太名")
        _insert_hardware(db_path, "A-2", "h2", "10.99.0.2", user_name="王五")
        conn = db.get_connection(db_path)
        try:
            pv = data_quality.preview_person_merge(conn, "李太名", "李太明")
            assert pv["total"] == 2          # user_name 1 + custodian 1
            assert conn.execute(
                "SELECT COUNT(*) FROM hardware WHERE user_name='李太名'").fetchone()[0] == 1
            with pytest.raises(ValueError):   # 原名=正確名要擋
                data_quality.preview_person_merge(conn, "王五", "王五")
            r = data_quality.apply_person_merge(conn, "李太名", "李太明")
            assert r["total"] == 2
            assert conn.execute(
                "SELECT COUNT(*) FROM hardware WHERE user_name='李太名'").fetchone()[0] == 0
            assert conn.execute(
                "SELECT COUNT(*) FROM hardware WHERE custodian='李太明'").fetchone()[0] == 1
        finally:
            conn.close()
