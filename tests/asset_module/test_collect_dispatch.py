...
def test_結果有存下來_重新整理還原得回上一次():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # 2026-09-16 起：完全沒回應**而且資產庫沒登記**＝那個位址上根本沒有機器，
        # 算成「要人工處理」是錯的（使用者：「為什麼是 508，還要人工處理?」——
        # 他貼的是兩個 /24，508 是位址數，掃網段本來就大部分是空的）。
        # 這裡 10.0.0.3 活著但進不去（要裝 agent）＝真的待辦；10.0.0.4 沒回應也沒登記＝空位址。
        assert latest["needs_action"] == 1
        assert latest["empty_addresses"] == 1
        conn.close()
