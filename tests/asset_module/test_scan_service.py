...
def test_排程設定往返(conn):
    ...
    got = scan_service.get_schedule(conn)
    # 比對「存進去的那幾個欄位」而不是整個 dict 全等：2026-09-16 加了 weekly 模式
    # （多回 weekday／weekday_label／modes），全等比對會因為「多了欄位」就紅，
    # 但那不是壞掉——測試該守的是既有欄位沒被動到。
    assert {k: got[k] for k in ("enabled", "mode", "time", "interval_hours")} == {
        "enabled": True, "mode": "interval", "time": "02:30", "interval_hours": 8,
    }
    assert "weekly" in got["modes"]
