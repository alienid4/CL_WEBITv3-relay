...
def test_scan_status_參數擋掉亂填的值():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            api.app.dependency_overrides.clear()
def test_os來源兩個數字與下鑽筆數一致():
    """首頁「OS 來源：N 台實際收集、M 台由掃描推測」那兩個數字要能點進去看是哪幾台。

    判定規則有兩份（composition() 的 has_real_os、/api/assets 的 os_source），
    只要有一份走鐘，畫面數字就會跟點進去的筆數對不上——這正是使用者最不能忍的那種錯。
    字面 "N/A" 也算「不知道」，跟空白同一邊（2026-08-13 已定案的口徑）。
    """
    with tempfile.TemporaryDirectory() as tmp:
        client, db_path = _client(tmp)
        try:
            _seed(
                db_path,
                hardware=[
                    {"asset_serial": "A1", "hostname": "h1", "ip": "10.0.0.1", "os": "Rocky Linux 9.7"},
                    {"asset_serial": "A2", "hostname": "h2", "ip": "10.0.0.2", "os": "Windows Server 2022"},
                    {"asset_serial": "A3", "hostname": "h3", "ip": "10.0.0.3", "os": ""},      # 空白＝不知道
                    {"asset_serial": "A4", "hostname": "h4", "ip": "10.0.0.4", "os": "N/A"},   # 字面 N/A 也算不知道
                    {"asset_serial": "A5", "hostname": "h5", "ip": "10.0.0.5"},                # NULL
                ],
                scanned=[],
            )
            comp = client.get("/api/dashboard/composition").json()
            assert comp["os_from_facts"] == 2
            assert comp["os_guessed"] == 3

            facts = client.get("/api/assets", params={"os_source": "facts"}).json()
            guessed = client.get("/api/assets", params={"os_source": "guessed"}).json()

            assert len(facts) == comp["os_from_facts"], "「實際收集」數字與下鑽筆數不一致"
            assert len(guessed) == comp["os_guessed"], "「掃描推測」數字與下鑽筆數不一致"
            assert {r["asset_serial"] for r in facts} == {"A1", "A2"}
            assert {r["asset_serial"] for r in guessed} == {"A3", "A4", "A5"}

            r = client.get("/api/assets", params={"os_source": "亂填"})
            assert r.status_code == 400
        finally:
            api.app.dependency_overrides.clear()
