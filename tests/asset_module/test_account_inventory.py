...
def test_手動改類型覆寫程式判定_收集不沖掉_可清除():
    # CBoss 那種：程式判成 builtin/human，稽核人員手動改成 service，且下次收集不能沖掉。
    with tempfile.TemporaryDirectory() as tmp:
        conn = _db(tmp)
        try:
            _seed(conn)
            ...
            def kind_of(user):
                return next(a for a in account_inventory.list_accounts(conn)
                            if a["username"] == user)

            assert kind_of("alice")["kind"] == "human"      # 程式原判

            account_inventory.set_account_kind(conn, ip, "alice", "service")
            a = kind_of("alice")
            assert a["kind"] == "service"                   # 覆寫贏
            assert a["kind_override"] == "service"
            assert a["kind_computed"] == "human"            # 保留原判

            account_inventory.collect_accounts(conn, runner=_runner())   # 再收一次
            assert kind_of("alice")["kind"] == "service"    # 收集沒沖掉覆寫

            account_inventory.set_account_kind(conn, ip, "alice", "")     # 清除
            a = kind_of("alice")
            assert a["kind"] == "human"                     # 回到程式判定
            assert a["kind_override"] is None
        finally:
            conn.close()
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            with pytest.raises(ValueError):
                account_inventory.set_account_kind(conn, "203.0.113.20", "alice", "亂填")
        finally:
            conn.close()


def test_特權帳號快照與趨勢():
    """每次採集後存一筆特權帳號現值，趨勢照時間序回傳（2026-09-19）。"""
    with tempfile.TemporaryDirectory() as tmp:
        conn = _db(tmp)
        try:
            _seed(conn)
            # 採集會在結尾自動存一筆快照
            account_inventory.collect_accounts(conn, runner=_runner())
            trend = account_inventory.privileged_trend(conn)
            assert len(trend) == 1
            snap = trend[0]
            # root(uid0) + alice(wheel→sudoer) 都算特權；bob 不算
            assert snap["uid0"] >= 1
            assert snap["privileged"] >= 2
            assert snap["total_accounts"] == 3
            # 再採一次 → 第二筆快照，趨勢照舊→新排
            account_inventory.collect_accounts(conn, runner=_runner())
            trend2 = account_inventory.privileged_trend(conn)
            assert len(trend2) == 2
            assert trend2[0]["taken_at"] <= trend2[1]["taken_at"]
        finally:
            conn.close()
