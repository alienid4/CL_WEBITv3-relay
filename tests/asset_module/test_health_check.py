...
def test_那個網段根本沒掃過_不能說它失聯():
    ...
    2026-09-18：失聯／未涵蓋改由正典 manage_state 判定（依 scan_coverage 涵蓋網段），
    這裡建一筆涵蓋紀錄＝只掃了 192.168.1.0/24，10.99.5.1 就落在未涵蓋。"""
    with _conn() as conn:
        hid = _host(conn, ip="10.99.5.1", collect_ok=1)
        _scanned(conn, "10.99.0.10", hostname="somewhere-else")
        # 這次掃描實際涵蓋的網段（manage_state 依這張表判涵蓋）：只有 192.168.1.0/24
        import scan_scope
        class _Src:
            cidr = "192.168.1.0/24"
            name = "網段 192.168.1.0/24"
        scan_scope.record_coverage(conn, "2026-08-24 01:00:00", [_Src()])
        ev = health_check.evaluate_all(conn)[hid]
        assert ev["machine"] == "ok", "機器燈不該因為我們沒去掃就變紅"
        nc = next(i for i in ev["issues"] if i["key"] == "not_covered")
        assert nc["light"] == "data" and "涵蓋" in nc["label"]
        assert all(i["key"] != "lost" for i in ev["issues"])


def test_失聯與未涵蓋一律跟manage_state同源():
    """2026-09-18 使用者：「收掉那第三份失聯判定」。體檢的失聯／未涵蓋不可以自己算一份，
    一律跟正典 manage_state 逐台狀態對齊。這裡建 LOST（同段掃到鄰居沒掃到它）＋
    NOT_COVERED（網段沒涵蓋）兩台，斷言體檢的判定 = manage_state 的判定。"""
    import manage_state
    with _conn() as conn:
        lost_id = _host(conn, serial="L1", hostname="lost01", ip="10.99.1.1", collect_ok=1)
        nc_id = _host(conn, serial="N1", hostname="nc01", ip="10.99.5.1", collect_ok=1)
        _scanned(conn, "10.99.1.250", hostname="neighbor")   # 只掃 10.99.1.0/24（掃到鄰居）

        import scan_scope
        class _Src:
            cidr = "10.99.1.0/24"
            name = "網段 10.99.1.0/24"
        scan_scope.record_coverage(conn, "2026-08-24 01:00:00", [_Src()])

        # manage_state 的判定
        ms_state = {it["asset_serial"]: it["state"] for it in manage_state.summarize(conn)["items"]
                    if it.get("asset_serial")}
        assert ms_state["L1"] == manage_state.LOST
        assert ms_state["N1"] == manage_state.NOT_COVERED

        # 體檢的判定必須一致
        ev = health_check.evaluate_all(conn)
        keys = {sid: {i["key"] for i in ev[sid]["issues"]} for sid in (lost_id, nc_id)}
        assert "lost" in keys[lost_id] and "not_covered" not in keys[lost_id]
        assert "not_covered" in keys[nc_id] and "lost" not in keys[nc_id]
