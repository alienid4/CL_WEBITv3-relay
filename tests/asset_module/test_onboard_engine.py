...
def test_腳本內嵌公鑰_且不含任何機密():
    ...
    assert eng.DEFAULT_ACCOUNT in lin
...
def test_收集帳號所有平台同一個名字():
    """使用者 2026-09-23：「還是統一都叫 webit3sc」。

    兩個名字是 2026-09-23 那個缺陷的根源：每個呼叫端都得記得帶 platform，
    四個收集器忘了帶就全部拿 Linux 的名字去登 AIX，而那個帳號在 AIX 上不存在——
    ...
    **一個名字就沒有「忘了帶 platform」這回事。**
    ...
    assert eng.DEFAULT_ACCOUNT == eng.DEFAULT_ACCOUNT_AIX
    assert ms.READONLY_ACCOUNT == ms.AIX_COLLECT_ACCOUNT == ms.DEFAULT_COLLECT_ACCOUNT
    assert eng.DEFAULT_ACCOUNT == ms.READONLY_ACCOUNT


def test_帳號名不可以超過8字元():
    """AIX 的 max_logname 預設只給 8 個字元，mkuser 會直接拒絕更長的名字。

    放寬要 chdev **並重開機**——為了帳號名重開正式 AIX 不划算。
    所以兩個平台能共用的只有短的那個。
    """
    import manage_state as ms

    assert len(ms.READONLY_ACCOUNT) <= 8, (
        f"{ms.READONLY_ACCOUNT} 有 {len(ms.READONLY_ACCOUNT)} 個字元，AIX 建不起來")


def test_兩個平台的納管腳本建的是同一個帳號():
    lin = eng.build_script("linux", PUBKEY, "10.0.0.221")
    aix = eng.build_script("aix", PUBKEY, "10.0.0.221")
    assert eng.DEFAULT_ACCOUNT in lin
    assert eng.DEFAULT_ACCOUNT in aix
    assert "webit3scan" not in aix, "AIX 腳本不可以出現 10 字元的舊名"
