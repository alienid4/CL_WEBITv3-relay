...
def test_腳本內嵌公鑰_且不含任何機密():
    lin = eng.build_script("linux", PUBKEY, "YOUR_SERVER_IP")
    assert PUBKEY in lin
    assert "webit3scan" in lin
    assert "from=\\\"YOUR_SERVER_IP\\\"" in lin or "YOUR_SERVER_IP" in lin
    # 腳本裡不該有私鑰痕跡
...
def test_平台選對腳本():
    ...
    with pytest.raises(ValueError):
        eng.build_script("aix", PUBKEY, "10.0.0.1")
...
def test_密碼絕不出現在回傳結果():
    ...
    result = eng.onboard("10.0.0.9", "linux", "sysctl", fake_pw,
                         "YOUR_SERVER_IP", pubkey=PUBKEY, executor=fake_exec)
...
def test_未知平台安全拒絕_不會誤跑():
    r = eng.onboard("10.0.0.9", "solaris", "u", "pw", "YOUR_SERVER_IP",
