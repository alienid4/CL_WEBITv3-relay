...
def test_平台歸類_真OS優先於推測():
    ...
    assert ms.platform_of("Rocky Linux 9.7") == "Linux"
    assert ms.platform_of("Ubuntu 22.04") == "Linux"
    assert ms.platform_of("Microsoft Windows Server 2022") == "Windows"
    assert ms.platform_of("AIX 7.2") == "AIX/Unix"
    # 真 OS 優先於掃描推測：第一個非空的候選勝出
    assert ms.platform_of("Rocky Linux 9.7", "Windows（RDP 3389）") == "Linux"
...
def test_組成統計_虛實判定容忍混形態():
    ...
    assert c["total"] == 3
    assert c["by_platform"]["Linux"] == 1        # C-1 真 OS
    assert c["by_platform"]["Windows"] == 1      # C-2 靠掃描推測
    assert c["by_platform"]["未知"] == 1          # C-3 沒線索
    # 'VM' 字串與 1 都要算成虛擬機
    assert c["by_virtualization"]["虛擬機"] == 2
    assert c["by_virtualization"]["實體機"] == 1
    assert c["by_environment"] == {"正式": 2, "測試": 1}
    # 誠實揭露：幾台的 OS 是真收到的、幾台是猜的
    assert c["os_from_facts"] == 1 and c["os_guessed"] == 2
