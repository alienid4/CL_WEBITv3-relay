...
def test_counts逐台不逐筆_同台多筆只算一台():
    """2026-09-17 使用者：「檢視每一個公式，確保不會因重複而有低級錯誤」。
    同一台登記多筆時，四態 counts 必須逐台（跟納管漏斗同口徑），不可逐筆灌水——
    否則首頁四格會比漏斗大（實測失聯 4641 筆 vs 4226 台）。items 仍逐筆。"""
    with tempfile.TemporaryDirectory() as tmp:
        db_path = Path(tmp) / "t.db"
        db.init_db(db_path)
        conn = db.get_connection(db_path)
        try:
            # 同一台（同主機名＋IP）登記三筆，都沒掃到 → 應該算 1 台失聯，不是 3
            for i in range(3):
                db.insert_hardware(conn, asset_serial=f"D-{i}", hostname="dup", ip="10.0.0.50",
                                   asset_status="使用中")
            db.insert_hardware(conn, asset_serial="S-1", hostname="solo", ip="10.0.0.51",
                               asset_status="使用中")
            conn.commit()
            s = ms.summarize(conn)
        finally:
            conn.close()
    assert s["counts"][ms.LOST] == 2, "三筆重複＋一台獨立、都沒掃到 → 2 台失聯（逐台）"
    assert s["total_known"] == 2, "四態母體是台數，不是筆數"
    assert len(s["items"]) == 4, "items 仍逐筆（資產清單狀態欄要一列一筆）"
...
def test_虛實判定_is_vm欄位空白時退回device_model判斷():
    """2026-08-25 查證：is_vm 欄位漏填的情況很多——真實資料裡 2536 筆 device_model
    寫著「(VM)」，is_vm 欄位卻是空/0，把運算平台概況的實體機數字灌水快5倍。
    device_model 開頭是 (VM)／VM-／VM( 或整格就是 VM 才算——不能找 VM 子字串，
    否則「ATEN…KVM」這種實體 KVM 切換器會被誤判成虛擬機。"""
    assert ms.is_vm_value(0, "(VM)") is True
    assert ms.is_vm_value(None, "(VM)-Hyper") is True
    assert ms.is_vm_value("0", "VM-Forcepoint WSG") is True
    assert ms.is_vm_value(0, "VM") is True
    assert ms.is_vm_value(0, "VM(VDI)") is True
    # is_vm 欄位本身已經明確標示，device_model 不覆蓋它
    assert ms.is_vm_value(1, "DELL R750") is True
    # KVM 切換器：字尾帶 VM 純屬巧合，不能被 device_model 判成虛擬機
    assert ms.is_vm_value(0, "ATEN SN0116A 16-port Serial Console KVM") is False
    assert ms.is_vm_value(0, "Dell R330 Server  KVM") is False
    assert ms.is_vm_value(0, "DELL R750") is False
    assert ms.is_vm_value(0, None) is False
...
def test_收集器收自己也走SSH(monkeypatch):
    """原本這條測的是反方向（「本機不需要 SSH 就能收集」）。2026-09-11 使用者選 3a 改掉：
    本機捷徑是以「跑服務的帳號」執行（221 是 sysctl，沒有 sudo），收自己拿到的資料跟
    別台用 webit3scan 收的不一樣——帳號盤點 85 條需 root 的欄位就是這樣全查不到；
    而服務帳號之後還會換成 sysinfra。收自己也用收集帳號 SSH，才是同一套身分。
    local_ips 還在（其他地方判斷「這台是不是收集器自己」要用），只是不再決定走哪條路。"""
    import subprocess

    assert "127.0.0.1" in ms.local_ips(), "本機位址至少要含 loopback"

    captured = []

    # ⚠️ 用 CompletedProcess 本尊，不要手捏假物件（2026-09-23）：
    # runner 開始讀 returncode／stderr 之後，手捏的假物件會缺欄位而爆掉。
    # 補欄位只能撐到下一次介面變動；用本尊才不會漂移。
    monkeypatch.setattr(
        subprocess, "run",
        lambda cmd, **kw: captured.append(cmd)
        or subprocess.CompletedProcess(cmd, 0, stdout="", stderr=""))
    ms._runner_for("127.0.0.1", "/no/such/key", account=ms.READONLY_ACCOUNT)("127.0.0.1", "echo x")
    ms._runner_for("203.0.113.9", "/no/such/key", account=ms.READONLY_ACCOUNT)("203.0.113.9", "echo x")
    assert [c[0] for c in captured] == ["ssh", "ssh"], "本機退回了 bash 直跑"


def test_composition_登記與帳外分開算():
    """在管資產頭條要跟公司官方清冊同一種算法：只算正式登記、排除帳外
    （DYN-/VC-/AUTO-）。帳外另用 off_book_total 顯示，不併進 registered_total。
    2026-09-07 使用者定案（兩系統數字才對得起來）。"""
    import tempfile
    from pathlib import Path
    with tempfile.TemporaryDirectory() as tmp:
        db_path = Path(tmp) / "t.db"
        ...
        conn = db.get_connection(db_path)
        try:
            db.insert_hardware(conn, asset_serial="HW-1", ip="10.99.0.1",
                               os="Rocky Linux 9", environment="正式", asset_status="使用中")
            db.insert_hardware(conn, asset_serial="HW-2", ip="10.99.0.2",
                               os="Windows Server 2022", environment="正式", asset_status="使用中")
            db.insert_hardware(conn, asset_serial="DYN-10.99.0.9", ip="10.99.0.9",
                               environment="正式", asset_status="使用中")
            db.insert_hardware(conn, asset_serial="VC-abc", ip="10.99.0.8",
                               environment="正式", asset_status="使用中")
            db.insert_hardware(conn, asset_serial="HW-R", ip="10.99.0.3",
                               os="Rocky Linux 9", environment="正式", asset_status="報廢")
            conn.commit()
            c = ms.composition(conn)
            assert c["registered_total"] == 2      # HW-1/HW-2（HW-R 退役不算）
            assert c["off_book_total"] == 2        # DYN- + VC-
            assert c["total"] == 4                 # 含帳外、排退役
            assert c["registered_total"] + c["off_book_total"] == c["total"]
        finally:
            conn.close()


def test_OCP節點第一期不納管_自動歸非納管設備(tmp_path):
    """2026-09-21 使用者拍板：「OCP 管理放第二期，151 台要移出『要納管』，
    他第一期不納管他」。

    RHCOS 系統區唯讀、本機帳號重佈就消失，要建帳號得改 MachineConfig（動整個節點池）。
    判準用正典 onboard_eligibility.classify_os，而且**只取 immutable**——
    ESXi 與設備也在那支的不納管名單裡，順手改掉等於自己擴大範圍。
    """
    import db
    ...
    db.insert_hardware(c, asset_serial="HW-OCP", hostname="n1.paas-a.ocp.example.com",
                       ip="10.99.1.1", os="CoreOS 4.12", asset_status="使用中")
    db.insert_hardware(c, asset_serial="HW-LNX", hostname="lnx1", ip="10.99.1.2",
                       os="Red Hat Enterprise Linux 9.4", asset_status="使用中")
    db.insert_hardware(c, asset_serial="HW-ESX", hostname="esx1", ip="10.99.1.3",
                       os="VMware ESXi 7.0", asset_status="使用中")
    c.commit()

    by = {r["asset_serial"]: r["state"] for r in ms.summarize(c)["items"]}
    assert by["HW-OCP"] == ms.EXEMPT, "OCP 節點不該再掛在要納管的那一疊"
    assert by["HW-LNX"] != ms.EXEMPT, "一般 Linux 照舊要納管"
    assert by["HW-ESX"] != ms.EXEMPT, "這次只拍 OCP，ESXi 維持原狀不順手改"
