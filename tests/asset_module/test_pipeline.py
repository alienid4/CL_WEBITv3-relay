...
def test_os_type_純函式分類():
    """OS 類型判定：登記的看 os 字串、未登記的看掃到的埠。AIX 未登記分不出來（誠實）。"""
    ...
    # 未登記（無 os 字串）→ 靠埠
    assert pipeline._os_type(None, "22") == "Linux"
    assert pipeline._os_type(None, "3389,445") == "Windows"
    assert pipeline._os_type(None, "5985") == "Windows"
    assert pipeline._os_type(None, "") == "未知"
    assert pipeline._os_type("N/A", None) == "未知"
