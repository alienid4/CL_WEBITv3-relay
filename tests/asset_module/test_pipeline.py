...
def test_重複登記要講得出為什麼(tmp_path):
    """2026-09-20 使用者看著「重複 2」問：「我看不出來為什麼他重複」。

    畫面只寫幾筆等於沒說。同一台多筆登記多半**不是錯**（CIA 一個服務一筆），
    所以要講出是哪一種：帳外自動補的／掛不同業務系統／用途分列／欄位幾乎一樣。
    """
    import db
    import pipeline
    ...
    # 同一台：CIA 兩筆（不同業務系統）＋ 帳外一筆
    db.insert_hardware(c, asset_serial="HW-1", hostname="m1", ip="192.0.2.1", api_id="N-086",
                       asset_status="使用中")
    db.insert_hardware(c, asset_serial="HW-2", hostname="m1", ip="192.0.2.1", api_id="N-232",
                       asset_status="使用中")
    db.insert_hardware(c, asset_serial="DYN-192.0.2.1", hostname="m1", ip="192.0.2.1",
                       asset_status="使用中")
    # 另一台：兩筆 CIA、欄位幾乎一樣 → 疑似真重複
    db.insert_hardware(c, asset_serial="HW-9", hostname="m2", ip="192.0.2.2", api_id="N-001",
                       asset_status="使用中")
    db.insert_hardware(c, asset_serial="HW-10", hostname="m2", ip="192.0.2.2", api_id="N-001",
                       asset_status="使用中")
    c.commit()

    it = {i["ip"]: i for i in pipeline.summarize(c)["items"]}
    r1 = it["192.0.2.1"]["dup_reason"]
    assert "帳外" in r1 and "業務系統" in r1, r1
    assert "疑似真重複" in it["192.0.2.2"]["dup_reason"]
