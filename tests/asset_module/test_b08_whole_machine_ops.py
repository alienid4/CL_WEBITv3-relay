...
def test_整台下線連帳外一起改_但CIA待異動只記CIA那幾筆(tmp_path):
    """2026-09-20 使用者拍板 A：兩張卡片都改（不然永遠掛「登記矛盾」），
    但帳外（DYN-/VC-/AUTO-，系統從 DY／vCenter 自己產的）不在公司清冊上，不該記 CIA 待異動。"""
    c = _conn(tmp_path)
    db.insert_hardware(c, asset_serial="DYN-192.0.2.10", hostname="web1", ip="192.0.2.10",
                       asset_status="使用中")
    c.commit()
    r = cia_pending.batch_set_status(c, ["A-2"], "停用", "現場確認已下線", "t", scope="machine")
    assert "DYN-192.0.2.10" in r["updated"], "帳外那筆也要改，否則同一台自相矛盾"
    pend = {x[0] for x in c.execute("SELECT asset_serial FROM cia_pending_change")}
    assert "DYN-192.0.2.10" not in pend, "帳外不在 CIA 清冊上，不可以叫人回頭改清冊"
    assert {"A-1", "A-2", "A-3"} <= pend

    rows = {x["asset_serial"]: x for x in ms.preview_rows(c, ms.expand_to_machines(c, ["A-2"]))}
    assert rows["DYN-192.0.2.10"]["off_book"] is True, "預覽要標出哪幾筆是系統自動產的"
    assert rows["A-1"]["off_book"] is False
