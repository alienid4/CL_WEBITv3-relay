...
def test_AIX不可以鎖整個帳號():
    """⚠️ 這條測試原本是 `assert "account_locked=true" in s`——**它把 bug 鎖住了**。

    2026-09-22：8 台 AIX 全部納管不起來，真因就是腳本寫了
    `chuser account_locked=true`，註解說「只能用金鑰登入，沒有可用密碼」。
    那是 Linux 的慣用法，AIX 上語意完全不同：
    ...
    但帳號是鎖的。錯誤的心智模型先寫進 docstring，再寫進腳本，
    最後**寫進測試變成「規格」**——於是沒有人會再質疑它。

    AIX 要「只能金鑰、不能密碼」的正確做法是帳號開著但不給密碼：
    mkuser 建出來預設就沒有密碼（/etc/security/passwd 的 password 是 `*`）。
    """
    s = eng.build_aix_script(PUBKEY, COLLECTOR)
    exec_lines = [ln.strip() for ln in s.splitlines()
                  if ln.strip() and not ln.strip().startswith("#")]
    assert not any("account_locked=true" in ln for ln in exec_lines),         "account_locked=true 會連金鑰一起擋，不可以拿它當「鎖密碼」"
    assert any("account_locked=false" in ln for ln in exec_lines),         "已存在的帳號可能被前一版腳本鎖著，一定要主動解鎖"
    # 只看可執行行：註解裡拿 Linux 的 passwd -l 做對照說明是刻意留的，
    # 那正是這個 bug 的成因，寫清楚比刪掉有價值。
    assert not any("passwd -l" in ln for ln in exec_lines), "AIX 沒有 passwd -l"
...
def test_playbook是合法YAML且動作齊全():
    ...
    if yaml is not None:
        ...
        # 比對「該做的動作在不在」，不是比對數量。
        # 2026-08-28 改：原本寫死 len(names) == 4，加一個 stat 判斷就紅了——
        # 但那個判斷正是修 bug 加上去的（沒裝 sudo 的機器沒有 /etc/sudoers.d）。
        # 數量斷言擋的是「加東西」，而這裡真正要擋的是「少做事」。
        for must in ("建立收集帳號", "佈署收集公鑰", "sudo 白名單", "驗證收集帳號"):
            assert any(must in n for n in names), f"playbook 少了「{must}」這個動作"
    else:
...
def test_AIX的一行指令不用base64也不用sudo():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            cmd = r.json()["command"]
            # 2026-08-28：改成 heredoc 送純文字，不再 base64。
            # 現在整份腳本本文都在 cmd 裡，所以「不用 sudo」只能對**呼叫那一行**斷言——
            # 腳本內文的註解本來就會提到 sudo（在解釋為什麼不需要它）。
            invocation = cmd.splitlines()[0]
            assert invocation.startswith("ksh -s <<")
            assert "sudo" not in invocation
            assert "base64" not in cmd
            assert "mkuser" in cmd            # 腳本本文確實在裡面，不是空殼
