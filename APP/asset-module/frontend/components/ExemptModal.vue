...
// [B-08] 豁免本質上就是整台：後端會展開成同一台的所有登記，這裡先預覽給人看
...
async function submit() {
  ...
  try {
    ...
    showToast(`已標記 ${r.added.length} 筆為非納管設備`
...
<template>
  <!-- 點背景**不關視窗**（2026-09-17 使用者：「這個頁面如果我沒有點好，很容易整個頁面就跳掉」）。
    ...
    <div class="box">
      ...
      <!-- [B-08] 豁免＝整台（同一台的所有登記一起），動作前先預覽 -->
      <ScopePreview :serials="serials" scope="machine" @count="previewCount = $event" />
      <p class="note">
      ...
      <div class="acts">
        ...
        <button class="btn primary" type="button" :disabled="busy || previewCount === null" @click="submit">
          {{ busy ? '處理中…' : (previewCount !== null ? `確定（標 ${previewCount} 筆）` : '確定') }}
        </button>
      </div>
    </div>
  </div>
