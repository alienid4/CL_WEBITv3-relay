...
// 批次標已處理（2026-09-19）：失聯會越積越多，逐筆點很慢。勾選只作用在「未處理」的列。
const sel = ref<Set<number>>(new Set())
const batching = ref(false)
const unreadShown = computed(() => sorted.value.filter((r) => !r.is_read))
const allSel = computed(() => unreadShown.value.length > 0 && unreadShown.value.every((r) => sel.value.has(r.id)))
function togSel(id: number) {
  const s = new Set(sel.value); s.has(id) ? s.delete(id) : s.add(id); sel.value = s
}
function togAll() {
  sel.value = allSel.value ? new Set() : new Set(unreadShown.value.map((r) => r.id))
}
async function markBatch() {
  const ids = [...sel.value]
  if (!ids.length) { showToast('先勾選要標記的問題', 'warn'); return }
  batching.value = true
  try {
    const r = await apiFetch<{ marked: number }>('/api/issues/mark-read', { method: 'POST', body: { ids } })
    showToast(`已批次標記 ${r.marked} 筆已處理`, 'success')
    sel.value = new Set()
    await load()
  } catch (err: any) {
    showToast(err?.data?.detail ?? '批次標記失敗，請稍後再試', 'error')
  } finally {
...
<template>
  <div>
    <div class="tabs">
      ...
      <span class="spacer" />
      <button v-if="sel.size" class="tab batch" type="button" :disabled="batching" @click="markBatch">
        {{ batching ? '處理中…' : `批次標已處理（${sel.size}）` }}
      </button>
    </div>
    ...
    <table v-else class="tbl">
      <thead>
        <tr>
          <th class="ck-col"><input v-if="unreadShown.length" type="checkbox" :checked="allSel" title="全選未處理" @change="togAll" /></th>
      ...
      <tbody>
        <tr v-for="row in sorted" :key="row.id">
          <td class="ck-col"><input v-if="!row.is_read" type="checkbox" :checked="sel.has(row.id)" @change="togSel(row.id)" /></td>
          <td>{{ row.hostname || '—' }}</td>
          <td class="mono">
...
.tab.batch { border-color: var(--brand); color: #fff; background: var(--brand); }
.tab.batch:hover:not(:disabled) { background: var(--brand-hover); }
.tab.batch:disabled { opacity: 0.5; cursor: default; }
.spacer { flex: 0; }
.ck-col { width: 32px; text-align: center; }
