...
interface Props {
  ...
  /** 查無符合時，可直接把輸入的字當新值（＋二次確認）——例：人名打錯字太容易，選單防呆但要留新增退路 */
  allowNew?: boolean
  /** 二次確認用的名詞，例：使用者、保管者 */
  newLabel?: string
}
...
const showSearch = computed(() => props.options.length >= props.searchThreshold || props.allowNew)
const filtered = computed(() => {
  const needle = q.value.trim().toLowerCase()
  if (!needle) return props.options
  return props.options.filter((o) => o.toLowerCase().includes(needle))
})
// 可新增：allowNew 開、有打字、且清單裡沒有一模一樣的值
const canAddNew = computed(() =>
  props.allowNew && !!q.value.trim() && !props.options.some((o) => o === q.value.trim()))
function addNew() {
  const v = q.value.trim()
  if (!v) return
  // 二次確認：人名太容易打錯，清單沒有就先問一句（使用者 2026-09-17 要「查無可新增」＋防打錯字）
  if (!window.confirm(`清單裡沒有「${v}」，確定新增這個${props.newLabel}？\n（請確認不是打錯字）`)) return
  pick(v)
}
...
function onKey(e: KeyboardEvent) {
  ...
  if (e.key === 'ArrowDown' && n) {
    ...
    e.preventDefault()
    // 查無候選、但允許新增 → Enter 直接新增打的字
    if (!n && canAddNew.value) { addNew(); return }
    ...
    <div v-if="open" class="panel">
      ...
      <div ref="listRef" class="list" role="listbox">
        ...
        <button
          ...
          @mouseenter="cursor = i"
        >{{ opt }}</button>
        <button v-if="canAddNew" type="button" class="opt add" @click="addNew">＋ 新增「{{ q.trim() }}」</button>
        <p v-if="filtered.length === 0 && !canAddNew" class="empty">找不到符合「{{ q }}」的選項</p>
      </div>
    </div>
  </div>
</template>
...
.opt.sel { color: var(--brand-dark); font-weight: 700; }
.opt.clear { color: var(--muted); }
.opt.add { color: var(--brand-dark); font-weight: 700; }
