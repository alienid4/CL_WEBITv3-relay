...
<template>
  <div class="page">
    <div class="hd">
      <h1>各環境實體機分布現況 <InfoNote>本頁只算<b>實體機</b>（虛擬機不計入）；口徑＝CIA 正式登記，排除退役、排除網路/儲存/BMC/未知（跟系統組月報表1同一套排除規則，兩頁數字對得起來）。</InfoNote></h1>
    ...
    <template v-else-if="data">
      ...
      <div v-if="data.excluded_models_total" class="card excl">
        <div class="card-title">
          ...
          <InfoNote>這些機器的 OS 欄位看起來像伺服器（例如 TippingPoint 寫 Linux），但實際是 PC／筆電／入侵偵測設備，不計入機房實體機。清單由人維護在 <code>report_groups.json</code>，改設定檔即可調整，不用改程式。</InfoNote>
        </div>
        <div class="ob-rows">
        ...
        <!-- 排除的是視角不是資料，這句話 2026-08-26 使用者要求印在畫面上（金融稽核缺失常來自「每個人都以為別人在管」）；
             依 2026-09-07 儀表板鐵律的 critical 例外，留一行警語在畫面、詳細理由收進註。 -->
        <p class="tip warn">
          ⚠ <b>排除的是視角，不是資料</b>——這些機器仍會 EOS、要打補丁，本表不列入不代表沒人管
          <InfoNote>這些機器仍在資產庫裡、仍然要打補丁、仍然被稽核問——特別是入侵偵測與金流加密設備。本表不列入只代表「它們不是機房伺服器」，<b>不代表沒人要管</b>；請確認它們在各自的權責清單上有歸屬。金融業的稽核缺失很多是這樣來的：不是沒人管，是每個人都以為別人在管。</InfoNote>
        </p>
      </div>
      ...
      <div v-if="offBookTotal" class="card offbook">
        <div class="card-title">另有 {{ offBookTotal.toLocaleString() }} 台不在 CIA 清單上（未列入以上台數）
          <InfoNote>這批是實際存在、但不在資產清單上的機器——正是待複核佇列卡住的來源，值得處理但不能算進報告主數字。</InfoNote>
        </div>
        <div class="ob-rows">
