...
// 功能也要搜得到（2026-09-16 使用者：「記得做功能搜尋」）。
// 後端 /api/search 查的是資料，功能目錄在前端（跟選單同一份），所以在這裡本地比對，
// 不為了搜尋在後端再抄一份頁面清單——抄了就會漂走。
// 放在最前面：打「ping」「下線」這種字時，多半是在找功能而不是找某台機器。
const featureGroup = computed(() => {
  const k = q.value.trim()
  if (!k) return null
  const hits = searchFeatures(k).slice(0, 6)
  if (!hits.length) return null
  return {
    label: '功能',
    items: hits.map((f) => ({
      title: f.label,
      subtitle: `${f.kind === 'action' ? '動作' : '頁面'}　${f.where}　·　${f.desc}`.slice(0, 90),
      to: f.to,
    })),
  } as Group
})
...
const shownGroups = computed(() => (featureGroup.value ? [featureGroup.value, ...groups.value] : groups.value))
const flat = computed(() => shownGroups.value.flatMap(g => g.items))

// 使用者 2026-09-16：「搜尋後按 enter 就是會執行第一行，而不是要人再點選一次」。
// 原本 Enter 已經會跳第一筆，但打完字馬上按 Enter 時結果還沒回來（去抖動 300ms），
// flat 是空的就直接 return，看起來像沒反應。改成記住這個意圖：搜尋一回來就跳。
let timer: any = null
...
async function search() {
  ...
  try {
    ...
    open.value = true
    // 使用者在結果回來之前就按了 Enter → 現在有結果了，直接帶他去第一筆
    if (enterPending.value) {
      enterPending.value = false
      const first = flat.value.find((i) => i.to)
...
function go(to: string) {
  if (!to) return          // 沒有目的頁的結果（通用掃表）不可點，避免跳去不存在的路由 404
  open.value = false
  q.value = ''
  router.push(to)
}
...
function onKey(e: KeyboardEvent) {
  if (e.key === 'Escape') { open.value = false; inputRef.value?.blur(); enterPending.value = false; return }
  if (e.key === 'Enter' && (flat.value.length === 0 || loading.value)) {
    // 結果還沒回來就按 Enter：立刻查（不等去抖動），查到再跳
    e.preventDefault()
  ...
  if (e.key === 'ArrowDown') { e.preventDefault(); cursor.value = (cursor.value + 1) % flat.value.length }
    ...
    e.preventDefault()
    // 沒有用方向鍵選過就走第一筆「點得進去」的結果——
    // 第一筆若是通用掃表那種沒有目的頁的，跳過它，不然按了等於沒反應
    const item = cursor.value >= 0 ? flat.value[cursor.value] : flat.value.find((i) => i.to)
    if (item) go(item.to)
  }
}
...
<template>
  <div ref="boxRef" class="gs">
    ...
    <div v-if="open" class="panel">
      ...
      <p v-else-if="shownGroups.length === 0" class="st">
        找不到「{{ q }}」。
      ...
      <template v-else>
        <div v-for="(g, gi) in shownGroups" :key="g.key || g.label" class="grp">
          <div class="ghd">{{ g.label }}</div>
          <button
            v-for="(it, ii) in g.items"
            :key="`${g.key || g.label}-${ii}`"
            class="row"
            :class="{ on: cursor === indexOf(gi, ii), nolink: !it.to }"
            type="button"
            @click="go(it.to)"
          ...
          <button v-if="g.more && g.more_to" class="more" type="button" @click="go(g.more_to!)">
            看全部 {{ g.label }} →
          </button>
        </div>
      </template>
...
.row.nolink { cursor: default; }
.row.nolink.on { background: none; }
