...
const ACCOUNT_HINT: Record<string, string> = {
  linux: '能 sudo 建帳號的管理帳號（例：sysinfra）；沒有 sysinfra 就用 root＋root 密碼也可以',
...
async function copyCmd() {
  try {
    const ok = await copy(localCmd.value)
  if (!ok) showToast('複製失敗，請直接選取下方指令複製', 'warn', 8000)
    copied.value = true
...
async function copyPlaybook() {
  try {
    const ok2 = await copy(playbook.value)
  if (!ok2) showToast('複製失敗，請直接選取下方內容複製', 'warn', 8000)
    pbCopied.value = true
...
async function submit() {
  ...
  try {
    ...
    if (r.ok) {
      ...
      // 腳本跑成了、帳號也建好了，但**一個欄位都收不到**——這不是「納管失敗」四個字
      // 講得清楚的事。2026-09-22 使用者就是在這裡被誤導：畫面報成功，
      // 然後帳號／軟體／硬體全空，他做了三次才發現。
      // 兩種失敗要分開講，因為下一步完全不同：
      //   腳本失敗 → 帳密或權限問題，重跑
      //   收不到   → 帳號建好了，是平台判錯或收集端問題，重跑沒有用
      failed.value = { stage: r.stage, message: r.message, output: r.output || '' }
      showToast('收集帳號已建立，但收不到任何資料', 'error')
    } else {
      failed.value = { stage: r.stage, message: r.message, output: r.output || '' }
...
<style scoped>
.stage { color: var(--brand-dark); font-size: 12px; }
...
.vresult b { color: var(--brand-dark); }
...
.mhint code { color: var(--brand-dark); }
...
.mguess { font-size: 11px; color: var(--brand-dark); }
...
.mtab.on { border-color: var(--brand); color: var(--brand-dark); background: rgba(0,145,66,.1); font-weight: 700; }
.cmdbox.tall { height: 300px; }
/* <pre> 預設 white-space:pre 不換行，主機吐回來的長訊息會整行衝出畫面右邊
   （2026-09-08 公司截圖：sudo 那句 `either use` 之後全被切掉，看不到重點）。
   .cmdbox 雖然有 word-break:break-all，但沒有允許換行時它不生效。 */
pre.cmdbox { white-space: pre-wrap; overflow: auto; margin: 0; }
.mhint code, .ahint code { font-family: ui-monospace, Consolas, monospace; color: var(--brand-dark); }
...
.mtip { font-size: 11px; color: var(--ink-aux); background: rgba(127,179,234,.1);
  border-radius: 6px; padding: 7px 10px; margin: -4px 0 12px; line-height: 1.5; }
.mconflict { font-size: 11.5px; color: var(--warn-text); background: rgba(255,184,103,.1);
