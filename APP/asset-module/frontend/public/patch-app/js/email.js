...
(function (global) {
  ...
  /* 2026-09-21 搬上系統版：Email 設定全站一份（不是每個人自己設一次）。 */
  function loadCfg() {
    var v = global.Store.shared.getJSON('mail_config');
    return v ? Object.assign({}, DEFAULTS, v) : Object.assign({}, DEFAULTS);
  }
  function saveCfg(cfg) {
    return global.Store.shared.setJSON('mail_config', cfg);
  }
  ...
  /* 發信紀錄改共享：不共用會重複催同一個人（A 催過了 B 看不到）。 */
  function loadLog() {
    var v = global.Store.shared.getJSON('mail_log');
  ...
  function appendLog(entries) {
    ...
    if (global.Store.shared.setJSON('mail_log', log)) return true;
    /* 寫入失敗原本靜默：使用者會看到舊紀錄而誤以為信沒寄出，進而重寄 */
    UI.toast('信已寄出，但發信紀錄未能寫入：'
             + (global.Store.shared.error() || '未知原因'), 'error');
    return false;
  }
  function nowStamp() {
  ...
  function buildForm(cfg) {
    var iHost = U.el('input', { type: 'text', class: 'email-input', value: cfg.smtpHost, placeholder: '公司 SMTP relay 主機（存在本系統伺服器，同單位共用）' });
    ...
    function doViewLog() {
      ...
      var footer = U.el('div', { class: 'reminder-actions' }, [
        ...
        U.el('button', { class: 'btn btn-secondary', text: '清空紀錄', disabled: log.length ? null : 'disabled',
          onclick: function () { global.Store.shared.remove('mail_log'); UI.toast('已清空發信紀錄', 'success'); UI.closeModal(); /* 只收這一層，回到 Email 設定 */ } }),
      ]);
