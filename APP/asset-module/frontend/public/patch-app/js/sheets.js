...
(function (global) {
  ...
  function parseSheet(ws, name) {
    ...
    // 原始欄位順序(去掉空白表頭)：供「完整欄位匯出」用，逐筆掛在紀錄上(同表共用同一份參照)
    var rawCols = keys.filter(function (k) { return k !== ''; });
    for (var i = 1; i < matrix.length; i++) {
      ...
      // 保留這一列的原始完整欄位(_raw)與欄序(_cols)；一列展開多筆時共用同一份原始列
      buildRecords(obj, map, name).forEach(function (r) { r._raw = obj; r._cols = rawCols; records.push(r); });
    }
