...
import type { ShownGroup } from '~/composables/useNav'
...
async function loadOnline() {
  // 線上人數是外殼上的裝飾資訊，載不到就不顯示那顆徽章即可——
      // 它不是任何判斷的依據，也不會被誤讀成「沒有人在線」（整個徽章會消失，不是顯示 0）。
...
onMounted(async () => {
  // 版號徽章同理：載不到就整個不顯示，不會變成一個錯的版號誤導人
...
// 合併邏輯在 composables/useNav.ts 的 useVisibleNav()——功能查詢頁用同一個，編號不會兩套。
const { savedLayout, canEditNav, visibleNav, loadNavLayout } = useVisibleNav()
...
/* 側欄固定（2026-09-16 使用者：「左邊的功能欄能固定嗎」）——
   資產清單動輒幾千列，往下捲之後功能欄就不見了，要換頁得先捲回最上面。
   sticky 而非 fixed：仍然佔住格線那一欄，不必替內容補 margin-left。
   自己可捲：側欄項目比一個螢幕高時，它自己有捲軸，不會被切掉看不到。 */
