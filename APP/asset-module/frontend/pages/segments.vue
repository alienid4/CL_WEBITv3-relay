...
// 快速多選（2026-09-12 使用者）：機房×環境×用途各自可複選，同維度 OR、跨維度 AND，
// 讓「板橋+敦南的測試」「內湖的 DMZ」這種組合一鍵篩出來。空＝該維度不限。
const roomSel = ref<Set<string>>(new Set())
const envSel = ref<Set<string>>(new Set())
const useSel = ref<Set<string>>(new Set())
// ⚠️ 不能把 ref 當參數傳（template 會自動 unwrap 成 Set 本身，sel.value 就是 undefined、
// 點了沒反應）。改用維度名稱在函式內部取對應 ref。
function togSel(dim: 'room' | 'env' | 'use', v: string) {
  const sel = dim === 'room' ? roomSel : dim === 'env' ? envSel : useSel
...
const filtered = computed(() => {
  ...
  return rows.value.filter((r) => {
    if (roomSel.value.size && !roomSel.value.has(r.location ?? '')) return false
    if (envSel.value.size && !envSel.value.has(r.environment ?? '')) return false
    if (useSel.value.size && !useSel.value.has(r.usage ?? '')) return false
...
const envOptions = computed(() =>
  [...new Set(rows.value.map((r) => r.environment).filter(Boolean))].sort() as string[])
const usageOptions = computed(() =>
  [...new Set(rows.value.map((r) => r.usage).filter(Boolean))].sort() as string[])
...
<template>
  <div>
    ...
    <div class="breadcrumb-bar"><span class="pin">📌</span> <b>網段配置表</b>
      <InfoNote>來源是公司的「總分公司網段配置表」Excel。<b>匯入是整批取代</b>——Excel 是這份清單的唯一真相，段被刪掉就該從系統消失；累加式匯入會讓作廢網段永遠留著，越用越髒。解析不掉的寫法（一格兩段、IP 範圍）<b>會保留並列出警告</b>，不會被靜默丟掉。</InfoNote>
    </div>

    <div class="card">
      <div class="bar-row">
        <!-- 匯入／匯出入口已移除（2026-09-12 使用者：匯入匯出已在資料匯入功能，這頁只做檢視／篩選／下鑽）。 -->
        <div class="stats">
    ...
    <div class="card">
      <div class="filters">
        ...
        <!-- 快速多選：機房×環境×用途（同維度可複選=OR、跨維度=AND） -->
        <div class="qsel">
          <div class="qrow">
            <span class="qlbl">機房</span>
            <button v-for="l in locations" :key="l" class="chip" :class="{ on: roomSel.has(l) }" @click="togSel('room', l)">{{ l }}</button>
            <button v-if="roomSel.size" class="chip clr" @click="roomSel = new Set()">清除</button>
          </div>
          <div class="qrow">
            <span class="qlbl">環境</span>
            <button v-for="e in envOptions" :key="e" class="chip" :class="{ on: envSel.has(e) }" @click="togSel('env', e)">{{ e }}</button>
            <button v-if="envSel.size" class="chip clr" @click="envSel = new Set()">清除</button>
          </div>
          <div class="qrow" v-if="usageOptions.length">
            <span class="qlbl">用途</span>
            <button v-for="u in usageOptions" :key="u" class="chip" :class="{ on: useSel.has(u) }" @click="togSel('use', u)">{{ u }}</button>
            <button v-if="useSel.size" class="chip clr" @click="useSel = new Set()">清除</button>
          </div>
        </div>
        <div class="tabs">
      ...
      <p v-else-if="rows.length === 0" class="muted">
        還沒有網段資料。到 <NuxtLink to="/import">6A-1 資料匯入</NuxtLink> 的「網段配置表」那一列上傳 Excel。
      </p>
            ...
            <template v-for="r in sorted" :key="r.id">
              <tr :class="{ clickable: !!r.cidr, on: openCidr === r.cidr }" @click="drill(r)">
                ...
                <td class="mono">
                  ...
                  <!-- 使用率條：已登記 ÷ 容量，一眼看出哪段快滿（>85% 轉紅） -->
                  <div v-if="r.capacity" class="usebar" :title="`使用率 ${Math.round(r.asset_count / r.capacity * 100)}%`">
                    <span :style="{ width: Math.min(100, Math.round(r.asset_count / r.capacity * 100)) + '%' }"
                          :class="{ hot: r.asset_count / r.capacity >= 0.85 }" />
                  </div>
                </td>
                <td>
...
.qsel { flex-basis: 100%; display: flex; flex-direction: column; gap: 6px; }
.qrow { display: flex; align-items: center; gap: 6px; flex-wrap: wrap; }
.qlbl { font-size: 12px; color: var(--muted); min-width: 34px; }
...
.cap { color: var(--muted); }
.usebar { margin-top: 3px; height: 4px; width: 60px; background: var(--border); border-radius: 3px; overflow: hidden; }
.usebar span { display: block; height: 100%; background: var(--brand); }
.usebar span.hot { background: var(--bad); }
