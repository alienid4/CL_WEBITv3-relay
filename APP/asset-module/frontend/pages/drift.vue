...
<template>
  <div>
    ...
    <div class="breadcrumb-bar"><span class="pin">📌</span> <b>基線失效</b>
      <InfoNote>上線前檢查通過時，機器可驗的項目會被記成<b>基線</b>——意思是「這台被刻意設定成這樣」。系統每天跟著排程掃描回檢一次，跟基線不一樣就列在這裡。<b>收不到資料的主機不會列入</b>（沒去收 ≠ 設定被改，那種假告警會讓整張表沒人看）。<br><br>「已確認」只是記下你看過了，<b>不會改掉基線</b>——機器實際恢復成基線時，下一次回檢會自動把它關掉。</InfoNote>
    </div>
...
.btn.ghost:hover:not(:disabled) { border-color: var(--brand); color: var(--brand-dark); }
.btn:disabled { opacity: 0.5; cursor: not-allowed; }
.muted { color: var(--muted); font-size: 12.5px; }
.ok-empty { color: var(--brand-dark); }
