...
// 只出現一次的人名＝打錯字高風險候選（人名改可搜尋新增後要能定期檢查，2026-09-17）
const singleNames = ref<string[]>([])
const singleOpen = ref(false)
onMounted(async () => {
  try { singleNames.value = (await apiFetch<{ names: string[] }>('/api/data-quality/single-person-names')).names } catch { /* 不擋整頁 */ }
})

// 人名合併（預覽版，2026-09-19）：把打錯的名字併到正確名字。先預覽會改幾筆，確認才寫。
const { showToast } = useToast()
interface MergePreview {
  from_name: string; to_name: string; total: number
  fields: { label: string; count: number }[]
  sample: { asset_serial: string; hostname: string | null; field: string }[]
}
const mergeFrom = ref('')
const mergeTo = ref('')
const mergePreview = ref<MergePreview | null>(null)
const mergeBusy = ref(false)
function pickMergeFrom(n: string) { mergeFrom.value = n; mergePreview.value = null }
async function doMergePreview() {
  if (!mergeFrom.value.trim() || !mergeTo.value.trim()) { showToast('原名與正確名都要填', 'warn'); return }
  mergeBusy.value = true
  try {
    mergePreview.value = await apiFetch<MergePreview>('/api/data-quality/person-merge/preview',
      { method: 'POST', body: { from_name: mergeFrom.value, to_name: mergeTo.value } })
    if (mergePreview.value.total === 0) showToast('這個原名在資料裡找不到（要完全一致）', 'warn')
  } catch (e: any) {
    showToast(e?.data?.detail ?? '預覽失敗，請稍後再試', 'error')
  } finally {
    mergeBusy.value = false
  }
}
async function doMergeApply() {
  if (!mergePreview.value || !mergePreview.value.total) return
  if (!confirm(`確定把「${mergePreview.value.from_name}」全部改成「${mergePreview.value.to_name}」？共 ${mergePreview.value.total} 筆，這個動作會直接改資料。`)) return
  mergeBusy.value = true
  try {
    const r = await apiFetch<{ total: number }>('/api/data-quality/person-merge',
      { method: 'POST', body: { from_name: mergePreview.value.from_name, to_name: mergePreview.value.to_name } })
    showToast(`已合併 ${r.total} 筆`, 'success')
    mergePreview.value = null; mergeFrom.value = ''; mergeTo.value = ''
    // 重載「只出現一次人名」——併完錯名應該消失
    try { singleNames.value = (await apiFetch<{ names: string[] }>('/api/data-quality/single-person-names')).names } catch { /* 不擋 */ }
  } catch (e: any) {
    showToast(e?.data?.detail ?? '合併失敗，請稍後再試', 'error')
  } finally {
    mergeBusy.value = false
  }
}

// 填充率最低的欄位＝「待補資料」清單（欄位先立、機器後補；機房搬遷的北極星，2026-09-19）
interface FieldFill { column: string; meaning: string; filled: number; fill_rate: number | null }
const fieldFill = ref<FieldFill[]>([])
const fillTotal = ref(0)
onMounted(async () => {
  try {
    const r = await apiFetch<{ fields: FieldFill[]; total_rows: number }>('/api/data-quality/field-fill')
    fieldFill.value = r.fields
    fillTotal.value = r.total_rows
  } catch { /* 輔助看板，載不到不擋整頁 */ }
})
const lowestFill = computed(() => fieldFill.value.slice(0, 12))

// 下鑽清單可匯出（2026-09-18 十項盤點第 8 條）：像「主機名與 IP 都有」這種要交給 CIA 清冊
// 維護單位回頭補的清單，只能在畫面上看等於沒用。BOM 讓 Excel 開中文不亂碼。
function exportOffenders(d: Dim) {
  const cols: [keyof Offender, string][] = [
    ['asset_serial', '資產序號'], ['hostname', '主機名稱'], ['ip', 'IP'], ['asset_status', '資產狀態'],
    ['os', 'OS'], ['inventory_department', '盤點部門'], ['custodian', '保管者'], ['updated_at', '最後更新'],
    ['reason', '原因'],
  ]
  const esc = (v: unknown) => { const s = String(v ?? ''); return /[",\n]/.test(s) ? '"' + s.replace(/"/g, '""') + '"' : s }
  const lines = [cols.map((c) => c[1]).join(',')]
  for (const r of oSorted.value) lines.push(cols.map(([k]) => esc((r as any)[k])).join(','))
  const blob = new Blob(['\uFEFF' + lines.join('\r\n')], { type: 'text/csv;charset=utf-8' })
  const a = document.createElement('a')
  a.href = URL.createObjectURL(blob)
  a.download = `資料品質_${d.label}_${new Date().toISOString().slice(0, 10)}.csv`
  a.click()
...
interface TrustDist {
  total: number; machines?: number; rule: string
...
const EVID_NAME: Record<string, string> = { managed: '已納管 +30', collected: '已收集（設備）+30', alive: '網路通 +20' }

onMounted(async () => {
  // 可信度分佈是這頁的輔助小圖，載不到就不顯示那一塊、不擋主資料（trust.value=null → v-if 收起）
  ...
  try {
    ...
    trustHosts.value = r.items.map((h) => ({
      ...
      evid_text: h.evidence === 'managed' ? '已納管 +30'
        : h.evidence === 'collected' ? '已收集（設備）+30'
        : h.evidence === 'alive'
  ...
  <div>
    ...
    <template v-else-if="summary">
      ...
      <div v-if="trust" class="card">
        <div class="card-title">資料可信度分布（{{ trust.total }} 筆登記<span v-if="trust.machines">／{{ trust.machines }} 台</span>，非退役）
          <InfoNote><b>一筆登記一個分數，越高越可信。</b>分數問的是「這一筆登記有幾個來源佐證」，所以母體是<b>登記筆數</b>；同一台若在 CIA 與 DY 各有一筆，會各自計分。括號裡另外附去重後的台數（2026-09-20 修：原本只寫「台」，其實是筆）。<br><br><br>{{ trust.rule }}。<br><br>實體機不會出現在 RVTools，所以實體機只算 dynassets＋CIA（照比例換算成 70 分滿分）；型態不明的照 VM 算。<br><br>第一版的「有」＝這個來源裡有這台；欄位對不上（IP、主機名不同）先不扣分。<br><br>點筆數看是哪幾筆。</InfoNote>
        </div>
          ...
          <div class="tbl-wrap">
            <table>
              <thead><tr>
                ...
                <SortTh k="count" :active="sKey" :dir="sDir" @sort="sToggle">筆數</SortTh>
                <SortTh k="reason_text" :active="sKey" :dir="sDir" @sort="sToggle">
                  分數怎麼來的 <InfoNote>型態・有哪幾個來源（來源分）＋機器證據（加分）。同一個分數可能由不同組合湊出來，分別列出各幾筆，加總就是左邊的筆數。</InfoNote>
                </SortTh>
              </tr></thead>
              <tbody>
          ...
          <div class="tbl-wrap">
            <table>
              <thead><tr>
                ...
                <SortTh k="count" :active="cbKey" :dir="cbDir" @sort="cbToggle">筆數</SortTh>
              </tr></thead>
              <tbody>
      ...
      <!-- 填充率最低的欄位＝待補資料（欄位先立、機器後補，2026-09-19） -->
      <div v-if="lowestFill.length" class="card">
        <div class="card-title">填充率最低的欄位（待補資料）
          <InfoNote>系統已立好的欄位裡，目前收得最少的前 12 個。欄位先立、機器後補——這裡就是「還缺什麼」。分母＝{{ fillTotal }} 筆 hardware。</InfoNote>
        </div>
        <div class="fill-list">
          <div v-for="f in lowestFill" :key="f.column" class="fill-row">
            <div class="fill-meta"><b>{{ f.meaning }}</b><span class="fmono">{{ f.column }}</span></div>
            <div class="fill-bar"><span :style="{ width: Math.round((f.fill_rate ?? 0) * 100) + '%' }" /></div>
            <div class="fill-pct mono">{{ Math.round((f.fill_rate ?? 0) * 100) }}%<span class="fdim">（{{ f.filled }}）</span></div>
          </div>
        </div>
      </div>

      <!-- SSH 測連線（2026-09-15 使用者：「把測試小工具也一次放進去，可以放在品質分析裡面」） -->
      <SshProbe />
              ...
              <template v-for="d in sorted" :key="d.key">
                ...
                <tr v-if="openKey === d.key" class="drill">
                  <td colspan="7">
                    ...
                    <div v-else class="tbl-wrap inner">
                      <div class="off-bar">
                        共 {{ offenders.length }} 筆
                        <button class="btn small" type="button" @click.stop="exportOffenders(d)">匯出 CSV</button>
                      </div>
                      <table>
                        <thead>
                          <tr>
      ...
      <!-- 只出現一次的人名：打錯字高風險候選（2026-09-17 使用者） -->
      <div class="card">
        <button type="button" class="dq-toggle" @click="singleOpen = !singleOpen">
          {{ singleOpen ? '▾' : '▸' }} 只出現一次的人名（可能打錯字）<b>{{ singleNames.length }}</b> 個
          <InfoNote>跨「使用者／保管者／人員名冊」統計，只用在一筆的名字。可能是真的只管一台，也可能是把「李太明」打成「李太名」。人名改成可搜尋＋可新增後，這裡定期看一眼、抓出近似的錯字。</InfoNote>
        </button>
        <div v-if="singleOpen" class="names">
          <span v-for="n in singleNames" :key="n" class="nchip" :class="{ on: mergeFrom === n }"
                title="點一下帶入下方合併的「原名」" @click="pickMergeFrom(n)">{{ n }}</span>
          <p v-if="!singleNames.length" class="muted">沒有只出現一次的人名。</p>
        </div>
        <!-- 一鍵合併（預覽版）：先看會改幾筆，確認才寫。改的是真資料，所以強制預覽這道關。 -->
        <div v-if="singleOpen" class="merge">
          <div class="merge-hd">合併到正確人名 <InfoNote>把打錯字的名字（原名）併到正確名字。會改 <b>使用者／保管者／人員名冊</b> 三處完全一致的值。<b>先按「預覽」看會動幾筆</b>，確認無誤再按「確認合併」——這個動作直接改資料。</InfoNote></div>
          <div class="merge-row">
            <input v-model="mergeFrom" class="minp" type="text" placeholder="原名（打錯的，可點上面名字帶入）" />
            <span class="marrow">→</span>
            <input v-model="mergeTo" class="minp" type="text" placeholder="正確名" />
            <button class="btn small" type="button" :disabled="mergeBusy" @click="doMergePreview">預覽</button>
          </div>
          <div v-if="mergePreview" class="merge-prev">
            <template v-if="mergePreview.total">
              <p class="mp-sum">會把「<b>{{ mergePreview.from_name }}</b>」改成「<b>{{ mergePreview.to_name }}</b>」，共 <b>{{ mergePreview.total }}</b> 筆：
                <span v-for="f in mergePreview.fields" :key="f.label" class="mp-f">{{ f.label }} {{ f.count }}</span>
              </p>
              <div v-if="mergePreview.sample.length" class="mp-sample">
                影響的機器（前 {{ mergePreview.sample.length }}）：
                <span v-for="s in mergePreview.sample" :key="s.asset_serial + s.field" class="mp-chip">
                  {{ s.hostname || s.asset_serial }}<i>{{ s.field }}</i>
                </span>
              </div>
              <button class="btn small danger" type="button" :disabled="mergeBusy" @click="doMergeApply">確認合併（{{ mergePreview.total }} 筆）</button>
            </template>
            <p v-else class="muted">找不到完全一致的「{{ mergePreview.from_name }}」，沒有可合併的資料。</p>
          </div>
        </div>
      </div>
...
<style scoped>
.fill-list { display: flex; flex-direction: column; gap: 6px; }
.fill-row { display: grid; grid-template-columns: minmax(180px, 1.4fr) 2fr auto; align-items: center; gap: 10px; font-size: 12.5px; }
.fill-meta { display: flex; flex-direction: column; }
.fill-meta b { color: var(--ink); font-weight: 600; }
.fmono { font-family: ui-monospace, monospace; font-size: 11px; color: var(--muted); }
.fill-bar { height: 8px; background: var(--border); border-radius: 4px; overflow: hidden; }
.fill-bar span { display: block; height: 100%; background: var(--brand); }
.fill-pct { color: var(--ink-soft); white-space: nowrap; }
.fdim { color: var(--muted); font-size: 11px; }
...
.dq-toggle { width: 100%; text-align: left; background: none; border: none; cursor: pointer;
  font-family: inherit; font-size: 14px; font-weight: 600; color: var(--ink); padding: 4px 0; }
.dq-toggle b { color: var(--brand-dark); font-family: var(--disp); margin: 0 2px; }
.names { display: flex; flex-wrap: wrap; gap: 6px; margin-top: 10px; }
.nchip { font-size: 12.5px; padding: 3px 10px; border-radius: 999px; background: var(--sub);
  color: var(--ink); border: 1px solid var(--border); cursor: pointer; }
.nchip:hover { border-color: var(--brand); }
.nchip.on { background: var(--mint); border-color: var(--brand); color: var(--brand-dark); font-weight: 600; }
.merge { margin-top: 14px; padding-top: 12px; border-top: 1px dashed var(--border); }
.merge-hd { font-size: 12.5px; font-weight: 600; color: var(--ink-soft); margin-bottom: 8px; }
.merge-row { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
.minp { font-family: inherit; font-size: 12.5px; padding: 6px 10px; min-width: 200px;
  border: 1px solid var(--border-strong); background: var(--card); color: var(--ink); border-radius: 6px; }
.marrow { color: var(--muted); }
.merge-prev { margin-top: 10px; font-size: 12.5px; }
.mp-sum { color: var(--ink-soft); margin: 0 0 6px; }
.mp-f { display: inline-block; margin-left: 8px; padding: 1px 8px; border-radius: 999px;
  background: var(--sub); font-size: 11.5px; }
.mp-sample { color: var(--muted); font-size: 11.5px; margin-bottom: 8px; line-height: 1.9; }
.mp-chip { display: inline-block; margin: 0 4px; padding: 1px 8px; border: 1px solid var(--border);
  border-radius: 6px; }
.mp-chip i { color: var(--muted); font-style: normal; margin-left: 4px; font-size: 10.5px; }
.btn.small.danger { border-color: var(--bad); color: var(--bad); }
.btn.small.danger:hover:not(:disabled) { background: var(--bad); color: #fff; }
.off-bar { display: flex; gap: 10px; align-items: center; font-size: 12px; margin: 4px 0 6px; }
</style>
