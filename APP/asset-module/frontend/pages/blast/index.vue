...
async function loadSystemsList() {
  ...
  try {
    ...
    systemsList.value = []
    showToast(`業務系統清單載入失敗：${e?.data?.detail ?? e?.message ?? '未知錯誤'}`, 'error')
  } finally {
  ...
  <div class="topo">
    <div class="head">
      <div class="title">
        ...
        <h1>影響範圍查詢 <InfoNote>輸入 IP／主機名／資產序號／系統名稱，查<b>停機會波及誰</b>（事故／計畫性停機），或<b>陌生 IP 打進來是什麼</b>（研判）。查不到登記資料時不回「查無此物」，改給網段推導的線索。</InfoNote></h1>
      </div>
    </div>
    ...
    <template v-if="impactResult">
      ...
      <div v-if="mode === 'planned' && !viewingSnapshot" class="glass snap-form">
        <div class="psec">存證快照 <InfoNote>圖會變——三週後要拿得出「當初評估說不影響」是哪次算出來的，存下這次結果。</InfoNote></div>
        <div class="snap-row">
      ...
      <div class="section-divider">應通知 <InfoNote>依業務系統展開表格已經列出每台機器的負責人；要開一份可勾狀態、寫備註、全隊共用的派工清單，按右上角「📋 建立檢查清單」——會依嚴重度自動排序，缺聯絡方式的機器會標警示，不用自己在通訊錄裡找。</InfoNote></div>
...
.cand { font-family: inherit; font-size: 12.5px; padding: 7px 12px; border-radius: 9px;
  border: 1px solid rgba(0,145,66,.4); background: rgba(0,145,66,.08); color: var(--brand-dark); cursor: pointer; }
...
.snap-banner b { color: var(--warn-text); }
...
.psec { font-size: 11px; letter-spacing: 1px; text-transform: uppercase; color: var(--brand-dark); margin: 14px 0 6px; }
...
.covh { font-size: 12px; color: var(--warn-text); margin-bottom: 6px; }
...
.stackth .sortable:hover { color: var(--brand-dark); }
.stackth .sortable.on { color: var(--brand-dark); }
...
.bizx { background: none; border: none; padding: 0; cursor: pointer; font: inherit;
        color: var(--brand-dark); display: inline-flex; align-items: center; gap: 4px; }
...
.bao.none { color: var(--warn-text); }      /* 沒有負責人是事故當下必須立刻看到的事 */
...
.bapend { color: var(--muted); font-style: italic; }
...
.pill.open { background: rgba(255,184,103,.16); color: var(--warn-text); }
.pill.gone-pill { background: rgba(255,107,107,.16); color: var(--bad); }

.section-divider { font-size: 13px; font-weight: 700; color: var(--ink); margin: 20px 0 10px; padding-bottom: 6px;
  border-bottom: 1px solid rgba(15,23,42,.08); }
.section-divider.warn-title { color: var(--warn-text); }
</style>
