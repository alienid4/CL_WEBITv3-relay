...
async function clearDrill() {
  await router.push({ path: '/services', query: {} })
}
...
<template>
  <div>
    ...
    <p class="lead">主機自己回報的監聽埠與行程 <InfoNote>進到已納管主機問「你在跑什麼」，收監聽埠與行程。這是主機自己講的，跟掃描結果（從外面看）不同——只綁 127.0.0.1 的服務外面掃不到，這裡看得到。</InfoNote></p>

    <!-- 先列出「是哪幾台」（2026-09-16 使用者：「每個盤點都是這概念」）：
         彙總數字對得起來也沒用，人要做的事是「去那台上面看／改」。 -->
    <InventoryHosts kind="service" drill-to="/services" />
    ...
    <div v-if="summary" class="tiles">
      ...
      <div class="tile">
        ...
        <div class="t-lbl">有行程名佐證 <InfoNote>其餘只有埠號推測。若全部都只有埠號推測，是因為收集帳號 <code>webit3scan</code> 唯讀非 root、看不到行程名——這是刻意不擴權的結果，不是採集失敗；埠與曝露仍是機器實際回報的事實。</InfoNote></div>
      </div>
