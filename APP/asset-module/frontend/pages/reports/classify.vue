...
// 效能：一次渲染 2740 列會讓瀏覽器卡住幾秒（2026-09-12 使用者回報）。只渲染前 renderLimit 列，
// 其餘用搜尋縮小或用下方「從外部盤點表帶入分類」批次處理。⚠️ 全選/批次仍作用在**全部符合的**
// （sorted），不是只有畫出來的——所以批次分類 2740 台照樣做得到，只是不把它們全畫出來。
const RENDER_STEP = 300
const renderLimit = ref(RENDER_STEP)
const sortedCapped = computed(() => sorted.value.slice(0, renderLimit.value))
watch([filter, keyword], () => { renderLimit.value = RENDER_STEP })   // 換篩選/搜尋就重置
...
<template>
  <div class="page">
    <header class="hd">
      <div>
        <h1>主機分類作業 <InfoNote>報表頁B／頁C 的「核心交易／非核心／測試」三桶與分色，全部看這一欄。<b>這是報告專用的分類，跟 CIA 資訊資產清冊無關。</b></InfoNote></h1>
        <p class="sub">決定每一台主機在部門報告裡屬於哪一個分類</p>
      </div>
    ...
    <section v-if="summary" class="cards">
      ...
      <div class="card" :class="{ hot: summary.unclassified > 0 }">
        <div class="k">未分類 <InfoNote>這些台在報表上是灰的，沒有歸屬。</InfoNote></div>
        <div class="v">{{ summary.unclassified }}</div>
      </div>
      <div class="card">
        <div class="k">分類種類 <InfoNote>已經被用到的分類數 ／ 可用的分類總數。</InfoNote></div>
    ...
    <div class="tbl-wrap">
      <table class="tbl">
        <thead>
          <tr>
            ...
            <SortTh k="category" :active="sortKey" :dir="sortDir" @sort="toggle">分類 <InfoNote>標 ＊ ＝ 由 APID 對照表推得的預設值，尚未逐台確認；勾選後套用即可定案。</InfoNote></SortTh>
          </tr>
        </thead>
        <tbody>
      ...
      <div v-if="sorted.length > sortedCapped.length" class="capnote">
        為了不讓瀏覽器卡住，只畫出前 <b>{{ sortedCapped.length.toLocaleString() }}</b> 台／共
        <b>{{ sorted.length.toLocaleString() }}</b> 台。
        <button class="btn small" type="button" @click="renderLimit += RENDER_STEP">再顯示 {{ RENDER_STEP }} 台</button>
        <InfoNote>大量分類建議用下方「從外部盤點表帶入分類」一次帶入，不用逐台勾。<br>「全選」與批次套用仍作用在<b>全部符合的 {{ sorted.length.toLocaleString() }} 台</b>（不只畫出來的），所以批次分類照樣做得到。</InfoNote>
      </div>
    </div>

    <!-- 從外部盤點表帶入 -->
    <section id="seed" class="seed" :class="{ hilite: seedHilite }">
      <h2>從外部盤點表帶入分類 <InfoNote>用管理員維護的盤點表（例如《系統盤點(全環境)Data.xlsx》）一次帶入。只讀 <b>「主機名稱」</b>與<b>「分類」</b>兩欄當對照，來源表的環境別／機房／APID <b>一律忽略</b>——那些以《資訊資產清冊》為準。<br><br><b>預設只試算不寫入。</b>先按「試算」看對上幾筆、對不上哪些，確認過再按寫入。</InfoNote></h2>
      <div class="seedrow">
...
.capnote { margin: 10px 2px 0; font-size: 12px; color: var(--ink-soft);
