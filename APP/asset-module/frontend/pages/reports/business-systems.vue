...
<template>
  <div class="page">
    ...
    <template v-else-if="data">
      ...
      <h2>業務系統分類對照表 <InfoNote>下載範本填好分類欄再上傳（可用分類見範本第二分頁），主機系統總覽頁與這裡都會立刻反映。</InfoNote></h2>
      <div class="card catcard">
        <p class="tip">{{ data.systems_without_category }} / {{ data.all_systems.length }} 個系統尚未分類</p>
        <!-- 2026-08-26：使用者把這張表誤認成主機分類的匯入口，差別要寫在畫面上；
             依 2026-09-07 儀表板鐵律 critical 例外，留一行、細節進註。 -->
        <p class="tip warn">
          ⚠ 這張表是<b>按業務系統（APID）</b>分類的預設值，真正決定報表數字的是<b>逐台的分類</b>
          <InfoNote>一個 APID 一類，只當「那台主機還沒逐台分類時的預設值」。<b>實測 155 個 APID 裡有 88 個底下的機器橫跨多種分類</b>——同一個業務系統的機器不見得同一類。要用管理員那份盤點表一次帶入 2000 多台，請到「主機分類作業 → 從外部盤點表帶入分類」。</InfoNote>
        </p>
        <div class="catrow">
