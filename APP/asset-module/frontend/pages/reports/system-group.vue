...
interface Report {
  ...
  platform_lifecycle: {
    ...
    excluded_total: number; retired_excluded: number; duplicates_merged?: number
  }
...
// 表1上方的狀態摘要卡：把各平台列同一狀態欄加總，讓「多少台已EOS／一年內到期」
// 一眼看到，不用橫著自己加。點卡片 → 下鑽同一批（帶 registered_only，口徑對齊卡片數字）。
const pl = computed(() => report.value?.platform_lifecycle)
const statusTotals = computed(() => {
  const t: Record<string, number> = { 已EOS: 0, 一年內EOS: 0, 尚未公布: 0, 需確認: 0, 支援中: 0 }
  for (const r of platformRows.value) for (const s of STATUS_COLS) t[s] += (r as any)[s] || 0
  return t
})
// 已EOS + 一年內EOS = 該優先處理的台數（風險）
const riskTotal = computed(() => statusTotals.value['已EOS'] + statusTotals.value['一年內EOS'])
// 全機隊帳對帳：納入 + 不列入 + 已退役 = 總計，讓合計跟資產總數兜得回去。
const grandTotal = computed(() =>
  (pl.value?.total ?? 0) + (pl.value?.excluded_total ?? 0) + (pl.value?.retired_excluded ?? 0))
const SUMMARY_CARDS = [
  { s: '已EOS', label: '已 EOS', tone: 'bad' },
  { s: '一年內EOS', label: '一年內 EOS', tone: 'warn' },
  { s: '需確認', label: '需確認', tone: 'plain' },
  { s: '支援中', label: '支援中', tone: 'good' },
] as const
// 「不列入本報告」＋帳對帳預設收起——主表獨立一張、乾淨可複製，要看排除明細再點開
// （2026-09-13 使用者：不要同一時間三張表並排）。
...
// ESXi 攤成一張交叉矩陣：叢集分類（列）× 機房（欄），格子＝ESXi 台數。
// 2026-09-13 使用者：原本一個機房一個小卡＝四格，要的是像「環境×機房」那種「一整格」。
const esxiLocs = computed(() => (report.value?.cluster_summary.locations ?? []).map(l => l.location))
const esxiServices = computed(() => {
  const set = new Set<string>()
  for (const l of report.value?.cluster_summary.locations ?? [])
    for (const r of l.rows) set.add(r.service)
  return [...set]
})
const esxiCellMap = computed(() => {
  const m: Record<string, number> = {}
  for (const l of report.value?.cluster_summary.locations ?? [])
    for (const r of l.rows) m[`${l.location}||${r.service}`] = r.esxi_count
  return m
})
function esxiCount(loc: string, svc: string) { return esxiCellMap.value[`${loc}||${svc}`] ?? 0 }
function esxiRowTotal(svc: string) { return esxiLocs.value.reduce((s, loc) => s + esxiCount(loc, svc), 0) }
const esxiColTotalMap = computed(() => {
  const m: Record<string, number> = {}
  for (const l of report.value?.cluster_summary.locations ?? []) m[l.location] = l.total
  return m
})
function esxiColTotal(loc: string) { return esxiColTotalMap.value[loc] ?? 0 }
const esxiGrand = computed(() => esxiLocs.value.reduce((s, loc) => s + esxiColTotal(loc), 0))
// 排序：點欄＝依那一欄台數排列各分類（天條）；__svc 依名稱、__total 依整列合計。
const esxiSortedServices = computed(() => {
  const k = loc2Sort.value.key, dir = loc2Sort.value.dir
  if (!k) return esxiServices.value
  const val = (svc: string) => k === '__svc' ? svc : k === '__total' ? esxiRowTotal(svc) : esxiCount(k, svc)
  const mul = dir === 'asc' ? 1 : -1
  return [...esxiServices.value].sort((a, b) => {
...
// 2026-09-13 使用者：系統組報告拆成多頁籤（一頁一塊，照分頁風格）。
const tab = ref<'plat' | 'os' | 'esxi' | 'ibm' | 'phys'>('plat')
const REPORT_TABS = [
  { k: 'plat', label: '平台數量與生命週期' },
  { k: 'os', label: '作業系統明細' },
  { k: 'esxi', label: 'ESXi 數量' },
  { k: 'ibm', label: 'IBM／虛擬化環境' },
  { k: 'phys', label: '實體主機' },
] as const
...
<template>
  <div class="page">
    ...
    <template v-else-if="report">
      ...
      <div class="rtabs">
        <button v-for="t in REPORT_TABS" :key="t.k" class="rtab" :class="{ on: tab === t.k }"
                type="button" @click="tab = t.k">{{ t.label }}</button>
      </div>
      ...
      <h2>平台數量與生命週期
        <InfoNote>
          <b>已 EOS</b>：官方支援已結束。<b>一年內 EOS</b>：一年內到期。
          <b>尚未公布</b>：查到此產品、但官方還沒公布 EOS 日期。
          <b>需確認</b>：EOS 對照表查無此產品，要人工確認。<b>支援中</b>：仍在支援期內。<br>
          數字都可點——點下去列出是哪幾台、每台為什麼被分到這一類。
        </InfoNote>
      </h2>

      <!-- 狀態摘要卡：一眼看到「多少台要優先處理」，不用橫著自己加 -->
      <div class="cards">
        <button v-for="c in SUMMARY_CARDS" :key="c.s" class="scard" :class="c.tone" type="button"
                @click="drill('platform', `全平台 ／ ${c.label}`, { status: c.s, registered_only: 1 })">
          <span class="sv num">{{ statusTotals[c.s].toLocaleString() }}</span>
          <span class="sl">{{ c.label }}</span>
        </button>
        <div class="scard risk">
          <span class="sv num">{{ riskTotal.toLocaleString() }}</span>
          <span class="sl">應優先處理（已+一年內）</span>
        </div>
      </div>

      <table class="dtable even">
        <thead><tr>
        ...
        <tbody>
          <tr v-for="r in platformSorted" :key="r.platform">
            ...
            <td class="txt">
              ...
              <span v-else class="note" :class="{ empty: !report.notes[`platform:${r.platform}`] }"
                    title="加備註" @click="startEdit(`platform:${r.platform}`)">
                {{ report.notes[`platform:${r.platform}`] || '＋' }}
              </span>
            </td>
          </tr>
          <tr class="sum">
            <td class="rowh">合計</td>
            <td class="num tot">{{ report.platform_lifecycle.total }}</td>
            <td class="txt" :colspan="STATUS_COLS.length + 1"></td>
          </tr>
        </tbody>
      </table>

      <!-- 主表底下一行小結；排除明細＋帳對帳收在後面，點開才出現（不跟主表並排） -->
      <p class="excsum">
        不列入本報告 <b>{{ pl?.excluded_total.toLocaleString() }}</b> 台
        · 已退役 <b>{{ pl?.retired_excluded.toLocaleString() }}</b> 台
        · 總計 <b>{{ grandTotal.toLocaleString() }}</b> 台
        <!-- 照主機算（2026-09-17）：使用者拿這個月 6084 跟上個月 3654 對照，問是不是算錯。
             查證後是同一台被 CIA／dynassets／RVTools 各登記一筆、以前一筆算一台。
             現在收成一台，但**要講出收了幾筆**——數字變動沒有說明，人只會更不信。 -->
        <template v-if="pl?.duplicates_merged">
          · 已收起 <b>{{ pl.duplicates_merged.toLocaleString() }}</b> 筆重複登記
          <InfoNote>同一台機器被 CIA／dynassets／RVTools 各登記一筆時，這裡<b>照主機算</b>（主機名＋IP 都相同才算同一台），跟納管漏斗同一個口徑。<br><br><b>資料沒有被合併也沒有刪除</b>，只是統計上收成一台；要決定留哪一筆到「資產查詢」的重複清單處理。<br><br>⚠️ 這是 2026-09-17 才修的：在那之前這張表照<b>筆數</b>算，所以跟更早的報告比會偏高。</InfoNote>
        </template>
        <a class="n exclink" @click="showExcluded = !showExcluded">{{ showExcluded ? '收合' : '看明細' }}</a>
      </p>

      <div v-if="showExcluded" class="excdetail">
        <table class="rt small">
          ...
          <tbody>
            ...
            <tr class="sum">
              <td class="rh">小計</td>
              <td class="num">{{ pl?.excluded_total.toLocaleString() }}</td>
            </tr>
            <tr>
              <td class="rh">已退役</td>
              <td class="num">
                <a class="n" @click="drill('platform', '已退役（不列入）', { retired: true })">{{ pl?.retired_excluded.toLocaleString() }}</a>
              </td>
            </tr>
            <tr class="sum">
              <td class="rh">總計</td>
      ...
      <table class="dtable">
        <thead><tr>
          <SortTh k="__svc" :active="loc2Sort.key" :dir="loc2Sort.dir" @sort="loc2Toggle">叢集分類＼機房</SortTh>
          <SortTh v-for="loc in esxiLocs" :key="loc" :k="loc" :active="loc2Sort.key" :dir="loc2Sort.dir"
                  @sort="loc2Toggle" class="num">{{ loc }}</SortTh>
          <SortTh k="__total" :active="loc2Sort.key" :dir="loc2Sort.dir" @sort="loc2Toggle" class="num tot">合計</SortTh>
        </tr></thead>
        <tbody>
          <tr v-for="svc in esxiSortedServices" :key="svc">
            <td class="rowh">{{ svc }}</td>
            <td v-for="loc in esxiLocs" :key="loc" class="num">
              <a v-if="esxiCount(loc, svc)" class="n"
                 @click="drill('cluster', `${loc} ／ ${svc}`, { location: loc, service: svc })">{{ esxiCount(loc, svc) }}</a>
              <span v-else class="empty">—</span>
            </td>
            <td class="num tot">{{ esxiRowTotal(svc) }}</td>
          </tr>
          <tr class="sum">
            <td class="rowh">合計</td>
            <td v-for="loc in esxiLocs" :key="loc" class="num">{{ esxiColTotal(loc) }}</td>
            <td class="num tot">{{ esxiGrand }}</td>
          </tr>
        </tbody>
      </table>
      ...
      <div v-show="tab === 'ibm'">
      <!-- ===== 表3：虛擬化與 AIX、IBM i 環境資訊 ===== -->
      <h2>虛擬化與 AIX、IBM i 環境資訊</h2>
      <table class="rt widemain">
        <thead><tr>
...
.rtabs { display: flex; gap: 8px; flex-wrap: wrap; margin: 14px 0 6px; }
.rtab { background: transparent; border: 1px solid var(--border-strong); color: var(--ink-aux);
  border-radius: 999px; padding: 6px 14px; font-size: 13px; cursor: pointer; }
.rtab:hover { border-color: var(--brand); }
.rtab.on { border-color: var(--brand); background: var(--mint); color: var(--brand-dark); font-weight: 600; }
...
.rt .wide { min-width: 140px; }
.rt.small { font-size: 11.5px; }
.rt .sum td { background: rgba(15,23,42,.03); font-weight: 600; }
.mono { font-family: ui-monospace, monospace; }
/* 0 台用淡灰印出來（不是留空）——空白會讓人分不出「0 台」還是「沒查到」 */
.z { color: var(--muted); }
.dim { color: var(--ink-soft); }

/* 狀態摘要卡：一眼看到多少台要優先處理 */
.cards { display: flex; flex-wrap: wrap; gap: 10px; margin: 4px 0 14px; }
.scard { display: flex; flex-direction: column; gap: 2px; align-items: flex-start;
  min-width: 116px; padding: 10px 14px; border-radius: var(--radius);
  border: 1px solid var(--border); background: var(--card); cursor: pointer;
  text-align: left; transition: border-color .15s, box-shadow .15s; }
button.scard:hover { border-color: var(--brand); box-shadow: var(--shadow); }
.scard .sv { font-size: 22px; font-weight: 700; line-height: 1.1; color: var(--ink); }
.scard .sl { font-size: 11.5px; color: var(--muted); }
.scard.bad .sv { color: var(--bad); }
.scard.warn .sv { color: var(--warn-text); }
.scard.good .sv { color: var(--brand-dark); }
.scard.risk { background: var(--sub); cursor: default; border-style: dashed; }
.scard.risk .sv { color: var(--bad); }

/* 主表底下一行小結：排除數＋總計講出來（不能靜默消失），但只有一行，
   明細收在「看明細」後面點開才出現——不跟主表同時擺三張表。 */
/* 滿版主表（2026-09-13，本機靜態預覽逐版對照後定案）：
   - 欄位「少」的表（平台8欄、作業系統3欄）→ 加 .even＝table-layout:fixed，
     每欄等寬鋪滿；不加的話 width:100% 會把多出的寬度全倒進某一欄（平台類別），
     變成一欄超寬、其餘擠著，看起來沒滿。
   - 欄位「多」的表（IBM叢集7欄、實體主機9欄，且含長字串如 vCenter IP／主機名）
     → 只用 .widemain（width:100%，欄寬看內容），不套 fixed，避免長字被 fixed 切掉
     （切字＝藏資料，違反「查得到的要看得到」）。 */
.widemain { width: 100%; }
.widemain.even { table-layout: fixed; }
.widemain.even th, .widemain.even td { overflow: hidden; text-overflow: ellipsis; }

/* ESXi 機房小卡：平均長滿整列（原本靠內容寬、四張擠左邊留一大塊空白） */
.locbox { flex: 1 1 240px; min-width: 0; }
.locbox .rt.small { width: 100%; }

.excsum { margin: 12px 0 0; font-size: 12.5px; color: var(--ink-soft); }
.excsum b { color: var(--ink); font-family: var(--disp); font-variant-numeric: tabular-nums; }
.exclink { margin-left: 10px; }
.excdetail { margin-top: 8px; }
...
/* 空備註只留一個淡淡的＋，滑到那一列才變明顯——不要整排「＋加備註」佔一欄 */
.note.empty { color: var(--border-strong); font-size: 14px; }
tr:hover .note.empty { color: var(--brand-dark); }
