...
const _TABS = ['connections', 'diagram', 'modules', 'schedule',
               'backup', 'credentials', 'autoonboard'] as const
...
<template>
  <div>
    ...
    <template v-else-if="activeTab === 'backup'">
      ...
      <template v-if="backupHealth">
        ...
        <div class="bk-card" style="margin-bottom:16px">
          ...
          <div v-if="keyStatus" class="actions" style="gap:10px;align-items:center;flex-wrap:wrap">
          ...
          <div v-if="keyStatus" class="actions" style="gap:10px;align-items:center;margin-top:10px;flex-wrap:wrap">
        ...
        <div class="bk-card" style="margin-bottom:16px">
          <div class="bk-card-title">匯出全部資料（下載到你的電腦） <InfoNote>跟上面的備份不同——這是直接下載一份完整資料的複本到你的電腦，用於搬機／備份轉移。<br><br>存在伺服器上的備份<b>不加密</b>（自己救得回來）；這份會離開機器，<b>一律加密</b>。<br><br>加密順便壓縮：實測 45MB 的資料庫壓完 4.1MB，多數情況一封信寄得完、不用切片。</InfoNote></div>
    ...
    <template v-else-if="activeTab === 'schedule'">
      ...
      <!-- 各網段自訂時間（2026-09-12）：讓公司多個測試網段錯開時間，不擠在同一時刻 -->
      <div class="sched-card" style="margin-top:16px; max-width:680px">
        <div class="sched-title">各網段自訂時間（選填）
          <InfoNote>公司有多個測試網段時，可以讓它們<b>錯開時間</b>、不要全部擠在上面那個全域時間一起掃。<b>設了時間</b>的網段會在自己的時間單獨掃；<b>沒設</b>的跟全域排程一起掃。留空按「存」＝清除、回到全域。</InfoNote>
        </div>
        <table class="segtime">
          <thead><tr><th>網段／來源</th><th>目標</th><th>自己的掃描時間</th><th></th></tr></thead>
          <tbody>
            <tr v-for="c in segConns" :key="c.id">
              <td>{{ c.name }}</td>
              <td class="mono">{{ c.target }}</td>
              <td><input v-model="c.scan_time" type="time" class="cell-input" /></td>
              <td>
                <button class="btn small" @click="saveScanTime(c)">存</button>
                <button v-if="c.scan_time" class="btn small ghost" @click="c.scan_time = ''; saveScanTime(c)">清除</button>
                <span v-else class="dim sm">跟全域一起</span>
              </td>
            </tr>
            <tr v-if="!segConns.length"><td colspan="4" class="dim">還沒有可掃的網段來源——到「連線設定」新增。</td></tr>
          </tbody>
        </table>
      </div>
    </template>
