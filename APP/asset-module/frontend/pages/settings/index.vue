    ...
    <template v-else-if="activeTab === 'backup'">
      ...
      <template v-if="backupHealth">
        ...
        <div v-if="isEnabled('restore')" class="bk-card" style="margin-bottom:16px;border-color:rgba(224,108,108,.45)">
          <div class="bk-card-title" style="color:#e06c6c">⚠ 資料庫還原（整庫覆蓋）</div>
          <p class="credhint" style="margin:6px 0 12px">
            上傳一份先前用「匯出全部資料」下載的<b>二進位 .db 檔</b>，會<b>整個覆蓋掉現在的正式資料庫</b>。
            這個動作不可逆，覆蓋前系統會自動先把現有資料存證一份到伺服器本機備份目錄，但那是最後防線，
            不是「反悔按鈕」。確定要做才繼續。<br>
            <b style="color:#e0b060">還原成功後你會被自動登出</b>——帳號與登入階段也存在同一個資料庫裡，
...
.tab-lamp.green { background: #009142; }
...
.bk-lamp.green { background: #009142; }
...
.bk-line .ok { color: #009142; font-weight: 700; }
...
.segsum .warn { color: #b45309; }
.segtbl { max-height: 320px; overflow-y: auto; }
.segtbl code { font-size: 12px; }
/* 同一網段跨多個機房：不是錯誤，但值得看一眼，所以標示而不擋 */
.mixed { margin-left: 6px; font-size: 11px; color: #b45309; opacity: .9; cursor: help; }
.segdone { margin: 12px 0 0; font-size: 13px; opacity: .7; }
.segok { margin: 12px 0 0; font-size: 13px; color: var(--brand, #009142); }
.segwarn { margin: 10px 0 0; font-size: 12px; color: #b45309; opacity: .85; line-height: 1.6; }
.notice-banner {
...
.ao-switch.on .ao-switch-track { background: #009142; }
...
.vc-cmd code {
  ...
  color: var(--brand, #009142); white-space: nowrap;
}
