...
<template>
  <div>
    ...
    <div v-if="collectCfg" class="collector-ip">
      ...
      <p class="pv-note">
        建帳號時寫進目標主機 <code>/etc/passwd</code> 的備註欄，別人在那台
        <code>getent passwd</code> 看到 <code>{{ collectCfg.account }}</code> 時，
        靠這一欄知道<b>是誰佈的、為什麼</b>——稽核與交接都會問這件事。
        建議填「員工編號-姓名_部門_系統」。
        <br>
        <b>只接受中英數與 <code>- _ . @ ( ) ／</code> 與空白</b>（最多 120 字）：
        這個值會被放進 <code>useradd -c</code>，引號、<code>$</code>、換行會改變整行指令的語意。
        <br>
        這個值<b>存在資料庫、不進程式碼</b>——裡面有員工編號與姓名，而納管引擎的
        原始碼會被打包送出去。
      </p>
      <div class="ci-edit">
        ...
        <button class="link-btn" type="button" :disabled="savingComment"
                @click="saveAccountComment">
          {{ savingComment ? '儲存中…' : '儲存' }}
        </button>
        <span class="pv-note">留空儲存＝清掉，退回預設值</span>
      </div>
      <p class="pv-note">
        ⚠️ 改了只影響<b>之後</b>新納管的機器。已經納管的要改備註，
        得在該機執行 <code>usermod -c "新備註" {{ collectCfg.account }}</code>。
      </p>
    </div>
