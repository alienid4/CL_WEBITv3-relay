...
interface CollectConfig {
  ...
  // 收集帳號在 /etc/passwd 的備註欄（GECOS）。在主機上 `getent passwd` 看到這個
  // 帳號的人，靠這一欄知道是誰佈的、為什麼。實際值存 DB **不進版控**——
  // 裡面有員工編號與姓名，而納管引擎的原始碼會進公開 relay。
...
async function saveAccountComment() {
  if (savingComment.value || !collectCfg.value) return
  savingComment.value = true
  try {
    const r = await apiFetch<{ account_comment: string }>('/api/accounts/collect-config', {
      method: 'PUT',
      body: { account: collectCfg.value.account, account_comment: commentEdit.value },
    })
    await loadCollectCfg()
    commentEdit.value = ''
    showToast(`帳號備註已設為「${r.account_comment}」`, 'success')
  } catch (e: any) {
    // 後端拒絕不合規字元時會回具體理由（引號、$、換行會改變 useradd 的指令語意）
...
async function copyProbe() {
  try {
    const ok = await copy(JSON.stringify(probeResult.value, null, 2))
  if (!ok) showToast('複製失敗，請直接選取內容複製', 'warn', 8000)
  ...
  <div>
    ...
    <div v-if="collectCfg" class="collector-ip">
      <div class="ci-head">
        ...
        <InfoNote>
          系統自動偵測，一般不用改；多網卡／NAT／對外用 DNS 名才需指定。<br>
          納管時會把它寫進目標主機 <code>authorized_keys</code> 的 <code>from=</code> 來源限制。
          <b>填錯不會有錯誤訊息</b>——金鑰會被目標主機拒絕，但納管仍顯示成功、之後永遠
          收不到。目前偵測到 <code>{{ collectCfg.collector_ip_detected }}</code>。<br>
          留空儲存＝清掉指定，退回自動偵測。
        </InfoNote>
      </div>
      <div class="ci-edit">
        <span class="edit-label">✎ 改成：</span>
        <input v-model="ipEdit" class="ci-in" :placeholder="collectCfg.collector_ip"
               :disabled="savingIp" @keyup.enter="saveCollectorIp" />
        <button class="btn primary small" type="button" :disabled="savingIp" @click="saveCollectorIp">
          {{ savingIp ? '儲存中…' : '儲存' }}
        </button>
      </div>
    </div>

    <!-- 帳號備註：主機上看到這個帳號的人，靠這一欄知道是誰佈的 -->
    <div v-if="collectCfg" class="collector-ip">
      <div class="ci-head">
        <b>帳號備註</b>
        <code class="ci-val">{{ collectCfg.account_comment }}</code>
        <InfoNote>
          建議填「員工編號-姓名_部門_系統」。建帳號時寫進目標主機
          <code>/etc/passwd</code> 的備註欄，別人在那台 <code>getent passwd</code> 看到
          <code>{{ collectCfg.account }}</code> 時靠它辨識——稽核與交接都會問。<br>
          <b>只接受中英數與 <code>- _ . @ ( ) ／</code> 與空白</b>（最多 120 字）：這個值會被
          放進 <code>useradd -c</code>，引號、<code>$</code>、換行會改變整行指令的語意。<br>
          存在資料庫、<b>不進程式碼</b>——裡面有員工編號與姓名，而納管引擎原始碼會被打包送出去。<br>
          留空儲存＝清掉退回預設值。改了只影響<b>之後</b>新納管的機器；已經納管的要一起改，
          用「整批佈署」的 Ansible playbook 一次改（比一台台 <code>usermod -c</code> 快）。
        </InfoNote>
      </div>
      <div class="ci-edit">
        <span class="edit-label">✎ 改成：</span>
        <input v-model="commentEdit" class="ci-in" :placeholder="collectCfg.account_comment"
               :disabled="savingComment" @keyup.enter="saveAccountComment" />
        <button class="btn primary small" type="button" :disabled="savingComment"
                @click="saveAccountComment">
          {{ savingComment ? '儲存中…' : '儲存' }}
        </button>
      </div>
    </div>
    ...
    <div v-if="(summary?.hosts_needing_root || 0) > 0" class="gap">
      ...
      <p v-if="showSudo" class="sudo-note">
        <b>不要手動貼這幾行</b>——重跑一次「納管腳本」就會佈好：它會先把現況備份到
        <code>/var/backups/webit3/&lt;時間&gt;/</code> 並產生 <code>restore.sh</code>（執行就還原成納管前），
        再放一支<b>內容固定、只有 root 能改、不接受任何參數</b>的唯讀輔助程式，
        sudo 只准「不帶參數」執行它。<br>
        為什麼不直接開 <code>chage -l *</code>／<code>cat /home/*/.ssh/authorized_keys</code>：
        sudoers 的 <code>*</code> 會吃空白，那種規則能被拿去順便讀 <code>/etc/shadow</code>。
        輔助程式只給密碼狀態碼、效期日期、sudoers 內容、金鑰<b>數量</b>，不吐密碼雜湊也不吐金鑰內容。
...
.ci-val { font-family: ui-monospace, Consolas, monospace; color: var(--brand-dark); font-weight: 700; }
.ci-src { font-size: 11px; color: var(--muted); }
.ci-edit { display: flex; align-items: center; gap: 8px; margin-top: 10px; flex-wrap: wrap;
  padding: 8px 10px; background: var(--sub, rgba(0,0,0,.03)); border-radius: 10px; }
.edit-label { font-size: 12px; font-weight: 600; color: var(--brand-dark); white-space: nowrap; }
.ci-in { font-family: ui-monospace, Consolas, monospace; font-size: 12.5px; padding: 8px 11px;
  border: 1.5px solid var(--brand); border-radius: 8px; background: var(--card-solid, #fff);
  color: var(--ink); min-width: 240px; flex: 1; }
.ci-in:focus { outline: 2px solid var(--brand); outline-offset: 1px; }
.ci-in::placeholder { color: var(--muted); }
