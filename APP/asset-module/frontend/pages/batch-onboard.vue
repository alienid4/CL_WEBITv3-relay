...
interface Status {
  ...
  remaining?: { ip: string; environment: string | null }[]
}
...
async function start(overrideIps?: string[]) {
  const ips = overrideIps ?? parseIps()
  ...
  try {
    await apiFetch('/api/onboard/batch-auto', {
      ...
      body: {
        ...
        // 繼續剩下的：目標已明確，直接當「勾選納入」送，不再經測試/未知分桶
        include_ips: overrideIps ? overrideIps : [...includeUnknown.value],
      },
    })
    // 密碼用完即清
...
// 繼續剩下的：上一批中斷後，只跑還沒跑到的那幾台（密碼要在上面重新輸入，系統從不留存）。
function resume() {
  const rem = (status.value?.remaining ?? []).map((t) => t.ip)
  if (!rem.length) { showToast('沒有待繼續的目標', 'warn'); return }
  start(rem)
}
  ...
  <div class="page">
    ...
    <section v-if="status && status.run" class="card">
      ...
      <p class="dim sm">
        ...
        <span v-else-if="status.interrupted">・<b style="color:var(--warn-text)">已停下</b>（跑到 {{ status.done }} / {{ status.total }}，還剩 {{ status.remaining?.length ?? 0 }} 台沒跑）</span>
      ...
      <!-- 中斷後：明講已停下、不是還在跑；要繼續得重新輸入密碼再按（不自動接，避免像暴力破解） -->
      <div v-if="status.interrupted && (status.remaining?.length)" class="resume">
        <span>上一批中途停了，還有 <b>{{ status.remaining!.length }}</b> 台沒跑。要繼續請在上方重新輸入密碼，再按這裡：</span>
        <button class="btn small primary" :disabled="starting" @click="resume">▶ 繼續剩下的 {{ status.remaining!.length }} 台</button>
        <InfoNote>不會自動接著跑——連續拿錯帳密打整批，看起來就像暴力破解，會鎖帳號、觸發資安告警。所以一定要人重新輸入密碼、手動按。</InfoNote>
      </div>
...
.resume { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; margin: 10px 0 14px;
  padding: 10px 12px; border-radius: 10px; background: var(--warn-soft); border: 1px solid var(--warn);
