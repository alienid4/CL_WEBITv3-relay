...
// 從主機清單點進來（?ip=）直接開那一台（2026-09-16 使用者：
// 「先列出哪幾台，我再進去點後才是那一台的全部資訊」「每個盤點都是這概念」）
const swRoute = useRoute()
watch(() => swRoute.query.ip, (ip) => {
  if (typeof ip === 'string' && ip) openHost({ ip, hostname: null })
}, { immediate: true })
...
<template>
  <div>
    ...
    <InventoryHosts kind="software" drill-to="/software" />
