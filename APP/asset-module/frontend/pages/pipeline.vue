...
interface Row {
  ...
  environment: string | null; os: string | null; os_type: string
...
const filtered = computed<Row[]>(() => {
  ...
  if (osFilter.value) rows = rows.filter((r) => r.os_type === osFilter.value)
...
function clearFilters() {
  stageFilter.value = ''; envFilter.value = ''; osFilter.value = ''
  keyword.value = ''; onlyTodo.value = true
}

// 匯出當前篩選結果——所見即所得，不是匯出全部
function exportCsv() {
  const head = ['關卡', 'IP', '主機名稱', '資產編號', '環境別', 'OS類型', '作業系統',
                '上次試連', '下一步', '錯誤']
...
// 2026-09-16 使用者：漏斗這一頁也要能「偵測存活／下線／非納管」，不然看到失聯 8062 台
// 只能一台一台跳去資產頁處理。三顆都走跟資產查詢頁同一組元件與端點。
const offlineTarget = ref<{ serials: string[]; who: string; reason: string } | null>(null)
const exemptTarget = ref<{ serials: string[]; who: string } | null>(null)
function who(r: Row) { return `${r.hostname || r.asset_serial || r.ip}（${r.ip}）` }
function askOffline(r: Row, reason = '') {
  if (!r.asset_serial) { showToast('這台還沒登記成資產，請先去「納入管理」建檔', 'warn'); return }
  offlineTarget.value = { serials: [r.asset_serial], who: who(r), reason }
}
function askExempt(r: Row) {
  if (!r.asset_serial) { showToast('這台還沒登記成資產，請先去「納入管理」建檔', 'warn'); return }
  exemptTarget.value = { serials: [r.asset_serial], who: who(r) }
}
...
<template>
  <div>
    ...
    <p class="lead">每台機器現在走到哪一關、下一步要做什麼
      ...
      <template v-if="data?.scan_time"> · 最近掃描 {{ data.scan_time }}</template>
    </p>
      ...
      <div class="filters">
        ...
        <label class="f">OS 類型
          <select v-model="osFilter">
            <option value="">全部</option>
            <option value="Linux">Linux（{{ data.os_counts?.Linux || 0 }}）</option>
            <option value="Windows">Windows（{{ data.os_counts?.Windows || 0 }}）</option>
            <option value="AIX">AIX（{{ data.os_counts?.AIX || 0 }}）</option>
            <option value="未知">未知（{{ data.os_counts?.['未知'] || 0 }}）</option>
          </select>
        </label>
        ...
        <table>
          ...
          <tbody>
            <tr v-for="r in sorted" :key="(r.asset_serial || '') + r.ip">
              ...
              <td>{{ r.environment || '—' }}</td>
              <td><span class="ostag" :class="`os-${r.os_type}`">{{ r.os_type }}</span></td>
              ...
              <td class="acts">
                <!-- 偵測存活：失聯那一批最需要——先確認到底還在不在，再決定下線或繼續查 -->
                <AliveCheck :ip="r.ip" :can-offline="!!r.asset_serial" @offline="(x) => askOffline(r, x)" />
                ...
                <button type="button" class="btn small ghost" :disabled="!r.asset_serial"
                        title="標記成停用／報廢／閒置（原因必填，會記進 CIA 待異動）"
                        @click="askOffline(r)">下線</button>
                <button type="button" class="btn small ghost" :disabled="!r.asset_serial"
                        title="不能納管、也不是下線（客製化系統／Oracle／廠商維護）"
                        @click="askExempt(r)">非納管</button>
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <ExemptModal v-if="exemptTarget" :serials="exemptTarget.serials" :who="exemptTarget.who"
                 @close="exemptTarget = null" @done="exemptTarget = null; load()" />
