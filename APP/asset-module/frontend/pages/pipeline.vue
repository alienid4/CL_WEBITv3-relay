...
interface Row {
  ...
  dup_count?: number; dup_serials?: string[]; dup_reason?: string
        ...
        <table>
          ...
          <tbody>
            <tr v-for="r in shown" :key="(r.asset_serial || '') + r.ip">
              ...
              <!-- 主機名會被截斷，但「同台 N 筆」這顆不可以跟著被截掉（2026-09-20 使用者
                   截到的畫面只剩「重…」，而且看不出為什麼重複）。名字自己截，標籤放旁邊不截。 -->
              <td class="hostcell">
                <span class="ell" :title="r.hostname || ''">
                  {{ r.hostname || (r.asset_serial ? '—' : '(未登記)') }}
                </span>
                <NuxtLink v-if="(r.dup_count ?? 1) > 1" class="dupchip"
                          :to="{ path: '/assets', query: { q: r.ip } }"
                          :title="`同一台在資產庫有 ${r.dup_count} 筆登記（${(r.dup_serials || []).join('、')}）`
                                  + (r.dup_reason ? `：${r.dup_reason}` : '')
                                  + '。資料沒有被合併——點進去看逐筆差在哪、要不要清。'">
                  同台 {{ r.dup_count }} 筆<i v-if="r.dup_reason" class="why">・為什麼</i>
                </NuxtLink>
              </td>
...
.dupchip .why { font-style: normal; opacity: .75; }
.hostcell { display: flex; align-items: center; gap: 2px; max-width: 260px; }
.hostcell .ell { max-width: 150px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
