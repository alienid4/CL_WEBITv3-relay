...
// 合規表關鍵字搜尋（2026-09-12 使用者：「要有搜尋」）：主機名／IP／帳號／序號
const mkw = ref('')
const mRows = computed(() => {
  const kw = mkw.value.trim().toLowerCase()
  if (!kw) return matrixFiltered.value
  return matrixFiltered.value.filter((r) =>
    [r.hostname, r.ip, r.username, r.asset_serial].some((v) => (v ?? '').toString().toLowerCase().includes(kw)))
})
  ...
  <div>
    ...
    <template v-else-if="tab === 'matrix'">
      ...
      <div v-if="accounts.length" class="bar">
        <input v-model="mkw" type="search" class="sel mkw" placeholder="搜尋 主機／IP／帳號／序號" />
