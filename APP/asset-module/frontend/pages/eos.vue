...
interface Summary { host_os: EosGroup; firmware: EosGroup; software: EosGroup; insufficient: EosGroup;
  other: EosGroup; hardware: EosGroup
  os_order?: string[]        // 第二層分類順序（後端給，＝pipeline.OS_ORDER）
}
...
// [2026-09-20] 分頁數字要跟著上面的狀態快篩走：以前篩了「一年內到期」，分頁還是顯示
// 全部的種類數（78／76／103…），人看不出「一年內到期裡作業系統佔幾種」。
function byStatus<T extends { status: string }>(list: T[]): T[] {
  return statusFilter.value ? list.filter((x) => x.status === statusFilter.value) : list
}
const tabItemCount = computed(() => ({
  host_os: byStatus(summary.value?.host_os.items ?? []).length,
  network: byStatus(networkItems.value).length,
  hardware: byStatus(hardwareOnlyItems.value).length,
  vm: byStatus(vmItems.value).length,
  software: byStatus(summary.value?.software.items ?? []).length,
  insufficient: byStatus(summary.value?.insufficient.items ?? []).length,
  other: byStatus(summary.value?.other.items ?? []).length,
...
// [2026-09-20 使用者拍板：「類別要一樣，改第二」] 作業系統分頁的第二層改用全站正典的
// OS 類型（後端 os_order＝pipeline.OS_ORDER：AIX 與 IBM i 分開、含 OpenShift 節點等）。
// 第一層分頁（作業系統／網路設備／硬體型號／VM／軟體／資訊不足／未分類）維持不動。
const GROUP_FAMILY_ORDER = computed<Partial<Record<UiTabKey, string[]>>>(() => ({
  host_os: summary.value?.os_order ?? ['Linux', 'Windows', 'AIX', 'IBM i', 'VMware', '其他'],
...
// 狀態快篩（點上方磚塊）：EOS 頁最常問「哪些一年內到期」，加一個跨分頁通用的狀態過濾。
const statusFilter = ref<string>('')
function togStatus(s: string) { statusFilter.value = statusFilter.value === s ? '' : s }
watch(activeTab, () => { familyFilter.value = null; subFilter.value = null })

const familyChips = computed(() => {
  const items = byStatus(activeItemsRaw.value)   // 第二層的數字同樣跟著狀態快篩
  ...
  const order = GROUP_FAMILY_ORDER.value[activeTab.value]
  if (!order) return []
  const counts = new Map<string, number>()
  for (const it of items) {
...
const activeItems = computed(() => {
  ...
  if (isGlobalSearch.value) {
    let g = globalSearchResults.value.filter((it) =>
      it.name.toLowerCase().includes(q)
      || (it.device_models ?? []).some((m) => m.toLowerCase().includes(q))
      || (it.vendor ?? '').toLowerCase().includes(q))
    if (statusFilter.value) g = g.filter((it) => it.status === statusFilter.value)
    return g
  }
  ...
  if (activeTab.value === 'network') {
    ...
    const sub = SUB_GROUP[activeTab.value]
    if (GROUP_FAMILY_ORDER.value[activeTab.value]) {
...
// EOS 到期提醒匯出成 ICS 行事曆（2026-09-19）：把「已過／一年內到期」且有官方日期的種類
// 變成全天日曆事件＋到期前 30 天提醒，匯進 Outlook/Google 行事曆。只用查得到日期的——
// 沒有官方日期的不編進去（不猜日期，跟這頁一貫立場一致）。
function icsDate(d: string): string | null {
...
function exportIcs() {
  if (!summary.value) return
  const buckets: (keyof Summary)[] = ['host_os', 'firmware', 'software', 'insufficient', 'other', 'hardware']
  // 跨全部桶收「已過／一年內到期」且有可解析日期的種類，同名同日只留一筆
  const seen = new Set<string>()
  const rows: { name: string; date: string; count: number; status: string }[] = []
  for (const b of buckets) {
    for (const it of summary.value[b].items) {
      if (it.status !== 'expired' && it.status !== 'upcoming') continue
      const d = it.eos_date ? icsDate(it.eos_date) : null
      if (!d) continue
      const k = it.name + '|' + d
      if (seen.has(k)) continue
      seen.add(k)
      rows.push({ name: it.name, date: d, count: it.count, status: it.status })
    }
  }
  if (!rows.length) { showToast('沒有可匯出的到期項目（要有官方日期的已過／一年內到期種類）', 'warn'); return }
  const stamp = new Date().toISOString().replace(/[-:]/g, '').replace(/\.\d+/, '')
  const lines = [
    'BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//CL WEBIT//EOS//TW', 'CALSCALE:GREGORIAN', 'METHOD:PUBLISH',
  ]
  rows.forEach((r, i) => {
    const label = r.status === 'expired' ? '已過 EOS' : 'EOS 一年內到期'
    lines.push(
      'BEGIN:VEVENT',
      `UID:eos-${i}-${r.date}@webit`,
      `DTSTAMP:${stamp}`,
      `DTSTART;VALUE=DATE:${r.date}`,
      `SUMMARY:${icsEsc(`${label}：${r.name}（${r.count} 台）`)}`,
      `DESCRIPTION:${icsEsc(`${r.name} 官方終止支援日 ${r.date}，目前 ${r.count} 台。請排換代／延支。`)}`,
      'BEGIN:VALARM', 'TRIGGER:-P30D', 'ACTION:DISPLAY', 'DESCRIPTION:EOS 前 30 天提醒', 'END:VALARM',
      'END:VEVENT',
    )
  })
  ...
  a.download = `EOS到期提醒_${new Date().toISOString().slice(0, 10)}.ics`
  a.click()
  URL.revokeObjectURL(a.href)
  showToast(`已匯出 ${rows.length} 筆到期提醒（ICS 行事曆）`, 'success')
}
...
<template>
  <div>
    ...
    <p class="lead">硬體型號與作業系統版本的官方終止支援（EOS/EOL）日期
      <InfoNote>只查「種類」——實際資產有幾千台，但型號跟版本種類少得多，查一次全部台數都對得上。日期一律來自官方公告，查不到的標「未公佈」，不會用猜的。</InfoNote></p>

    <!-- 先列出「是哪幾台」（2026-09-16 使用者：「每個盤點都是這概念」）：
         彙總數字對得起來也沒用，人要做的事是「去那台上面看／改」。 -->
    <InventoryHosts kind="eos" />
    ...
    <template v-else-if="summary">
      <div class="tiles">
        <!-- 點磚塊＝只看該狀態（再點一次取消）。EOS 頁最常問「哪些一年內到期」。 -->
        <div class="tile bad" :class="{ on: statusFilter === 'expired' }" @click="togStatus('expired')">
          <div class="t-num mono">{{ statusCount('expired') }}</div>
          <div class="t-lbl">已過 EOS 的台數 <InfoNote>作業系統＋韌體＋硬體合計，<b>逐台</b>（同一台多筆登記只算一次）。
            下面分頁的數字是<b>種類數</b>（版本／型號有幾種），兩者本來就不一樣：一種 Windows Server 2012 可能掛著幾百台。
            2026-09-20 修正：這四格以前寫「種類」，其實一直是台數。點一下只看這些。</InfoNote></div>
        </div>
        <div class="tile warn" :class="{ on: statusFilter === 'upcoming' }" @click="togStatus('upcoming')">
          <div class="t-num mono">{{ statusCount('upcoming') }}</div>
          <div class="t-lbl">一年內到期（台） <InfoNote>該排換代／延支了。逐台。點一下只看這些。</InfoNote></div>
        </div>
        <div class="tile" :class="{ on: statusFilter === 'ok' }" @click="togStatus('ok')">
          <div class="t-num mono">{{ statusCount('ok') }}</div>
          <div class="t-lbl">尚在官方支援期（台）</div>
        </div>
        <div class="tile" :class="{ on: statusFilter === 'unknown' }" @click="togStatus('unknown')">
          <div class="t-num mono">{{ statusCount('unknown') }}</div>
          <div class="t-lbl">查不到官方日期（台） <InfoNote>未公佈或還沒查。逐台。</InfoNote></div>
        </div>
      </div>
      <p v-if="statusFilter" class="filt-note">
        只看「{{ STATUS_LABEL[statusFilter] }}」·
        <button type="button" class="lnk" @click="statusFilter = ''">清除狀態篩選</button>
      </p>
      ...
      <div class="eos-toolbar">
        ...
        <button class="btn" type="button" @click="exportIcs">匯出到期提醒（行事曆 ICS）</button>
      </div>
...
.filt-note { font-size: 12.5px; color: var(--ink-soft); margin: 4px 0 10px; }
.filt-note .lnk { background: none; border: none; color: var(--brand-dark); cursor: pointer; font: inherit; padding: 0; text-decoration: underline; }
