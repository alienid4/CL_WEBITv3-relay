...
<template>
  <div>
    ...
    <section v-if="!selected" class="card">
        ...
        <div v-if="segOpen" class="sp-body">
          ...
          <div class="sp-row">
            <label class="sp-ex">
              ...
              連「建議排除掃描」的也顯示（{{ segExcludedCount }} 段）
            </label>
            <InfoNote>網段表的註記是「<b>建議</b>排除掃描」——那是給弱點掃描的建議（UAT 環境／員工電腦／重複設備 IP 網段），不等於不能做資產收集。221 實測：內湖 30 個測試段有 29 段帶這個註記。<br><br>要收這些就打開這個開關，每一列都會寫出它被註記的原因，你看過再決定。</InfoNote>
          </div>
          <div v-if="segVisible.length" class="sp-list">
            <label class="sp-item all">
              ...
              <span>全選（{{ segPickable.length }} 段）</span>
            </label>
            <label v-for="s in segVisible" :key="s.cidr" class="sp-item" :class="{ ex: s.scan_excluded }">
              <input type="checkbox" :checked="segSel.has(s.cidr)" @change="toggleSeg(s.cidr)" />
              <span class="mono">{{ s.cidr }}</span>
              <span class="dim small">{{ s.label.replace(s.cidr, '').trim() }}</span>
              <span v-if="s.scan_excluded" class="exmark">網段表註記：{{ s.scan_note || '建議排除掃描' }}</span>
            </label>
          </div>
          <div class="sp-acts">
            <button class="btn small primary" type="button" :disabled="!segSel.size" @click="addSelectedSegs">
              加入範圍（已勾 {{ segSel.size }} 段）
            </button>
            <span class="dim small">勾好加進下面的輸入框，確認台數後再按「開始收集」</span>
          </div>
        </div>
      </div>
      ...
      <CollectLog ref="logRef" :kinds="['dispatch']" title="開始收集紀錄" />
