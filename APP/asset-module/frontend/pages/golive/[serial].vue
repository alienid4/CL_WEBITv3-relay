      ...
      <div class="card head">
        <div class="head-main">
          ...
          <p v-if="isPassed" class="rv-hint" style="margin:0">
            ...
            <InfoNote>通過當下的自動判定結果已存成基線，之後每天回檢；被改掉會出現在「基線失效」清單。</InfoNote>
          </p>
          <p v-else class="rv-hint" style="margin:0">
            還差 <b>{{ detail.total - detail.done }}</b> 項
            <InfoNote>自動項目測不到時不會放行——那代表沒人真的看過這台機器，不是通過。</InfoNote>
          </p>
        </div>
...
.badge.ok { border-color: var(--brand); color: var(--brand-dark); }
...
.btn.ghost:hover { border-color: var(--brand); color: var(--brand-dark); background: none; }
...
.tag.auto { border-color: var(--brand); color: var(--brand-dark); }
.verdict { font-size: 12px; }
.verdict.pass { color: var(--brand-dark); font-weight: 700; }
.verdict.na { color: var(--muted); }
.verdict.fail { color: var(--bad); font-weight: 700; }
.verdict.unknown { color: var(--warn-text); }
