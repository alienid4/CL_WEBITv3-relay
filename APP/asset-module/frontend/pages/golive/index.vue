...
<template>
  <div>
    ...
    <div class="breadcrumb-bar"><span class="pin">📌</span> <b>上線前檢查</b>
      <InfoNote>新資產（申請單轉錄或直接新增）都會自動開一份上線前檢查表，項目來自公司現行的「伺服器上線前檢查表」。<b>機器測得到的項目自動判定</b>（監聽埠、OS 版本），其餘人工勾。<b>全部處理完才能按通過</b>，通過的同時資產轉「使用中」，並把當下的狀態記成基線——之後每天回檢，被改掉就會出現在「基線失效」清單。</InfoNote>
    </div>
