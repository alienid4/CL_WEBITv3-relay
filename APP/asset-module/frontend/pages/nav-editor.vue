...
// 匯出／匯入（2026-09-20 使用者：221 跟公司機選單順序不同，「我要抓什麼回來才會同步」）。
// 選單順序存在各台自己的資料庫、不隨 patch 走；整庫備份雖然含這份設定，但為了搬選單
// 去覆蓋整個資料庫太超過，所以給一個只搬選單的檔。
function exportLayout() {
  const base = (useRuntimeConfig().public as any).apiBase || ''
  window.open(base + '/api/nav-layout/export', '_blank')
}
...
async function onImportFile(ev: Event) {
  ...
  try {
    ...
    const r = await apiFetch<{ groups: number; items: number; note: string }>(
      '/api/nav-layout/import', { method: 'POST', body })
    await load()
    msg.value = `${r.note}——重新整理任一頁就會看到（全站共用）`
  } catch (e: any) {
    msg.value = `匯入失敗：${e?.data?.detail || e?.message || '檔案不是有效的選單設定'}`
  } finally {
    saving.value = false
    if (importFile.value) importFile.value.value = ''   // 同一個檔要能再選一次
  }
}
...
<template>
  <div class="ne">
    ...
    <p class="lead">拖拉項目在群組之間搬；群組可上下移、改名、新增
      <InfoNote>選單順序存在<b>這一台自己的資料庫</b>，不隨程式更新（patch）走——所以兩台機器會各自不同。
        要同步就在排好的那台按「匯出設定」，到另一台按「匯入設定」。整庫備份本來就含這份設定，
        但為了搬選單去還原整個資料庫太超過，所以另給這個只搬選單的檔。<br><br>
        這是<b>全站共用一份順序</b>，你改了大家（含手機、其他人）看到的選單都會變。編號（2-1、3-7…）會依你排的順序自動重算。改完按「儲存」，重新整理就生效；不滿意按「回復預設」。</InfoNote>
    </p>
    ...
    <template v-else>
      <div class="bar">
        ...
        <span class="sep" />
        <button class="btn" :disabled="saving" title="把這台的選單設定存成一個檔，拿到另一台匯入"
                @click="exportLayout">匯出設定</button>
        <button class="btn" :disabled="saving" title="匯入另一台匯出的選單設定（會覆蓋這台目前的順序）"
                @click="importFile?.click()">匯入設定</button>
