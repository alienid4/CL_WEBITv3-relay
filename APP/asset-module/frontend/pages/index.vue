...
interface DashboardStats {
  ...
  total_ica_count: number            // 筆（副標用）
  total_overlap_count: number        // 筆
  total_ica_machines?: number        // 台（檯面用；舊後端沒有就退回筆）
  total_overlap_machines?: number
...
// [2026-09-20] 首頁的頭條、對帳、組成統計全部是**全站**口徑（理由見 hero 註解），
// 所以這頁不再有環境下拉。API 仍吃 environment 參數（給別頁用），這裡固定要全部，
// 免得留下一個「看起來會篩、其實不會」的控制項。要看單一環境：3-1 資產查詢／1-4 分佈統計。
const environment = '全部' 
...
async function loadIssues() {
  try {
    ...
    issues.value = []
    showToast(`問題清單載入失敗：${e?.data?.detail ?? e?.message ?? '未知錯誤'}`, 'error')
  }
}
...
//
// ⚠️ 2026-09-20 公司 198.14：按下去按鈕閃一下就跳回「重新掃描」、時間戳沒變，人無從判斷跑了沒。
// 原因是這裡只看 status.running／status；剛按下的瞬間新的一輪還沒建紀錄，讀到的是**上一輪**的
// 「ok」，就當成完成了（還會報上一輪的台數）。改成記下按之前的 started_at，
// 一定要看到**新的一輪**（started_at 不同）才算數；等不到就老實說「還沒看到新的掃描紀錄」。
function pollScan(startTs: number, prevStartedAt: string | null) {
  const TIMEOUT = 120000
  if (pollTimer) clearInterval(pollTimer)
  pollTimer = setInterval(async () => {
    scanElapsed.value = Math.round((Date.now() - startTs) / 1000)
    if (Date.now() - startTs > TIMEOUT) {
      if (pollTimer) clearInterval(pollTimer)
      scanning.value = false
      showToast('等了 2 分鐘還沒看到這一輪的掃描紀錄。它可能還在背景跑（網段多時會很久），'
                + '稍後重新整理、或到「系統設定 → 掃描排程」看第三關的段數', 'warn', 12000)
      return
    }
    let s: any
    try {
    ...
    if (s.running) return
    // 還是上一輪的紀錄＝這一輪還沒開始建紀錄，繼續等（不可以拿上一輪的結果報喜）
    if (s.started_at && prevStartedAt && s.started_at === prevStartedAt) return
...
async function triggerRescan() {
  ...
  scanning.value = true
  scanElapsed.value = 0
  // 先記下「按之前最後一輪是什麼時候開始的」，才分得出等一下看到的是不是新的一輪
  let prevStartedAt: string | null = null
  try {
    prevStartedAt = (await apiFetch<any>('/api/scan/status'))?.started_at ?? null
  } catch { /* 讀不到就當沒有上一輪，後面照樣等新紀錄 */ }
  try {
    await apiFetch('/api/scan/run', { method: 'POST' })
    showToast('已開始掃描（背景跑，可以離開這頁）', 'info', 6000)
  } catch (err: any) {
    scanning.value = false
  ...
  pollScan(Date.now(), prevStartedAt)
}
...
// 開頁時先問一次掃描狀態，只為了決定要不要顯示「掃描中」那顆轉圈。
// 問不到就當沒有在掃（不顯示），下面的輪詢每幾秒會再問一次並把真正的錯誤顯示出來。
...
// 在管資產＝只算「正式登記」（排除帳外 DYN-/VC-/AUTO-），跟公司官方清冊同一種算法，
// 兩邊數字才對得起來（2026-09-07 使用者定案）。舊後端沒 registered_total 就退回 total。
// headline 用「台數」（去重）＝跟各盤點頁一致；登記「筆數」放括號＝跟公司清冊一致。
// 舊後端沒 registered_hosts 就退回 registered_total（筆），再退回 total。
const totalAssetCount = computed(() =>
  comp.value?.registered_hosts ?? comp.value?.registered_total ?? comp.value?.total
  ?? stats.value?.total_ica_count ?? 0)
const registeredRows = computed(() => comp.value?.registered_total ?? 0)
// 帳外＝DY／vCenter 有、CIA 清冊沒有，另外顯示、不併進在管資產頭條。
// [口徑] 2026-09-20 公司驗收：這裡原本顯示 off_book_total（**筆**）卻寫「台」，
// 而且叫「未登記」，跟對帳明細的「未登記」（這次掃到、清冊查無）撞名。逐台優先，名字也改掉。
const offBookCount = computed(() => comp.value?.off_book_machines ?? comp.value?.off_book_total ?? 0)
const offBookRows = computed(() => comp.value?.off_book_total ?? 0)
const retiredRows = computed(() => comp.value?.retired_count ?? 0)
...
interface ManageState {
  ...
  total_known: number
  not_covered_total?: number; covered_segments?: number
...
// 2026-09-10：四態那一區已搬到 1-3 異常報告，fetch 一併拿掉——但下方對帳明細的「已納管」
// 「已收集」還在用 stateCount，結果從那天起一直顯示 0（221 實際已納管 1 台）。
// 2026-09-18 十項盤點抓到，恢復抓取（summary=1 只拿數字，不拿 4,788 筆明細）。
// ⚠️ 宣告後立刻 await，跟下面 comp 同一個寫法：不可以放進上方 reloadAll()——那個在
// setup 更早的 await 裡跑，這行還沒宣告 → TDZ（資產詳細頁 SAN 載不到就是這類錯）。
// 抓失敗時 mstate 維持 null，stateCount 回 null、畫面顯示「—」，不可以顯示成 0。
const mstate = ref<ManageState | null>(null)
try {
  mstate.value = await apiFetch<ManageState>('/api/manage-state', { params: { summary: 1 } })
} catch (e: any) {
  console.warn('四態數字載入失敗，對帳明細的已納管／已收集／登記矛盾會顯示「—」', e)
}

// 抓不到四態時回 null（畫面顯示「—」），不回 0——0 會被當成「真的沒有」
// 十項盤點第 1 條（2026-09-18）：掃描範圍沒套用時，「失聯／搜不到／還需要處理」幾乎全是
// 「沒掃過」。221 實測未涵蓋 3,910／4,369 台（90%），首頁卻沒有任何提示。超過三成就亮紅燈。
const scanGap = computed(() => {
  const m = mstate.value
  if (!m || !m.total_known || !m.not_covered_total) return null
  const pct = (m.not_covered_total / m.total_known) * 100
  return pct >= 30 ? { n: m.not_covered_total, total: m.total_known, pct: Math.round(pct), segs: m.covered_segments ?? 0 } : null
})
function stateCount(k: string): number | null { return mstate.value ? (mstate.value.counts?.[k] ?? 0) : null }

// 組成統計：儀表板該回答「我的機器長什麼樣子」（幾台 Windows／虛實／環境），
// 那是統計；「兩邊相符／登記卻掃不到」是對帳細節，屬於小功能，不該佔戰情室頭條。
interface Composition {
  total: number
  // 在管資產頭條改用 registered_total（只算正式登記、排除帳外），跟公司官方清冊
  // 同一種算法；帳外另用 off_book_total 顯示。舊後端沒這兩欄，選用、退回用 total。
  registered_total?: number
  registered_hosts?: number
  off_book_total?: number
  off_book_machines?: number
...
// 2026-09-10：體檢那一區已搬到 1-3 異常報告，這裡不再顯示。
// **連 fetch 一起拿掉**——留著會在每次開總覽時多打一支沒人看的 API。
// 型別與 topIssues 留著沒有成本（computed 沒被引用就不會執行），
// 但真的打出去的請求有成本。
...
const drill = computed(() => {
  return {
    // 在管資產＝只算正式登記（排除帳外），點進去也要同口徑，否則數字對不上（天條）
    allAssets: { path: '/assets', query: { source: 'registered' } },
    offBook: { path: '/assets', query: { source: 'offbook' } },
    // [2026-09-20] 下鑽不再帶環境：檯面是全站，帶了會變成「檯面全站、點進去只剩正式」
    overlap: { path: '/assets', query: { scan_status: 'overlap' } },
    icaOnly: { path: '/assets', query: { scan_status: 'ica_only' } },
...
// [口徑] 檯面一律逐台（跟頭條「在管 N 台」同一把尺）；筆數放副標。
// 2026-09-20 公司驗收看到「搜不到 3,417 > 在管 3,374 台」，就是檯面用了筆數。
const icaOnlyAll = computed(() => {
  const s = stats.value
  if (!s) return 0
  return (s.total_ica_machines ?? s.total_ica_count) - (s.total_overlap_machines ?? s.total_overlap_count)
})
const icaOnlyRows = computed(() => {
  const s = stats.value
...
const reconRows = computed(() => {
  ...
  if (!s) return []
  return [
    { key: '一致', tone: 'good', n: s.total_overlap_machines ?? s.total_overlap_count,
      sub: `登記有、網路上也掃得到（${n(s.total_overlap_count)} 筆登記）`, to: drill.value.overlap },
    { key: '搜不到', tone: 'bad', n: icaOnlyAll.value,
      sub: `登記在案，這次掃描沒回應（${n(icaOnlyRows.value)} 筆登記）`, to: drill.value.icaOnly },
    // 改名：跟圓環旁邊的「CIA 之外」分開——這一列是「這次掃到、清冊完全查無」
    { key: '這次掃到、清冊查無', tone: 'warn', n: s.scan_only_count,
      sub: '網路上活著，資產庫（含退役／帳外）查無此機', to: drill.value.scanOnly },
    { key: '已納管', tone: 'good', n: stateCount('已納管'),
      sub: '連得進去，收得到主機名／OS／序號', to: { path: '/assets' } },
    // 2026-09-15：設備「收集」≠「納管」，獨立一列；跟已納管一樣不算有問題
    { key: '已收集', tone: 'good', n: stateCount('已收集'),
      sub: 'SAN switch 等設備資料已收集（純收集，非納管）', to: { path: '/san' } },
    // [B-04] 同一台有的登記標退役、有的仍使用中——算待辦，要確認 CIA 哪一筆才對
    { key: '登記矛盾', tone: 'warn', n: stateCount('登記矛盾'),
      sub: '同一台有的標退役、有的仍使用中，確認 CIA 哪筆對', to: { path: '/pipeline', query: { stage: 'conflict' } } },
    // [B-07] 標退役卻還掃得到／收得到——稽核發現（該關沒關，或清冊標錯）
    { key: '退役但仍在線', tone: 'bad', n: stateCount('退役但仍在線'),
      sub: '清冊標停用／報廢／閒置，卻還掃得到或收得到', to: { path: '/pipeline', query: { stage: 'retired_alive' } } },
    // [B-09] 沒有 IP＝掃描無從判斷，以前被算成失聯（221 失聯 316 台全是這種）
    { key: '沒有 IP', tone: 'warn', n: stateCount('沒有 IP'),
      sub: '登記了但沒有可掃描的 IP，在不在無從判斷；到 CIA 補 IP', to: { path: '/pipeline', query: { stage: 'no_ip' } } },
    // [口徑] 2026-09-20：這裡原本是 composition.retired_count（**筆**、含帳外）＝162，
    // 跟漏斗 105 台、分佈統計 123 台三個並存。檯面改用跟漏斗同一份（逐台），筆數放副標。
    { key: '已退役', tone: 'muted', n: stateCount('已退役') ?? retiredRows.value,
      sub: `停用／報廢／閒置，不計入在管台數（${n(retiredRows.value)} 筆登記；同一台有退役也有使用中的另計在「登記矛盾」）`,
...
<template>
  <div class="dash">
    ...
    <header class="top">
      ...
      <div class="tright">
        <!-- [2026-09-20] 環境下拉拿掉：這頁的頭條與對帳**刻意是全站口徑**（理由見 hero 註解），
             所以切環境一個數字都不會變，使用者實測後以為壞了；而它又偷偷把環境帶進下鑽連結，
             造成「檯面全站、點進去只剩正式」。要看單一環境請用 3-1 資產查詢或 1-4 分佈統計。 -->
        <button class="btn-primary" :disabled="scanning" @click="triggerRescan">
          <span v-if="scanning" class="spin" />{{ scanning ? `掃描中…（${scanElapsed} 秒）` : '重新掃描' }}
        </button>
      </div>
    </header>
    <!-- 掃描範圍不完整的紅燈：數字不可信時要先講，不要讓人照著錯的數字去查機器 -->
    <NuxtLink v-if="scanGap" to="/settings?tab=schedule" class="scan-gap">
      <b>⚠ 掃描範圍不完整：{{ scanGap.n.toLocaleString() }} / {{ scanGap.total.toLocaleString() }} 台（{{ scanGap.pct }}%）這次沒被掃到</b>
      <span>最近一次掃描只涵蓋 {{ scanGap.segs }} 個網段——下方「搜不到」、漏斗「還需要處理」大多是<b>沒掃過</b>，不是失聯。到系統設定的掃描範圍按「套用」→</span>
    </NuxtLink>
        ...
        <div class="mainc">
          ...
          <section class="card hero">
            <div class="gauge">
              ...
              <NuxtLink :to="drill.allAssets" class="gc" title="只算正式登記的資產（不含退役、不含帳外）">
                <div class="gn mono">{{ n(totalAssetCount) }}<small class="gu"> 台</small></div>
                <div class="gl">在管資產<small v-if="registeredRows > totalAssetCount">（登記 {{ n(registeredRows) }} 筆）</small><small v-else>（正式登記）</small></div>
                <div class="gp mono"
                     :title="stats ? `帳實相符 = 帳上有且網路也在 ÷（帳上有 ∪ 網路上有）\n= ${n(stats.total_overlap_count)} ÷ (${n(stats.total_ica_count)} ∪ ${n(stats.scan_only_count)}) = ${matchPct.toFixed(1)}%` : ''">
                  {{ matchPct.toFixed(1) }}% 帳實相符</div>
              </NuxtLink>
            </div>
            <NuxtLink v-if="offBookCount" :to="drill.offBook" class="offbook-note">
              另有 <b class="mono">{{ n(offBookCount) }}</b> 台在 CIA 之外（DY／vCenter 有、CIA 清冊沒有；{{ n(offBookRows) }} 筆登記）·
              <span class="ob-arw">看清單 ›</span>
            </NuxtLink>

            <div class="recon">
              <div class="ck">RECONCILIATION
                <InfoNote>外環＝帳實相符率、內環＝資料收集率
                  {{ collectPct.toFixed(1) }}%（{{ n(comp?.os_from_facts) }} 台的 OS 是機器自己回報的）。</InfoNote>
              </div>
...
.gl small { color: var(--muted); font-weight: 400; }
.gu { font-size: 16px; font-weight: 400; letter-spacing: 0; color: var(--muted); }
/* 未登記（帳外）另計：放在在管資產下面（grid 第 1 欄第 2 列，剛好在儀表下方） */
.offbook-note { grid-column: 1; align-self: start; text-align: center; margin-top: -12px;
  font-size: 11.5px; color: var(--muted); text-decoration: none; line-height: 1.5; }
.offbook-note b { color: var(--warn-text); }
.offbook-note:hover { color: var(--ink); }
.offbook-note .ob-arw { color: var(--brand-dark); white-space: nowrap; }
...
.scan-gap { display: block; margin: 8px 0 12px; padding: 10px 14px; border-radius: 10px; background: #fdecec; border: 1px solid #f1b0b0; color: #8a1c1c; text-decoration: none; }
.scan-gap b { display: block; font-size: 14px; }
.scan-gap span { font-size: 12px; }
.scan-gap span b { display: inline; font-size: 12px; }
</style>