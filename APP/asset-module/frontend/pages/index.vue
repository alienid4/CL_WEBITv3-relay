...
const totalAssetCount = computed(() =>
  comp.value?.registered_total ?? comp.value?.total ?? stats.value?.total_ica_count ?? 0)
// 帳外＝掃到/vCenter 有、但沒登記在 CIA 的，另外顯示、不併進在管資產頭條。
const offBookCount = computed(() => comp.value?.off_book_total ?? 0)
...
// 2026-09-10：四態那一區已搬到 1-3 異常報告，這裡不再顯示，fetch 一併拿掉。
...
const drill = computed(() => {
  ...
  return {
    ...
    allAssets: { path: '/assets', query: { environment: env, source: 'registered' } },
