...
<template>
  <div>
    ...
    <div class="quickfilter">
      <!-- OS 類型（大類，跟漏斗同一套）＋ Linux 細分（2026-09-18）。
           數字是「筆」（登記筆數，這頁一筆一列）；漏斗與交叉表是「台」，兩者會不同——畫面上講明。 -->
      <div class="qf-row">
        <span class="qf-lbl">OS 類型 <InfoNote>跟納管漏斗、OS×環境交叉表<b>同一套分類</b>（含你在漏斗手動改過的）。這頁一筆一列，數字是<b>登記筆數</b>；漏斗與交叉表算的是<b>台數</b>（同一台多筆登記只算一次），所以數字會比漏斗大。</InfoNote></span>
        <button
        ...
        <button v-if="osTypeFilter || osDetailFilter || platformFilter" class="chip all" type="button" @click="clearOsFilters">清除</button>
      </div>
      ...
      <div class="qf-row">
        <span class="qf-lbl">類型 <InfoNote>類型由服務盤點推導（在聽 3306 就是資料庫），不是人填的欄位。</InfoNote></span>
        <button
