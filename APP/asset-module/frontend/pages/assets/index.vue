  ...
  <div>
    ...
    <template v-if="activeTab === 'hardware' && fieldGroups">
      ...
      <div v-if="dupData && dupData.groups.length" class="dup-box">
        <div class="dup-head">
          ⚠ 發現 {{ dupData.groups.length }} 組重複登記的資產（可少 {{ dupData.extra_rows }} 筆）
          <InfoNote>主機名稱與 IP 都相同＝同一台被登記多次。清掉重複的可少 {{ dupData.extra_rows }} 筆，資產總數才會是真的。「內容完全相同」的可放心刪掉多餘那筆；「有欄位不同」請先點進去比對再決定留哪一筆——系統不替你選，因為選錯會丟掉正確的資料。</InfoNote>
          ...
          <thead>
            <tr>
              ...
              <SortTh k="trust_score" :active="hwSortKey" :dir="hwSortDir" @sort="sortHardware">
                <span title="資料可信度：dynassets／RVTools／CIA 幾個來源有這台（滿分 70，只算適用的）＋機器證據（網路通 +20／已納管 +30）。分布與下鑽在 3-3 資料品質">可信度</span>
              </SortTh>
            ...
            <tr v-for="row in hardwareRows" :key="row.asset_serial">
              ...
              <td>
                <NuxtLink v-if="row.trust_score !== null && row.trust_score !== undefined"
                          ...
                          :title="`dynassets：${row.trust_sources?.dynassets === null ? '不適用' : row.trust_sources?.dynassets ? '有' : '沒有'}｜RVTools：${row.trust_sources?.rvtools === null ? '不適用' : row.trust_sources?.rvtools ? '有' : '沒有'}｜CIA：${row.trust_sources?.cia ? '有' : '沒有'}｜機器證據：${row.trust_evidence === 'managed' ? '已納管 +30' : row.trust_evidence === 'alive' ? (row.trust_alive_basis === 'dynassets' ? `網路通 +20（依據 dynassets 存活清單，匯入 ${(row.trust_dyn_at || '').slice(0, 10)}）` : '網路通 +20（我們掃描看到）') : '沒有'}`">
                  {{ row.trust_score }}
                </NuxtLink>
