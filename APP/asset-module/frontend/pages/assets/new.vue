...
<template>
  <div>
    ...
    <div class="card">
      <div class="card-title">手動新增資產 <InfoNote>只有一台要單獨補登記時用這裡，不用重跑整份 Excel 匯入。欄位跟 CIA 資產清單一致；<b>資產序號</b>必填且不能跟既有資產重複。送出後資產是<b>「待上線」</b>，要過完上線前檢查表才會變成「使用中」。</InfoNote></div>
      ...
      <div v-if="provisionSource === 'form'" class="fgroup prov-block">
        <div class="fgroup-hd">
          <span class="fgroup-label">申請單資訊 <InfoNote>這張單的來歷，之後稽核要查「當初是誰申請、誰簽的」。</InfoNote></span>
        </div>
      ...
      <label class="manual-vm-toggle">
        ...
        是否為 VM <InfoNote>勾選後會隱藏機櫃編號／硬體編號——這兩個只有實體機才有意義。</InfoNote>
      </label>
    ...
    <div class="card">
      <div class="card-title">網段存活掃描 <InfoNote>輸入網段（例：<code>192.168.1.0/24</code>，只能是內網私有網段），掃這段裡有哪些主機開了 <b>22（SSH）</b>或 <b>445（SMB）</b>——用這兩個 port 判斷是不是「連得到、可管理」的存活主機。掃完只有 IP／主機名，沒有業務欄位，納入後可以到資產詳細頁補。</InfoNote></div>
      <div class="actions">
        <input
          v-model="scanCidr"
          type="text"
