...
const canOnboard = computed(() => {
  const hw = detail.value?.hardware
  return hw && hw.ip && hw.collect_ok !== 1 && !hw.onboard_block
})
    ...
    <template v-if="detail && fieldGroups">
      <div class="host-head">
        ...
        <div class="head-right">
          ...
          <button v-if="!editing && isOnboarded" class="ebtn" type="button" :disabled="collectingSpec"
                  @click="collectSpec"
          ...
          <div v-else class="box-body">
            ...
            <template v-if="b.label === '網路／環境'">
              <div class="f">
                ...
                <div v-else class="idrow"><span class="chip ip">{{ detail.hardware.hostname ?? '—' }}</span></div>
              </div>
              <div class="f">
                <label>IP</label>
                <input v-if="editing" v-model="draft.ip" class="ein" />
                <div v-else class="idrow">
                  <span class="chip ip">{{ detail.hardware.ip ?? '—' }}</span>
