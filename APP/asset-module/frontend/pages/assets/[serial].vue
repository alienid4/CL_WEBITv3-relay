...
interface VmPlacement { found: boolean; esxi?: string; esxi_serial?: string | null; esxi_ip?: string | null; cluster?: string; site?: string; env?: string }
...
// 「人員」分頁的數字：原本只算關聯人員紀錄（personnel 表，常年是 0），沒算上面那排
// 已填的人員/單位欄位（保管者/使用者/使用單位/所屬公司…）。2026-09-13 使用者：明明有
// 資料卻顯示 0。改成「有填的人員/單位欄位數 ＋ 關聯人員紀錄數」，有資料就不會是 0。
const peopleCount = computed(() => {
  const hw = detail.value?.hardware ?? {}
  const filled = peopleFields.value.filter((k) => {
    const v = hw[k]
    return v !== null && v !== undefined && String(v).trim() !== ''
  }).length
  return filled + (detail.value?.personnel.length ?? 0)
})

// 進階分頁＝advanced 扣掉「人員」那組：保管者/使用單位/所屬公司在 field_groups.json
// 同時列在 advanced（給『資產查詢』列表頁當欄位）與 people（詳細頁人員分頁）。詳細頁這裡
// 把 people 的欄位從進階扣掉，避免同一欄在進階＋人員兩個分頁重複出現（2026-09-13）。
const advancedFields = computed(() => {
  const adv = fieldGroups.value?.hardware.advanced ?? []
  const ppl = new Set(fieldGroups.value?.hardware.people ?? [])
  return adv.filter((k) => !ppl.has(k))
})
  ...
  <div>
    ...
    <template v-if="detail && fieldGroups">
      ...
      <div v-if="activeTab === 'advanced'" class="field-grid">
        <div v-for="key in advancedFields" :key="key" class="f">
          <label :title="fieldHelp(key) || undefined">{{ fieldLabel(key) }}</label>
          <template v-if="editing">
            <select v-if="optionsOf(key)" v-model="draft[key]" class="ein">
              <option value="">—</option>
        ...
        <!-- 虛擬化位置：只有在 vCenter 關聯圖查得到的 VM 才補這兩格（VC 以叢集名代表；
             實體機/查無關聯者不顯示，機房與機櫃已是上面的登記欄位）。 -->
        <template v-if="placement && placement.found">
          <div class="f">
            ...
            <div v-if="detail.hardware.vi_sdk_server">{{ detail.hardware.vi_sdk_server }}</div>
          ...
          <div class="f">
            ...
            <div>{{ placement.cluster || '—' }}</div>
          </div>
          <div class="f">
            <label>虛擬化 ESXi</label>
            <div><NuxtLink v-if="placement.esxi_serial" class="dl" :to="`/assets/${encodeURIComponent(placement.esxi_serial)}`">{{ placement.esxi }}</NuxtLink><template v-else>{{ placement.esxi }}</template></div>
          </div>
          <div class="f">
            <label>虛擬化 ESXi IP <InfoNote>VM 實際落在哪台實體主機的 IP（該 ESXi 有登記成資產才有）。vCenter 伺服器本身的 IP 系統未收（vi_sdk_server 為空），故不顯示。</InfoNote></label>
            <div>{{ placement.esxi_ip || '—' }}</div>
          </div>
        </template>
        <div v-else-if="placement && vmMarked" class="f">
...
.subh { font-size: 13px; color: var(--ink-soft); margin: 18px 12px 6px; }
/* 軟體清單搜尋列 */
