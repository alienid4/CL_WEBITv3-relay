...
async function submitBsImport() {
  if (!bsFile.value) { bsError.value = '請先選擇對照表 .xlsx'; return }
  ...
  try {
    ...
    showToast(`對照表匯入完成：${bsResult.value.imported} 個系統`, 'success')
    bsFile.value = null
  } catch (err: any) {
    const d = err?.data?.detail
    bsError.value = (typeof d === 'string' ? d : d?.message) ?? '對照表匯入失敗'
    showToast(bsError.value, 'error')
  } finally {
...
<template>
  <div>
    ...
    <div id="bizsys" class="card">
      <div class="card-title">業務系統對照表</div>
      <p class="rv-hint">
        資產庫裡只有 <code>api_id</code>（<code>N-008</code> 這種代碼），沒有中文名稱——
        畫面上到處是看不懂的代碼，帳號盤點要交出去的 Excel 第一欄也需要它。
        上傳一份對照表（.xlsx，第一列是表頭）就補齊：<b>系統代碼／系統名稱／AP 部門／AP 負責人</b>。
        <br>
        欄名不寫死，<code>system_id</code>、<code>AP_ID</code>、<code>系統代碼</code> 都認得。
        以系統代碼為鍵，<b>重匯是更新同一筆，不會長出重複</b>。
      </p>
      <div class="dropzone">
        <div class="dropzone-text">選擇業務系統對照表</div>
        <label class="btn file-label">
          選擇檔案
          <input type="file" accept=".xlsx" class="file-input" @change="handleBsFile" />
        </label>
        <div class="dropzone-hint">
          至少要有系統代碼那一欄；名稱／AP 部門／AP 負責人缺了就留空，之後補匯會自動補上（不會洗掉既有值）
        </div>
        <div v-if="bsFile" class="selected-file">已選擇：{{ bsFile.name }}</div>
      </div>
      <p v-if="bsError" class="error-text">{{ bsError }}</p>
      <div class="actions">
        <button class="btn" type="button" :disabled="bsUploading" @click="submitBsImport()">
          {{ bsUploading ? '匯入中…' : '匯入對照表' }}
        </button>
      </div>
    ...
    <div id="dynassets" class="card">
      ...
      <!-- 空白範本：緊鄰上傳（2026-08-25 通則）。原本這裡只講「需含 IP 欄」，
           查證發現系統其實還吃主機名／MAC／OS 這些欄位，範本把全部欄位列出來，
           順便更正這句舊文案。 -->
      ...
      <div class="dropzone">
        ...
        <div class="dropzone-hint">支援 .csv／.xlsx（一列一台存活主機，需含 IP 欄；主機名／MAC／作業系統等欄位也吃得到，範本裡都列出來了）</div>
    ...
    <div id="cia" class="card">
      ...
      <div class="card-title">
        上傳新檔案（CIA 資產清單 Excel）
        <span class="sheets">三個分頁：硬體／<b>人員（通訊錄・分機・代理人）</b>／軟體</span>
      </div>
      ...
      <p class="rv-hint sm">
        回交給資產清單維護單位時：<b>不勾</b>＝只給你們登記過的那批（更新機器事實）；
        <b>勾選</b>＝連我們額外盤到、你們清單上沒有的機器也一起給，檔案裡會多一欄
        「來源」分得出哪些是新發現的。
      </p>
    ...
    <div class="card" style="border:1px solid #c0392b">
      <div class="card-title" style="color:var(--bad)">⚠️ 清空所有盤點資料</div>
      <p class="rv-hint">
        把資產／人員／軟體／掃描／帳號盤點／待審／業務系統／匯入紀錄<b>全部刪除</b>，回到空白供重新匯入。
        <b>會保留</b>登入帳號、憑證庫、連線設定、授權網段、系統設定。
        <b style="color:var(--bad)">此動作不可逆</b>（只能靠備份還原）——執行前請先到管理頁按「立即備份」。
      </p>
