  ...
  <div>
    ...
    <div v-if="exportSources.length" class="card">
      <div class="card-title">
        匯入
        <InfoNote>四種來源都在這裡選檔匯入，一眼看得到「有哪些、現在各幾筆」——這是<b>唯一的上傳入口</b>。<br><br>RVTools 那列可以<b>一次選多個檔</b>（每台 vCenter 一個），會一個一個匯。<br><br>按「明細」跳到下面的卡片，看匯入結果的逐檔報告、RVTools 的待審核合併、CIA 的欄位對應與匯出、dynassets 空白範本。</InfoNote>
      </div>
      <div class="tbl-wrap">
        <table class="exp-tbl">
          <thead><tr>
          ...
          <tbody>
            <tr>
              ...
              <td><input type="file" accept=".csv,.xlsx" :disabled="dynUploading" @change="handleDynFile" /></td>
              <td class="ops">
                ...
                <button v-if="exportSrc('dynassets')" class="btn small" type="button"
                        :disabled="!rowCount('dynassets') || !!exportBusy"
                        title="匯出目前資料（同匯入欄位，可原封匯回）" @click="doExport(exportSrc('dynassets')!, 'excel')">匯出</button>
            ...
            <tr>
              ...
              <td class="ops">
                ...
                <button v-if="exportSrc('rvtools')" class="btn small" type="button"
                        :disabled="!rowCount('rvtools') || !!exportBusy"
                        title="匯出目前資料（同匯入欄位，可原封匯回）" @click="doExport(exportSrc('rvtools')!, 'excel')">匯出</button>
            ...
            <tr>
              <td>系統類別對照表
                <InfoNote>第一／二／三類系統的分級表（例：APID.xlsx，欄位「系統類別」＋「APID」）。<br><br><b>只會更新系統類別</b>，不動 AP 部門與負責人。<b>整份取代</b>：不在這份清單裡的系統會回到「未分級」。<br><br>同一個代碼在檔案裡有兩種類別時不會猜，那個系統不設類別並列出來；清單有、對照表沒有的代碼也會列出來，不會自己新建。</InfoNote>
              </td>
              <td class="num">—</td>
              <td class="fmt">.xlsx／.csv</td>
              <td><input type="file" accept=".xlsx,.csv" :disabled="clsUploading" @change="handleClsFile" /></td>
              <td class="ops">
                <button class="btn small primary" type="button"
            ...
            <tr>
              ...
              <td><input type="file" accept=".csv,.xlsx" :disabled="bsUploading" @change="handleBsFile" /></td>
              <td class="ops">
                <button class="btn small primary" type="button"
                ...
                <button v-if="exportSrc('business_system')" class="btn small" type="button"
                        :disabled="!rowCount('business_system') || !!exportBusy"
                        title="匯出目前資料（同匯入欄位，可原封匯回）" @click="doExport(exportSrc('business_system')!, 'excel')">匯出</button>
            ...
            <tr>
              ...
              <td><input type="file" accept=".xlsx" :disabled="uploading" @change="handleFileInput" /></td>
              <td class="ops">
                <button class="btn small primary" type="button"
                ...
                <button v-if="exportSrc('cia_excel')" class="btn small" type="button"
                        :disabled="!rowCount('cia_excel') || !!exportBusy"
                        title="匯出目前資料（同匯入欄位，可原封匯回）" @click="doExport(exportSrc('cia_excel')!, 'excel')">匯出</button>
            ...
            <tr>
              ...
              <td><input type="file" accept=".xlsx,.xlsm,.txt,.tsv,.csv" :disabled="segUploading" @change="handleSegFile" /></td>
              <td class="ops">
                <button class="btn small primary" type="button"
                ...
                <button class="btn small" type="button" :disabled="segExporting || !segCount"
                        title="匯出目前網段（同匯入欄位，可原封匯回）" @click="exportSegments">匯出</button>
    ...
    <!-- 匯出：三種格式各回答不同的問題，所以按鈕分開擺、各自寫清楚 -->
    <div v-if="exportSources.length" class="card">
      <div class="card-title">
        匯出
        <InfoNote>三種格式回答的是<b>不同的問題</b>，不是同一份東西的三種包裝：<br><br><b>原始檔</b>＝當初送進來的那個檔，一個位元都沒動。要拿去跟來源系統對帳就用這個。只留最新一份，重匯會蓋掉。<br><b>Excel</b>＝<b>現在</b>系統裡是什麼（正規化之後的結果）。要給人看、要改、要轉寄就用這個。<br><b>dump</b>＝整包搬到另一台機器用的格式，可以再匯回來。<br><br>⚠️ dump <b>目前是明碼</b>，裡面是完整的資產清單（IP、主機名、用途、負責人）。不要當備份寄出去——要寄請用「備份與還原」那邊的加密匯出。</InfoNote>
      </div>
      <div class="tbl-wrap">
        <table class="exp-tbl">
          <thead><tr>
          ...
          <tbody>
            <tr v-for="src in exportSources" :key="src.key">
              <td>{{ src.label }}</td>
              <td class="num">{{ src.row_count }}</td>
              <td class="orig">
                <template v-if="src.has_original">
                  <span class="mono">{{ src.original_name }}</span>
                  <span class="exp-sz">{{ prettySize(src.original_size) }}</span>
                </template>
                <span v-else class="muted">
                  沒有<InfoNote>不是壞掉。以前的匯入流程處理完就把上傳的暫存檔刪了，所以這個功能上線<b>之前</b>匯入的那些，原始檔是<b>真的不存在</b>。<br><br>這裡刻意<b>不拿重新產生的檔頂替</b>——那看起來一樣，但欄位是系統正規化過的，拿去跟來源系統對帳會對不起來，而且找不到原因。下次匯入之後這一格就會有東西。</InfoNote>
                </span>
              </td>
              <td class="ops">
                <button class="btn small" type="button"
                        :disabled="!src.has_original || !!exportBusy"
                        @click="doExport(src, 'original')">原始檔</button>
                <button class="btn small" type="button"
                        :disabled="!src.row_count || !!exportBusy"
                        @click="doExport(src, 'excel')">Excel</button>
                <button class="btn small" type="button"
                        :disabled="!src.row_count || !!exportBusy"
                        @click="doExport(src, 'dump')">dump</button>
              </td>
            </tr>
            <tr>
              <td>網段配置表</td>
              <td class="num">{{ segCount ?? '—' }}</td>
              <td class="orig">
                <span class="muted">沒有<InfoNote>網段匯入沒有保留原始檔。要拿去改再匯回來，用右邊的 Excel：表頭跟匯入認得的一樣，改完直接在上面那列重匯。</InfoNote></span>
              </td>
              <td class="ops">
                <button class="btn small" type="button" disabled>原始檔</button>
                <button class="btn small" type="button"
                        :disabled="!segCount || segExporting" @click="exportSegments">
                  {{ segExporting ? '準備中…' : 'Excel' }}
                </button>
