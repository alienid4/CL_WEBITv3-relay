...
const CY_STYLE = [
  { selector: 'node', style: {
    ...
    'border-width': 2, color: '#e4efe9', 'text-wrap': 'wrap', 'text-max-width': '120px',
  } },
  { selector: 'node[health="ok"]', style: { 'background-color': 'rgba(47,214,172,.13)', 'border-color': '#2fd6ac' } },
  { selector: 'node[health="warn"]', style: { 'background-color': 'rgba(255,184,103,.15)', 'border-color': '#ffb867' } },
  { selector: 'node[health="err"]', style: { 'background-color': 'rgba(255,107,107,.15)', 'border-color': '#ff6b6b' } },
  ...
  { selector: '.blast', style: { 'border-color': '#7fb3ea', 'border-width': 4 } },
  { selector: 'edge.blast', style: { 'line-color': '#7fb3ea', 'target-arrow-color': '#7fb3ea', width: 4 } },
  { selector: 'node.sel', style: { 'border-color': '#4fe3c0', 'border-width': 5 } },
...
function selectNode(n: any) {
  ...
  selected.value = {
    ...
    health_source: sys?.health_source,
    hosts: sys?.hosts ?? [],
...
.title h1 { font-family: var(--disp); font-size: 24px; font-weight: 600; margin: 4px 0 0; color: #fff; letter-spacing: -.5px; }
...
.tb { font-family: inherit; font-size: 12.5px; font-weight: 600; padding: 7px 14px; border-radius: 9px;
  border: 1px solid var(--border-strong); background: rgba(255,255,255,.05); color: var(--ink-soft); cursor: pointer; }
.tb:hover:not(:disabled) { border-color: var(--brand); color: var(--brand); }
.tb:disabled { opacity: .4; cursor: not-allowed; }
.tb.add { color: #4fe3c0; border-color: rgba(47,214,172,.4); }
.tb.go { background: linear-gradient(135deg,#2fd6ac,#1e8a6f); color: #04120e; border: none; font-weight: 700; }

.glass { background: rgba(255,255,255,.035); border: 1px solid rgba(255,255,255,.07); border-radius: 18px; backdrop-filter: blur(10px); }
...
.frow .arrow { font-size: 12px; color: #4fe3c0; padding-bottom: 8px; font-weight: 700; }
...
.empty .et { font-family: var(--disp); font-size: 17px; color: #fff; margin: 10px 0 6px; }
...
.pname { font-size: 16px; font-weight: 700; color: #fff; display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
.pname.muted { color: var(--muted); font-size: 14px; }
.tag { font-size: 10.5px; padding: 2px 9px; border-radius: 20px; font-weight: 700; }
.tag.ok { background: rgba(47,214,172,.16); color: #4fe3c0; }
.tag.warn { background: rgba(255,184,103,.16); color: #ffb867; }
.tag.err { background: rgba(255,107,107,.16); color: #ff6b6b; }
.tag.spof { background: rgba(255,107,107,.16); color: #ff6b6b; }
.pmeta { font-size: 12px; color: var(--muted); margin-top: 4px; }
.tag.src { font-size: 9.5px; opacity: .85; }
.tag.src.derived { background: rgba(47,214,172,.16); color: #4fe3c0; }
.tag.src.manual { background: rgba(255,184,103,.16); color: #ffb867; }
.hostlink { color: inherit; text-decoration: none; border-bottom: 1px dotted rgba(38,168,137,.45); }
.hostlink:hover { color: #26a889; }
.hstate { font-size: 10px; margin-left: 8px; padding: 1px 6px; border-radius: 3px; }
.hstate.st-已納管 { background: rgba(47,214,172,.14); color: #4fe3c0; }
.hstate.st-未納管 { background: rgba(127,179,234,.16); color: #7fb3ea; }
.hstate.st-失聯 { background: rgba(255,107,107,.16); color: #ff8f8f; }
.psec { font-size: 10.5px; font-weight: 700; text-transform: uppercase; color: #7fa89b; letter-spacing: .5px; margin: 14px 0 6px; }
.pbig { font-family: var(--disp); font-size: 26px; font-weight: 700; color: #e4efe9; }
.pbig.hot { color: #7fb3ea; }
.plist { list-style: none; margin: 0; padding: 0; font-size: 13px; color: var(--ink-soft); }
.plist li { padding: 6px 0; border-bottom: 1px dashed rgba(255,255,255,.08); display: flex; align-items: center; gap: 8px; }
.plist li:last-child { border-bottom: none; }
.plist .dt { font-size: 11px; color: #6f8880; margin-left: auto; }
.plist .muted, .muted { color: #6f8880; }
.xdep { background: none; border: none; color: #ff6b6b; cursor: pointer; font-size: 12px; padding: 0 2px; opacity: .6; }
...
.delsys { margin-top: 16px; width: 100%; font-family: inherit; font-size: 12px; padding: 8px; border-radius: 8px;
  background: rgba(255,107,107,.1); border: 1px solid rgba(255,107,107,.3); color: #ff6b6b; cursor: pointer; }
...
.legend .sw.ok { background: rgba(47,214,172,.13); border: 1.6px solid #2fd6ac; }
...
.legend .ln.hot { border-top-color: #7fb3ea; }
</style>
