  ...
  <div class="topo">
    <div class="head">
      <div class="title">
        ...
        <h1>系統聯通圖 <InfoNote>點任一系統 → 圖上即時高亮它<b>停機會波及的整條上游鏈</b>（blast radius）。拖曳可移動節點、按自動佈局重新排列、依健康度上色。空白處點一下清除高亮。</InfoNote></h1>
      </div>
    </div>
...
.title .ey { font-family: var(--disp); font-size: 11px; letter-spacing: 3px; color: var(--brand-dark); text-transform: uppercase; }
...
.tb:hover:not(:disabled) { border-color: var(--brand); color: var(--brand-dark); }
.tb:disabled { opacity: .4; cursor: not-allowed; }
.tb.add { color: var(--brand-dark); border-color: rgba(0,145,66,.4); }
.tb.go { background: var(--brand); color: #fff; border: none; font-weight: 700; }
...
.frow .arrow { font-size: 12px; color: var(--brand-dark); padding-bottom: 8px; font-weight: 700; }
...
.tag.ok { background: rgba(0,145,66,.16); color: var(--brand-dark); }
.tag.warn { background: rgba(255,184,103,.16); color: var(--warn-text); }
.tag.err { background: rgba(255,107,107,.16); color: var(--bad); }
.tag.spof { background: rgba(255,107,107,.16); color: var(--bad); }
.pmeta { font-size: 12px; color: var(--muted); margin-top: 4px; }
.tag.src { font-size: 9.5px; opacity: .85; }
.tag.src.derived { background: rgba(0,145,66,.16); color: var(--brand-dark); }
.tag.src.manual { background: rgba(255,184,103,.16); color: var(--warn-text); }
.hostlink { color: inherit; text-decoration: none; border-bottom: 1px dotted rgba(0,145,66,.45); }
.hostlink:hover { color: var(--brand-dark); }
.hstate { font-size: 10px; margin-left: 8px; padding: 1px 6px; border-radius: 3px; }
.hstate.st-已納管 { background: rgba(0,145,66,.14); color: var(--brand-dark); }
.hstate.st-未納管 { background: rgba(127,179,234,.16); color: var(--ink-aux); }
.hstate.st-失聯 { background: rgba(255,107,107,.16); color: var(--bad); }
...
.xdep { background: none; border: none; color: var(--bad); cursor: pointer; font-size: 12px; padding: 0 2px; opacity: .6; }
.xdep:hover { opacity: 1; }
.impact { background: rgba(21,101,192,.16); color: var(--ink-aux); font-weight: 600; padding: 10px 12px; font-size: 12.5px; border-radius: 9px; margin-top: 14px; line-height: 1.5; }
.impact b { color: var(--ink-aux); }
...
.delsys { margin-top: 16px; width: 100%; font-family: inherit; font-size: 12px; padding: 8px; border-radius: 8px;
  background: rgba(255,107,107,.1); border: 1px solid rgba(255,107,107,.3); color: var(--bad); cursor: pointer; }
