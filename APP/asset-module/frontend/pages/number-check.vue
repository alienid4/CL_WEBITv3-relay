...
// 天條：只要是表格，每一欄都要能排（2026-09-20）
const numbers = computed(() => data.value?.numbers ?? [])
const { sortKey, sortDir, toggle, sorted } = useSort(numbers, '')
async function load() {
  ...
  <div class="page">
    ...
    <template v-else>
      ...
      <table class="t">
        <thead>
          <tr>
            <SortTh k="label" :active="sortKey" :dir="sortDir" @sort="toggle">數字</SortTh>
            <SortTh k="screen" :active="sortKey" :dir="sortDir" class="r" @sort="toggle">畫面值</SortTh>
            <SortTh k="recalc" :active="sortKey" :dir="sortDir" class="r" @sort="toggle">獨立重算值</SortTh>
            <SortTh k="diff" :active="sortKey" :dir="sortDir" class="r" @sort="toggle">差異</SortTh>
            <SortTh k="mismatch" :active="sortKey" :dir="sortDir" @sort="toggle">差異解釋</SortTh>
            <SortTh k="key" :active="sortKey" :dir="sortDir" @sort="toggle">是 0 的話</SortTh>
          </tr>
        </thead>
        <tbody>
          <template v-for="n in sorted" :key="n.key">
