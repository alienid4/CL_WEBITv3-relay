...
// 分批上傳的進度。使用者 2026-08-26：「一次匯入很多份，我要怎麼知道會不會當機、
// 等很久我不知道狀態」——原本整批塞進**一個** POST，畫面只有一句「匯入中…」，
// 400 份要等就是乾等，而且中途瀏覽器或反向代理一斷，人不知道到底進去了幾份。
// 改成每批 CHUNK 份送一次：① 每批回來就更新進度，人看得到在動
// ② 就算第 9 批失敗，前 8 批已經進去了（後端本來就是逐份 upsert，重跑同一批不會變兩筆）
const progress = ref<{ done: number; total: number; imported: number; failed: number } | null>(null)
...
async function onFiles(e: Event) {
  ...
  const acc = { imported: 0, failed: 0, auto_bound: 0, need_review: 0,
                errors: [] as any[], results: [] as any[] }
  progress.value = { done: 0, total: files.length, imported: 0, failed: 0 }
  try {
    for (let i = 0; i < files.length; i += UPLOAD_CHUNK) {
      const batch = files.slice(i, i + UPLOAD_CHUNK)
      const fd = new FormData()
      for (const f of batch) fd.append('files', f)
      let r: any
      try {
        r = await apiFetch<any>('/api/documents/import', { method: 'POST', body: fd })
      } catch (err: any) {
        // 這一批整批失敗（連線斷、逾時）。**不中止**——前面幾批已經進去了，
        // 後面幾批也該有機會；把這批的檔名逐一記成失敗，人才知道要重傳哪幾份。
        const d = err?.data?.detail
        const msg = (typeof d === 'string' ? d : d?.message) ?? '這批送不出去（連線中斷或逾時）'
        acc.failed += batch.length
        acc.errors.push(...batch.map((f) => ({ file_name: f.name, error: msg })))
        progress.value = { done: Math.min(i + batch.length, files.length), total: files.length,
                           imported: acc.imported, failed: acc.failed }
        continue
      }
      acc.imported += r.imported
      acc.failed += r.failed
      acc.auto_bound += r.auto_bound
      acc.need_review += r.need_review
      acc.errors.push(...(r.errors ?? []))
      acc.results.push(...(r.results ?? []))
      progress.value = { done: Math.min(i + batch.length, files.length), total: files.length,
                         imported: acc.imported, failed: acc.failed }
    }
    lastResult.value = acc
    showToast(
      `匯入 ${acc.imported} 份（自動綁定 ${acc.auto_bound}、待確認 ${acc.need_review}）` +
      (acc.failed ? `，${acc.failed} 份失敗` : ''),
      acc.failed ? 'warn' : 'success',
    )
    await load()
  } finally {
...
<template>
  <div>
    ...
    <div class="breadcrumb-bar"><span class="pin">📌</span> <b>單據檔案室</b>
      <InfoNote>既有的「主機及網路異動需求單」與「伺服器上線前檢查表」Word 檔（<b>.doc／.docx 都吃</b>）。抽<b>單據編號、日期、主機名、IP、勾選欄位、申請規格</b>並保存原檔。主機命名把 IP 編在裡面（<code>SECSVR195-059 ↔ 10.99.195.59</code>），所以檔名、內文、主機名三個來源可以互相對帳，<b>三方一致才自動綁定資產</b>。<b>抽出來的值不會自動寫進資產欄位</b>——規格值要人工確認過才算數（點任一列展開）。同一個 IP 有多張單時只有<b>最新那張算現行</b>（IP 會回收再分配，舊單描述的是別台機器）。含帳密的單，進資料庫的全文已遮罩；原始檔仍含帳密，<b>下載會留稽核紀錄</b>。<br><br><b>歷史檢查表不會變成系統裡的上線檢查表</b>：那些勾選是當年的狀態，轉成基線會立刻產生一堆過期的假 drift；基線只從今天以後在系統裡跑完的檢查表產生。</InfoNote>
    </div>
    ...
    <div class="card">
      <div class="bar-row">
        ...
        <div v-if="progress" class="prog">
          ...
          <span class="prog-txt">
            已處理 <b>{{ progress.done }}</b> / {{ progress.total }} 份
            （成功 {{ progress.imported }}<span v-if="progress.failed">、失敗 {{ progress.failed }}</span>）
            — 每 {{ UPLOAD_CHUNK }} 份送一批，可以放著等，中途離開已送出的那幾批仍會保留
          </span>
        </div>
...
<style scoped>
.prog { flex: 1; min-width: 260px; }
.prog-bar { height: 6px; background: var(--line); border-radius: 3px; overflow: hidden; }
.prog-bar i { display: block; height: 100%; background: var(--brand); transition: width 0.2s; }
.prog-txt { font-size: 11px; color: var(--muted); display: block; margin-top: 4px; line-height: 1.6; }
...
.warn-row { font-size: 11.5px; color: var(--warn-text); }
...
.hit b { color: var(--brand-dark); }
.snip { font-size: 10.5px; color: var(--brand-dark); margin-top: 4px; line-height: 1.6;
...
.tag.provision_form { border-color: var(--brand); color: var(--brand-dark); }
.st { font-size: 11.5px; white-space: nowrap; }
.st.auto { color: var(--brand-dark); }
.st.review { color: var(--warn-text); font-weight: 700; }
...
.chip.ok { border-color: var(--brand); color: var(--brand-dark); margin-top: 6px; }
...
.bd.dec { border-color: var(--warn, #d8a13a); color: var(--warn-text); }
.bd.pend { border-color: var(--warn, #d8a13a); color: var(--warn-text); }
...
.ph { font-size: 11px; color: var(--brand-dark); margin-bottom: 6px; }
...
.kv .appl { color: var(--brand-dark); }
.kv b.warn { color: var(--warn-text); }
.kv.done { color: var(--brand-dark); }
