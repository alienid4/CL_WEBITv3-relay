...
interface SysRow {
  ...
  total: number; rows?: number; by_env: Record<string, number>
}
interface SysOut {
  ...
  partition?: { managed: number; retired: number; offbook: number }; unmapped_active_cia?: number
  mapped_rows?: number; total_rows?: number
  mapping?: { reason: string | null; business_system_rows: number; assets_total: number;
    assets_with_api_id: number; sample_table_codes: string[]; sample_asset_codes: string[] } | null
...
interface CellAsset {
  ...
  serials?: string[]; row_count?: number
}
interface Drill {
  api_id: string; name: string | null; ap_department: string | null; ap_owner: string | null
  kind: string; env_filter: string | null; total: number; rows?: number
...
type TabKey = 'infra' | 'ap' | 'matrix' | 'phys' | 'vip' | 'onboard'
const TABS: { key: TabKey; label: string }[] = [
  ...
  // 入口（VIP）（2026-09-14）：從 CIA 的 VIP 欄反查後面是哪幾台、影響哪些系統。未經 F5 驗證。
  { key: 'vip', label: '入口（VIP）' },
  // OS × 環境的納管狀態（2026-09-16 使用者：「OS 為列、環境為欄，格子放納管」）。
  // 補佈納管按 OS 分批做、能不能動看環境，所以這個切法才排得出行程。
  { key: 'onboard', label: 'OS × 環境（納管）' },
]
...
// 匯出交叉表成 CSV（2026-09-19）：畫面看得到的那張機房×環境，一鍵拿去 Excel。
// 加 UTF-8 BOM，不然 Excel 開繁中會亂碼。
function csvCell(v: string | number) {
  const s = String(v)
  return /[",\n]/.test(s) ? '"' + s.replace(/"/g, '""') + '"' : s
}
function exportMatrixCsv() {
  const locs = matrixLocs.value
  const lines: string[] = []
  lines.push(['環境＼機房', ...locs, '合計'].map(csvCell).join(','))
  for (const env of matrixEnvs.value) {
    lines.push([env, ...locs.map(l => cellCount(l, env)), rowTotal(env)].map(csvCell).join(','))
  }
  lines.push(['合計', ...locs.map(l => colTotal(l)), matrixTotal.value].map(csvCell).join(','))
  const blob = new Blob(['﻿' + lines.join('\r\n')], { type: 'text/csv;charset=utf-8' })
  const url = URL.createObjectURL(blob)
  const a = document.createElement('a')
  a.href = url
  a.download = `機房x環境_${new Date().toISOString().slice(0, 10)}.csv`
  a.click()
  URL.revokeObjectURL(url)
  showToast('已匯出機房 × 環境交叉表', 'success')
}
...
async function loadSystems(_all = false) {
  if (tab.value === 'matrix' || tab.value === 'phys' || tab.value === 'vip') return   // 這幾頁不用系統×環境資料
...
watch(tab, async () => {
  drill.value = null
  if (tab.value !== 'matrix' && tab.value !== 'phys' && tab.value !== 'vip') await loadSystems(false)
})
...
<template>
  <div>
    ...
    <section v-if="tab === 'matrix' && matrixLocs.length" class="card">
      <div class="ck">機房 × 環境
        <button class="btn small csv" type="button" @click="exportMatrixCsv">匯出 CSV</button>
      </div>
      <div class="tblwrap">
        <table>
          <thead><tr>
    ...
    <VipEntries v-if="tab === 'vip'" />
    <OnboardMatrix v-if="tab === 'onboard'" />

    <section v-if="tab !== 'matrix' && tab !== 'phys' && tab !== 'vip' && sys" class="card">
      <div class="ck">
        ...
        <InfoNote>
          ...
          <br><br>「全庫＝在管＋退役＋帳外」：跟首頁「在管」、報表列印來源對照表同一套拆法，三塊加起來＝全庫。<b>帳外</b>（DY／vCenter／系統自動補）本來就沒有 AP ID，<b>退役</b>不用管，所以「沒填 AP ID」只算<b>在管</b>的——那才是要回頭補的。
        </InfoNote>
      </div>
      <p class="note">
        本頁涵蓋 <b>{{ n(sys.mapped) }}</b> 台<span v-if="sys.mapped_rows && sys.mapped_rows !== sys.mapped" class="rowsub">（{{ n(sys.mapped_rows) }} 筆登記）</span>　／
        <!-- 十項盤點第 5、6 條（2026-09-18）：「共 N 台」拆成在管＋退役＋帳外，每個可點；
             「對不上業務系統」只算在管卻沒填 AP ID 的（帳外本來就沒有 AP ID、退役不用管） -->
        <template v-if="sys.partition">
          全庫 <b>{{ n(sys.total_assets) }}</b> 台 ＝
          <NuxtLink to="/hosts">在管 {{ n(sys.partition.managed) }}</NuxtLink> ＋
          <NuxtLink :to="{ path: '/assets', query: { filter_field: 'asset_status', filter_value: '停用,報廢,閒置' } }"
                     title="這裡的『退役』＝有 CIA 序號、而且同一台的所有登記都標退役（逐台）。漏斗的『已退役』另外把「有退役也有使用中」的抽去「登記矛盾」，所以會比這個少。">退役 {{ n(sys.partition.retired) }}</NuxtLink> ＋
          <NuxtLink :to="{ path: '/assets', query: { source: 'offbook' } }">帳外 {{ n(sys.partition.offbook) }}</NuxtLink>　／
          <NuxtLink :class="{ warn: (sys.unmapped_active_cia ?? 0) > 0 }"
                    :to="{ path: '/assets', query: { filter_field: 'api_id', filter_value: '', source: 'registered' } }">在管卻沒填 AP ID <b>{{ n(sys.unmapped_active_cia ?? 0) }}</b> 台</NuxtLink>
          　·　{{ n(sys.system_count) }} 個系統
        </template>
        <template v-else>
          <span :class="{ warn: sys.unmapped > 0 }">全庫對不上業務系統 <b>{{ n(sys.unmapped) }}</b> 台</span>　／
          共 {{ n(sys.total_assets) }} 台　·　{{ n(sys.system_count) }} 個系統
        </template>
        <InfoNote><b>「台」是機器台數，「筆」是 CIA 登記筆數</b>（2026-09-14 起）。<br><br>CIA 清冊是<b>每個服務／VIP 一筆</b>：一台 VM 掛 3 個 VIP、5 個系統就登記好幾筆。所以這頁改成<b>主機名＋IP 相同算同一台</b>，同一台在同一個系統底下只算一次；筆數另外列在括號裡，<b>資料一筆都沒少</b>。<br><br>同一台掛兩個系統，兩個系統各算一台，但全庫合計只算一台。沒填主機名或 IP 的無法判斷是不是同一台，各算一台（所以台數是上限）。</InfoNote>
      </p>
      <!-- 一台都對不上業務系統：講出原因＋兩邊代碼樣本（2026-09-14 公司機涵蓋 0 台） -->
      <div v-if="sys.mapping && sys.mapping.reason" class="mapwarn">
        <b>為什麼是 0 台：</b>{{ sys.mapping.reason }}
        <div class="mapsub">
          業務系統對照表 {{ n(sys.mapping.business_system_rows) }} 筆，代碼例如：<code>{{ sys.mapping.sample_table_codes.join('、') || '（空）' }}</code><br>
          資產 {{ n(sys.mapping.assets_total) }} 筆，其中有填 AP ID 的 {{ n(sys.mapping.assets_with_api_id) }} 筆，代碼例如：<code>{{ sys.mapping.sample_asset_codes.join('、') || '（全空白）' }}</code>
        </div>
      </div>
            ...
            <template v-for="s in grp.rows" :key="s.api_id">
              <tr :class="{ opened: drillOpenOn(s.api_id) }">
                ...
                <td class="tot">
                  ...
                  <span v-if="s.rows && s.rows !== s.total" class="rowsub"
                        title="同一台登記多筆（不同 VIP／服務），台數只算一次">（{{ n(s.rows) }} 筆）</span>
                </td>
                <td class="ops">
                ...
                <td :colspan="sys.envs.length + 6">
                  <div class="dwrap">
                    ...
                    <div class="dblock wide">
                      <div class="dk">
                        機房 × 環境
                        <span class="basis">全部 {{ n(drill?.total) }} 台<template v-if="drill?.rows && drill.rows !== drill.total">（{{ n(drill.rows) }} 筆登記）</template></span>
                                ...
                                <table class="inner">
                                  ...
                                  <tbody>
                                    <tr v-for="a in sortedLeItems" :key="a.asset_serial">
                                      ...
                                      <td class="rowh mono" :title="(a.serials || []).join('、')">{{ a.row_count ?? 1 }}</td>
                                    </tr>
                                  </tbody>
                                </table>
                              </td>
...
<style scoped>
.mapwarn { margin: 8px 0; padding: 9px 12px; border-radius: 6px; background: var(--warn-soft);
           color: var(--warn-text); border: 1px solid rgba(176,106,0,.3); font-size: 13px; }
.mapwarn .mapsub { margin-top: 5px; font-size: 12px; line-height: 1.7; }
.rowsub { font-size: 11px; color: var(--ink-soft); margin-left: 3px; font-weight: normal; white-space: nowrap; }
...
.btn.csv { margin-left: 10px; font-weight: 400; }
</style>
