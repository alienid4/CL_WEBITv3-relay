...
<template>
  <div class="page">
    <header class="hd">
      <div>
        <h1>操作紀錄 <InfoNote>只記<b>會改東西的請求</b>（新增／修改／刪除／匯入）與<b>登入、登出、登入失敗</b>。純查看（GET）刻意不記——那佔九成以上流量而稽核價值低，全記會把真正要查的東西淹掉。<b>不記你輸入的內容</b>（可能含密碼與真實資料），只記「誰、什麼時候、哪一支功能、結果如何、從哪個 IP」。紀錄保留 <b>{{ data?.retain_days ?? '—' }}</b> 天。</InfoNote></h1>
    ...
    <section v-if="summary" class="who">
      ...
      <p v-else class="empty-inline">
        這段期間沒有任何異動紀錄
        <InfoNote><b>這不代表沒人用</b>——只看報表不改東西的人不會留下紀錄（純查看刻意不記）。要看「現在誰開著」請點左上角的線上人數。</InfoNote>
      </p>
    </section>
