...
def load_segments(path: Path) -> list[str]:
    ...
    for raw in path.read_text(encoding="utf-8").splitlines():
        # 行尾註解也要吃（10.92.198.0/24  # 板橋）：人手動維護這份清單時很自然
        # 會在後面標機房，原本那樣寫會讓整行解析失敗被略過，而且只印一行警告，
        # 很容易變成「清單看起來滿滿的、實際掃不到幾段」。
...
def main() -> None:
    ...
    # 只要 3389/445 其中一個在就掃得到 Windows，不必兩個都要。
    # 原本只檢查 3389，會在 ports=22,445 這種合法設定下印出假警報——
    # 假警報比沒有警報更糟，看久了真的警告也會被當雜訊略過。
    win_blind = 3389 not in ports and 445 not in ports
    print(f"探測埠： {','.join(str(p) for p in ports)}"
          f"{'（未含 3389 也未含 445，Windows 可能全部無回應）' if win_blind else ''}")
