...
BONUS_ALIVE = 20
BONUS_MANAGED = 30
#: 2026-09-15 使用者定：設備「已收集」（SAN 收集／離線匯入）跟已納管同分 +30。
#: 兩者都是「我們手上有這台機器端的資料」，只是取得方式不同。
BONUS_COLLECTED = 30
PING_ONLY = 10
...
def score_one(kind: str, has: dict[str, bool], alive: bool, managed: bool,
              collected: bool = False) -> dict:
    """單台的分數與拆解。純函式，測試直接打它。"""
    ...
    if managed:
        ...
        bonus, evidence = BONUS_COLLECTED, "collected"
    elif alive:
...
def compute_all(conn) -> dict[str, dict]:
    ...
    except Exception:  # noqa: BLE001 - 舊 DB 沒這張表
        collected_ips = set()
    ...
    for a in conn.execute(
        ...
        r = score_one(kind, has, seen_by_us or has["dynassets"], a["collect_ok"] == 1,
                      collected=(a["ip"] or "").strip() in collected_ips)
...
def explain(r: dict) -> str:
    ...
    elif r["evidence"] == "collected":
        ev = f"＋已收集(設備) {BONUS_COLLECTED}"
...
def distribution(conn) -> dict:
    ...
    # [口徑] 2026-09-20 公司驗收：畫面寫「7,923 台非退役資產」，其實是「不重複的資產序號數」
    # （逐筆、含帳外、序號空白會被併成一筆）。分數是「一筆登記有幾個來源佐證」，本來就是逐筆的，
    # 所以這裡**不改分數**，改成把兩個數字都給畫面：筆數（分數分布的母體）與台數（正典 machine_key）。
    import system_stats as _ss

    machines = {_ss.machine_key(r.get("hostname"), r.get("ip"), r.get("asset_serial"))
                for r in allr.values()}
    return {
        "total": len(allr),                 # 筆（分數分布的母體）
        "machines": len(machines),          # 台（machine_key 去重）
        "by_score": [
            {"score": s, "count": n,
