...
#: 匯出時要不要加密（2026-09-18 使用者：「我自己都會忘，所以我要拿掉，以後功能關閉即可」）。
#: 取代 SEC5（2026-09-08）的「一律加密、不留不加密選項」。**預設關**；打開就恢復原本行為，
#: 加密程式碼與金鑰都保留。關閉時每次匯出記一筆操作紀錄（誰、何時、格式、大小）。
ENCRYPT_SETTING = "export_encrypt_enabled"


def encryption_enabled(conn) -> bool:
    from db import get_setting
    return str(get_setting(conn, ENCRYPT_SETTING, "0")).strip() == "1"


def set_encryption_enabled(conn, enabled: bool) -> bool:
    from db import set_setting
    set_setting(conn, ENCRYPT_SETTING, "1" if enabled else "0")
    return enabled
