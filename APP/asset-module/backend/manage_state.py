...
ALL_STATES = (UNREGISTERED, NOT_ONBOARDED, ONBOARDED, COLLECTED, LOST, RETIRED, EXEMPT)
#: 「要人處理」的狀態。已納管、已收集都不算——統計「有問題幾台」一律用這份，不要各處自己寫「!= 已納管」
NEEDS_ACTION_STATES = (UNREGISTERED, NOT_ONBOARDED, LOST)
...
def classify(registered: bool, seen_in_scan: bool, collect_ok: int | None,
             collected: bool = False, retired: bool = False, exempt: bool = False) -> str:
...
def summarize(conn) -> dict:
    ...
    for r in hw:
        ...
        state = classify(True, seen, r["collect_ok"], collected=(r["ip"] in collected_ips),
                         retired=((r["asset_status"] or "").strip() in RETIRED_STATUS),
                         exempt=(r["asset_serial"] in exempt_serials))
        counts[state] += 1
        items.append({
            ...
            "os_type": _os_type(r["os"], None),   # 已納管的有 os 字串，不需靠埠推
        })
...
def _ssh_runner(key_path: str, account: str = DEFAULT_COLLECT_ACCOUNT, timeout: int = 10):
    ...
    def run(host: str, cmd: str) -> str:
        r = subprocess.run(
            ["ssh", "-i", key_path, "-o", "BatchMode=yes", *_ssh_hostkey_opts(),
...
def composition(conn) -> dict:
    ...
    def _is_offbook(serial) -> bool:
        ...
        # 跟 system_report.is_off_book 同一條規則，但 composition 不 import 它避免循環。
        return str(serial or "").startswith(("DYN-", "VC-", "AUTO-"))

    import normalize

    by_platform, by_env, by_status, by_virt = {}, {}, {}, {}
    by_os, by_model, by_location = {}, {}, {}
    registered_active = 0   # 有效資產中「正式登記」的（排除帳外）——對外頭條用這個
    ...
    return {
        ...
        "registered_total": registered_active,
        "off_book_total": off_book_active,        # 掃到/vCenter 有、但沒登記的（未登記，另外顯示）
