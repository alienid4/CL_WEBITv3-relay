...
def list_assets(
    ...
    _check_sort("hostname" if sort_health else sort_by, order,
    ...
    query += f" ORDER BY {'hostname' if sort_health else sort_by} {order.upper()}"
...
def import_business_systems(
    ...
    if not (file.filename or "").lower().endswith(".xlsx"):
        raise HTTPException(400, "只接受 .xlsx（第一列是表頭）")
    with tempfile.NamedTemporaryFile(suffix=".xlsx", delete=False) as tmp:
        tmp.write(file.file.read())
        tmp_path = Path(tmp.name)
    try:
        try:
            return business_system.import_xlsx(tmp_path, conn)
    ...
    if scanned["groups"]:
        groups.append({
            ...
            "items": [{
                ...
                "to": f"/search?q={kw}&table={g['table']}",
            } for g in scanned["groups"][:limit]],
            "more": len(scanned["groups"]) > limit,
            "more_to": f"/search?q={kw}",
        })
