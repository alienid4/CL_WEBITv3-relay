...
def dashboard_stats(
    ...
    # [口徑] 2026-09-20 公司驗收：畫面寫「台」、這裡卻是逐筆，於是出現「搜不到 3,417 > 在管 3,374 台」。
    # 一致／搜不到改逐台（正典 machine_key，同一台只要有一筆掃得到就算掃得到），
    # 筆數仍一併回傳給畫面當副標——兩個都給，人才知道自己在看哪一種。
    import system_stats as _ss

    _machines: dict[str, bool] = {}
    for r in reg_rows:
        k = _ss.machine_key(r["hostname"], r["ip"], r["asset_serial"])
        _machines[k] = _machines.get(k, False) or _row_in_keys(r, scan_ips, scan_hostnames)
    total_ica_machines = len(_machines)
    total_overlap_machines = sum(1 for v in _machines.values() if v)
    ...
    return {
        ...
        # 別拿上面那組環境篩選過的去算
        "total_ica_count": total_ica_registered,          # 筆（副標用）
        "total_overlap_count": total_overlap_count,        # 筆
        # 檯面數字用這兩個：逐台，跟頭條「在管 N 台」同一把尺
        "total_ica_machines": total_ica_machines,
        "total_overlap_machines": total_overlap_machines,
...
def asset_facets(
    ...
    # [口徑] 2026-09-20 公司驗收：籌碼上的數字是**筆**而且**含退役**，
    # 加起來 8,085 跟同一頁分頁寫的「硬體 7,923」差 162（剛好是退役筆數），
    # 「AIX 16」也對不起漏斗的「AIX 8 台」。改成逐台（machine_key）並排除退役，
    # 同時把筆數一起回傳，畫面要講得出自己在數什麼。
    import manage_state as _ms
    import system_stats as _ss

    meta = {r["asset_serial"]: r for r in conn.execute(
        "SELECT asset_serial, hostname, ip, asset_status FROM hardware")}

    cls = asset_classify.classify_all(conn)
    by_platform: dict[str, set] = {}
    by_os: dict[str, set] = {}
    by_detail: dict[str, set] = {}
    by_role: dict[str, set] = {}
    rows_os: dict[str, int] = {}
    retired_rows = 0
    for serial, info in cls.items():
        m = meta.get(serial)
        if m is not None and (m["asset_status"] or "").strip() in _ms.RETIRED_STATUS:
            retired_rows += 1
            continue                      # 退役不進篩選數字（跟同頁的「硬體 N 筆」同母體）
        mk = (_ss.machine_key(m["hostname"], m["ip"], serial) if m is not None else serial)
        o = info.get("os_type") or pipeline.OS_UNSET
        by_os.setdefault(o, set()).add(mk)
        rows_os[o] = rows_os.get(o, 0) + 1
        by_platform.setdefault(info.get("platform") or "未知", set()).add(mk)
        if info.get("os_detail"):
            by_detail.setdefault(info["os_detail"], set()).add(mk)
        for r in info.get("roles") or ["unknown"]:
            by_role.setdefault(r, set()).add(mk)
    # 同一台的多筆登記若 OS 不一致，會同時被算進兩類 → 籌碼加總會比母體多幾台。
    # 那是實情不是 bug，但畫面要講出來（不講的話人只會覺得數字在騙他）。
    _seen: dict[str, int] = {}
    for mks in by_os.values():
        for mk in mks:
            _seen[mk] = _seen.get(mk, 0) + 1
    os_overlap_machines = sum(1 for n in _seen.values() if n > 1)
    _n = lambda d: {k: len(v) for k, v in d.items()}      # noqa: E731 - 集合換成台數
    by_platform, by_os, by_detail, by_role = _n(by_platform), _n(by_os), _n(by_detail), _n(by_role)
    ...
    return {"os_type": by_os, "os_order": list(pipeline.OS_ORDER),
            ...
            "platform": by_platform, "role": by_role,
            # total 維持「筆」（舊行為、給既有畫面用），另外給台數與母體說明
            "total": len(cls), "total_rows": len(cls) - retired_rows,
            "total_machines": len({_ss.machine_key(m["hostname"], m["ip"], s)
                                   for s, m in meta.items()
                                   if (m["asset_status"] or "").strip() not in _ms.RETIRED_STATUS}),
            "retired_rows": retired_rows,
            "os_overlap_machines": os_overlap_machines,
            "basis": "非退役（含帳外）；籌碼數字為台數，machine_key 去重"}
...
def _collect_trace_of(conn, asset_serial: str) -> dict | None:
    """這台最近一次收集的 SSH 事實。沒有紀錄回 None（畫面顯示「還沒收過」）。

    ⚠️ 這一塊跟其他「收不到的理由」不同層：其他那些是**收集器的推論**
    （「AIX 不可能零 fileset，所以是收集失敗」——對，但那是推論）；
    這裡是**離開碼與 stderr 的原文**，是事實。兩者畫面上要分開標示。
    """
    import json
    ...
    except Exception:  # noqa: BLE001 - 舊 DB 還沒有這張表時不要讓整頁掛掉
        return None
    if row is None:
        return None
    out = dict(row)
    try:
        out["samples"] = json.loads(out.get("samples") or "[]")
    except (ValueError, TypeError):
        out["samples"] = []
    out.update(_ms.trace_verdict(row))
    return out



def _norm_serial(v) -> str:
    """序號比對用的正規化：去掉大小寫、空白、連字號與廠商前綴。

    **不是為了合併成一個值**，是為了判斷「這兩個到底算不算同一個」。
    AIX 的 `uname -u` 給的是 systemid（帶 `IBM,02` 之類的前綴），
    `prtconf` 的 Machine Serial Number 給的是裸序號——
    那**本來就是兩種寫法**，不正規化就會把它們誤判成互相矛盾。
    """
    t = str(v or "").strip().upper().replace(" ", "").replace("-", "")
    if "," in t:
        t = t.split(",", 1)[1]
    return t
...
def _same_serial(a: str, b: str) -> bool:
    """這兩個序號算不算同一台。

    ⚠️ 不能直接比字串。同一台機器在不同指令下**本來就有不同寫法**：
    AIX 的 `uname -u` 給 systemid（`IBM,0206781A091`——`IBM,` 是廠商、`02` 是型別碼），
    `prtconf` 的 Machine Serial Number 給裸序號（`06781A091`）。
    直接比會把「同一台的兩種寫法」誤報成矛盾——那是假警報，而且會讓真正的矛盾被淹掉。

    判準：正規化後**一個是另一個的結尾**就算同一台（處理各種前綴碼），
    但短字串要完全相同才算——`123` 是一堆序號的結尾，那樣比會把不同機器判成同一台。
    """
    x, y = _norm_serial(a), _norm_serial(b)
    if not x or not y:
        return False
    if x == y:
        return True
    short, long_ = (x, y) if len(x) <= len(y) else (y, x)
    return len(short) >= 6 and long_.endswith(short)


def _serial_view(hw, host_spec) -> dict:
    """把兩個來源的序號攤開給畫面看：值、出處、有沒有互相矛盾。

    ⚠️ **不自動挑一個當正解。** 兩個來源各自有出處，挑一個等於替使用者決定
    要相信誰，而且挑錯的時候畫面上完全看不出來。

    三種下場要分得開：

    ======================  ==================================================
    兩邊都有、正規化後相同   一致，顯示值＋兩個出處
    兩邊都有、正規化後不同   **矛盾**，兩個都列出來給人判（不挑邊）
    只有一邊有              顯示那一邊，並講明另一邊為什麼沒有
    ======================  ==================================================
    """
    facts = (hw.get("hw_serial") or "").strip() if hw else ""
    spec = ""
    if host_spec:
        spec = (host_spec.get("serial") or "").strip()
    sources = []
    if facts:
        sources.append({"value": facts, "from": "主機事實收集（hardware.hw_serial）"})
    if spec:
        sources.append({"value": spec, "from": "硬體規格收集（host_spec.serial）"})

    if not sources:
        return {"value": None, "sources": [], "conflict": False,
                "note": "兩個來源都沒有收到序號——不是這台沒有序號，是沒收到。"}
    conflict = len(sources) == 2 and not _same_serial(facts, spec)
    note = None
    if conflict:
        note = ("兩個收集器拿到「不一樣」的序號，系統不替你挑邊："
                f"主機事實收集拿到 {facts}，硬體規格收集拿到 {spec}。"
                "兩個都列在下面，請以實機標籤為準。")
    elif len(sources) == 1:
        缺的 = "硬體規格收集" if facts else "主機事實收集"
        note = (f"這個值來自{sources[0]['from']}；{缺的}那一邊沒有收到。"
                "兩邊各跑各的指令，少一邊不代表這個值有問題，"
                "但也代表沒有第二個來源可以對帳。")
    return {"value": sources[0]["value"], "sources": sources,
            "conflict": conflict, "note": note}


def _virt_kind_of(hw) -> str | None:
    """這台要顯示 LPAR 還是 VM。uname -L 目前沒有獨立欄位，B-24 收到後從這裡接。"""
    import manage_state as _ms
    return _ms.virt_kind(hw.get("os"), hw.get("is_vm"), hw.get("device_model"))
...
def asset_detail(
    ...
    # 納管紀錄（2026-09-22 使用者：「AIX 我已經納管多次，每次狀態我看都是未納管」）。
    #
    # 他納管過多次，資產頁卻**什麼都看不到**——因為納管只寫 onboard_audit，
    # 而那張表在這一頁完全沒有呈現。上面那個 history 讀的是 comparison_result
    # （**差異偵測結果**，不是操作紀錄），名字叫「歷史時間軸」很容易被當成
    # 「這台發生過什麼」，實際上它只在收集成功、有東西可比對之後才會有資料。
    #
    # 結果是：納管失敗 → 收不到資料 → 沒有比對 → 時間軸 0 →
    # 使用者看到的是「系統根本沒收到我的動作」，於是再納管一次。做了三次才來問。
    #
    # 資料本來就在，只是沒拿出來。**失敗的也要列**——只列成功的話，
    # 「試過三次都失敗」跟「從來沒試過」在畫面上仍然長得一模一樣。
    onboard_history = [dict(r) for r in conn.execute(
        "SELECT created_at, platform, trigger, triggered_by, ok, stage, message "
        "FROM onboard_audit WHERE target_ip = ? ORDER BY id DESC LIMIT 50",
        (hardware["ip"],))]
    ...
    return {
        ...
        # 虛擬化怎麼稱呼**由後端統一判**（manage_state.virt_kind）。前端自己判
        # 就是第二份定義，兩邊遲早不一致 -- 平台判定今天才剛因為手抄本吃過虧。
        "virt_kind": _virt_kind_of(hw),
        # 序號有兩個來源（facts 與 host_spec），以前各寫各的、沒有人對帳。
        # 攤開給畫面看：值、出處、有沒有互相矛盾（2026-09-23）。
        "serial_view": _serial_view(hw, host_spec),
        # 收集這一跳的 SSH 事實（離開碼／stderr／指令原文）＋一句從離開碼直接讀出的結論。
        # 使用者的規矩：診斷要做進畫面，不要叫他跑指令；換主機還會再遇到，要能重現。
        "collect_trace": _collect_trace_of(conn, asset_serial),
...
def onboard_endpoint(
    ...
    if result.ok:
        try:
            ...
            if serial_row:
                # ⚠️ 2026-09-22：這裡原本**把回傳值丟掉**。
                # 使用者原話：「AIX 我可以連線，畫面也顯示納管成功，納管後我沒看到
                # 納管後的任何資訊跟畫面呈現，例如帳號、軟體、硬體資訊等等」。
                # 原因就在這行：帳號確實建好了（result.ok=True → 畫面「納管成功」），
                # 但緊接著的收集拿到 0 個欄位，而**沒有人看那個回傳值**——
                # post_note 只在丟例外時才寫，「跑完了但什麼都沒收到」不算例外。
                # 於是「連得上、報成功、資料全空」三件事同時成立，
                # 使用者完全分不出是納管壞了還是這台真的沒有帳號。
                collected = manage_state.collect_facts_into_assets(
                    conn, only_serial=serial_row["asset_serial"])
                if collected.get("updated", 0) < 1:
                    os_row = conn.execute(
                        "SELECT os FROM hardware WHERE asset_serial = ?",
                        (serial_row["asset_serial"],)).fetchone()
                    base = (collected["failed"][0]["error"]
                            if collected.get("failed") else None)
                    collect_error = _collect_diagnosis(
                        conn, body.ip, body.platform,
                        os_row["os"] if os_row else None,
                        collected.get("updated", 0), base)
                    # 明確寫回「收不到」，不要停在上一次的舊狀態
                    conn.execute(
                        "UPDATE hardware SET collect_ok = 0, collect_checked_at = ?, "
                        "collect_error = ? WHERE asset_serial = ?",
                        (datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                         collect_error[:500], serial_row["asset_serial"]))
                    # 納管紀錄也要留一筆，資產頁才看得到「這次是收集這步敗的」
                    _log_onboard(conn, hardware_ip=body.ip, platform=body.platform,
                                 who=session["username"] if session else None,
                                 ok=False, stage="collect", message=collect_error,
                                 trigger="manual", login_user=body.username)
    ...
    # 納管的目的是「之後收得到資料」。帳號建好但一個欄位都收不到，
    # 對使用者而言就是失敗——顯示成功會讓他以為完成了，然後過幾天才發現什麼都沒有。
    # （使用者 2026-09-22 就是這樣做了三次。）
    # 所以這裡把兩件事分開回報：script_ok=腳本有沒有跑成，ok=這次納管有沒有達到目的。
    if result.ok and collect_error:
        return {
            "ok": False, "script_ok": True, "stage": "collect",
            "message": ("納管腳本執行成功（收集帳號已建立），"
                        f"但**納管後收不到任何資料**。{collect_error}"),
            "output": result.output,
        }
    message = result.message + (post_note or "")
    return {"ok": result.ok, "script_ok": result.ok, "stage": result.stage,
...
def _collect_diagnosis(conn: sqlite3.Connection, ip: str, sent_platform: str,
                       os_val: str | None, updated: int, base_error: str | None) -> str:
    """收集失敗時，把「為什麼」直接組成一句人看得懂的話寫進納管紀錄。

    2026-09-22 使用者：「我不希望你提供什麼樣的資料，我會繼續錄影給你看」
    「換到正式區，如果遇到相同的問題才好排除」——**他手上只有畫面**，
    不該被叫去跑 SQL 查 hardware.os 或 onboard_audit.platform。
    所以這裡把四件事一次講完：送出平台、判定平台、實際平台、收到幾個欄位。

    AIX 被判成 Linux 的症狀是「連得上、每個欄位都空」，跟權限不足長得一模一樣。
    把 `uname -s` 的實際結果擺在判定平台旁邊，一眼就看得出是不是判錯。
    """
    import manage_state

    row = conn.execute("SELECT asset_serial FROM hardware WHERE ip = ?", (ip,)).fetchone()
    guessed = manage_state.collect_platform_of(
        conn, ip, os_val, asset_serial=row["asset_serial"] if row else None)
    actual = manage_state.probe_uname(ip)
    parts = [f"送出平台={sent_platform}", f"判定平台={guessed}",
             f"實際 uname -s={actual or '問不到'}", f"收到欄位={updated} 個"]
    msg = "收集失敗：" + "、".join(parts)
    if not (os_val or "").strip():
        msg += "。這台的 OS 欄位是空的，平台是**用開放埠猜的**"
    if actual and guessed and actual.lower() not in guessed.lower():
        msg += (f"。**判定平台與實際不符——拿了 {guessed} 的指令去問 {actual}，"
                f"所以每個欄位都空**。請在資產資料把 OS 填成 {actual}")
    elif updated == 0 and not actual:
        msg += "。連 uname -s 都問不到，可能是收集帳號連不進去（而不是平台判錯）"
    if base_error:
        msg += f"。原始錯誤：{base_error}"
    return msg


def _log_onboard(conn: sqlite3.Connection, *, hardware_ip: str, platform: str,
                 who: str | None, ok: bool, stage: str, message: str,
                 trigger: str = "verify", login_user: str | None = None) -> None:
    """把納管動作寫進 onboard_audit——**成功失敗都寫**。

    2026-09-22 使用者：「AIX 我已經納管多次，每次狀態我看都是未納管」。
    只寫成功的話，「試過三次都失敗」跟「從來沒試過」在畫面上一模一樣，
    人就只能一直重試。失敗要寫得出**原因**，不是只寫「失敗」。

    寫紀錄失敗不可以影響主流程——紀錄是為了事後看得懂，不是業務本身。
    """
    try:
        conn.execute(
            "INSERT INTO onboard_audit (target_ip, platform, login_user, trigger, "
            "triggered_by, ok, stage, message, output) VALUES (?,?,?,?,?,?,?,?,?)",
            (hardware_ip, platform, login_user, trigger, who,
             1 if ok else 0, stage, message[:1000], None))
    except sqlite3.Error:
        pass
...
def onboard_verify(
    ...
    #
    # ⚠️ 2026-09-22 修正：這裡原本在**收集之前**就無條件 mark_collect_ok(=1)，
    # 而且失敗那條 return 路徑沒有收回。症狀是「按驗證當下顯示已納管，
    # 隔天排程探測又打回未納管」——使用者以為納管會自己失效。
    # 現在改成：**先收集，成功才寫 1；失敗明確寫 0 並留下原因**。
    ...
    if result["updated"] >= 1:
        ...
        manage_state.mark_collect_ok(conn, asset_serial, now)
        _log_onboard(conn, hardware_ip=body.ip, platform=body.platform,
                     who=session["username"], ok=True, stage="verify",
                     message=f"本機執行後驗證成功：收到 {result['updated']} 個欄位")
        conn.commit()
        return {
    ...
    # 失敗：**明確寫回 0 並留原因**，不要停在上一次的舊狀態讓人以為還好好的
    base_err = result["failed"][0]["error"] if result["failed"] else None
    error = _collect_diagnosis(conn, body.ip, body.platform,
                               row["os"] if row else None, result["updated"], base_err)
    conn.execute(
        "UPDATE hardware SET collect_ok = 0, collect_checked_at = ?, collect_error = ? "
        "WHERE asset_serial = ?", (now, error[:500], asset_serial))
    _log_onboard(conn, hardware_ip=body.ip, platform=body.platform,
                 who=session["username"], ok=False, stage="verify", message=error)
    conn.commit()
...
@app.get("/api/reports/scan-gaps")
def scan_gaps_endpoint(
    only_in_scope: bool = True,
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """掃不到的網段清單：分「從沒掃過／掃描失敗／掃通了整段沒回應／最近一次沒掃到」，附那段登記幾台。"""
    import scan_gaps

    return scan_gaps.report(conn, only_in_scope=only_in_scope)


class ScannerVisibilityBody(BaseModel):
    environments: list[str]


@app.put("/api/reports/scan-gaps/visibility")
def scan_gaps_visibility_endpoint(
    body: ScannerVisibilityBody,
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """這台掃描機看得到哪些環境。設了之後，看不到的環境掃不到＝預期中的事，不列進申請名單。"""
    import scan_gaps

    return {"environments": scan_gaps.set_visible_environments(conn, body.environments)}


class SegmentNoteBody(BaseModel):
    cidr: str
    status: str = ""                 # unused／policy／pending／watch，空＝清除
    expected_hosts: int | None = None
    note: str = ""


@app.put("/api/reports/scan-gaps/note")
def scan_gaps_note_endpoint(
    body: SegmentNoteBody,
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """一段的人工標註：目前沒在使用／政策不掃／等開通／盯著＋預計台數＋備註。

    標註只補人的判斷，不覆蓋掃描事實；標了「沒在使用／政策不掃」的預設就不顯示了。
    """
    import scan_gaps

    try:
        return scan_gaps.set_note(conn, body.cidr, body.status, body.expected_hosts, body.note,
                                  session["username"] if session else None)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc


@app.get("/api/reports/scan-gaps/export")
def scan_gaps_export_endpoint(
    only_in_scope: bool = True,
    q: str = "",
    environment: str = "",
    location: str = "",
    ...
    """同一份資料匯成 CSV（拿去當防火牆申請的附件）。

    帶畫面上的搜尋／篩選條件——匯出要等於畫面看到的那幾列（所見即所得），
    不然人篩完再匯出，拿到的卻是全部，送出去就錯了。
    """
    import scan_gaps

    data = scan_gaps.report(conn, only_in_scope=only_in_scope)
    data = {**data, "rows": scan_gaps.filter_rows(
        data["rows"], q=q, environment=environment, location=location, kind=kind,
        only_registered=only_registered, include_expected=include_expected)}
    csv = scan_gaps.to_csv(data)
    return Response(
        content="﻿" + csv,          # Excel 沒有 BOM 會把中文顯示成亂碼
        media_type="text/csv; charset=utf-8",
        headers={"Content-Disposition": 'attachment; filename="scan_gaps.csv"'},
    )
...
@app.get("/api/arch/ocp")
def arch_ocp(
    session: sqlite3.Row = Depends(require_auth), conn: sqlite3.Connection = Depends(get_db)
):
    """OCP 叢集架構圖：叢集 → 節點，附每個節點的納管狀態與叢集層風險。"""
    import arch

    return arch.ocp_topology(conn)


@app.get("/api/arch/ocp/siblings")
def arch_ocp_siblings(
    asset_serial: str,
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """這台的兄弟節點（同一座 OCP 叢集）——出事時要一眼看出關聯。"""
    import arch

    return arch.ocp_siblings(conn, asset_serial)


class OcpClusterBody(BaseModel):
    asset_serial: str
    cluster: str = ""          # 空＝清除人工指定，回到自動判斷
    reason: str = ""


@app.put("/api/arch/ocp/cluster")
def set_ocp_cluster(
    body: OcpClusterBody,
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """人工指定某一台屬於哪座 OCP 叢集（判不出來的由人自己換群組）。指定的是整台。"""
    import ocp_cluster

    try:
        return ocp_cluster.set_cluster(conn, body.asset_serial, body.cluster, body.reason,
                                       session["username"] if session else None)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc


class OcpRoleBody(BaseModel):
    asset_serial: str
    role: str = ""          # 空字串＝清除指定、回到自動判斷
    reason: str = ""


@app.put("/api/arch/ocp/role")
def set_ocp_role(
    body: OcpRoleBody,
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """人工指定某一台的 OCP 角色（2026-09-21 使用者：「我要怎麼定義角色」）。

    自動判斷只看用途／主機名的關鍵字，寫不出來就卡在「未標示」；
    這裡讓人訂正，指定值優先，改的是整台（跟指定叢集同一套規矩）。
    """
    import ocp_cluster

    try:
        return ocp_cluster.set_role(conn, body.asset_serial, body.role, body.reason,
                                    session["username"] if session else None)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc


# ===== 弱點彙總（CL_Patch）共享儲存（2026-09-21）=====
# 使用者：「我只是要轉移到系統上，變系統版不是單機版」。
# 那支工具的分析邏輯一行不動，只把「該共享的幾項」從 localStorage 改存這裡。
# 哪幾項該共享、哪幾項不該（個人篩選搬上來會互相蓋掉），見 patch_store 的說明。
...
def patch_status(
    ...
    return patch_store.status(conn)


@app.get("/api/patch/blob/{key}")
def patch_blob_get(
    key: str,
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """讀一項。還沒人匯入過回 404——前端要能分辨「沒有」跟「壞掉」。"""
    from urllib.parse import quote as _quote

    import patch_store

    try:
        row = patch_store.get(conn, key)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc
    if not row:
        raise HTTPException(404, "還沒有這一項")
    return Response(
        content=bytes(row["body"]),
        media_type=row["content_type"] or "application/octet-stream",
        headers={
            "X-Updated-At": row["updated_at"] or "",
            "X-Updated-By": row["updated_by"] or "",
            "X-File-Name": _quote(row["file_name"] or ""),
        },
    )


@app.put("/api/patch/blob/{key}")
async def patch_blob_put(
    key: str,
    request: Request,
    file_name: str = "",
    session: sqlite3.Row = Depends(require_auth),
...
@app.delete("/api/patch/blob/{key}")
def patch_blob_delete(
    key: str,
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    ...
    try:
        return patch_store.delete(conn, key)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc


# ===== 弱點：解析成一筆一筆，才查得動單台（2026-09-21）=====
# 使用者：「以後每一台就可以看到有修過什麼弱點了吧? 多一個弱點管理紀錄」
# patch_store 存的是整包 Excel（給那支工具還原畫面用），查不了單台；
# 這裡解析成列存進 vuln_finding，給資產頁與差異比對用。兩件事不衝突。


@app.post("/api/vulns/import")
def vulns_import(
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """把目前存著的那份 Excel 解析成一筆一筆（匯入成功後由工具呼叫）。"""
    import patch_store
    import vuln_import

    row = patch_store.get(conn, "workbook")
    if not row:
        raise HTTPException(400, "還沒有匯入過弱點報告")
    try:
        return vuln_import.import_workbook(
            conn, bytes(row["body"]), row["file_name"],
            session["username"] if session else None)
    except Exception as exc:  # noqa: BLE001
        raise HTTPException(400, f"解析失敗：{exc}") from exc


@app.get("/api/vulns/batches")
def vulns_batches(
    limit: int = 20,
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """匯入批次。以**匯入時間**排序，不以檔名或週次——同一週匯兩次就是兩批。"""
    import vuln_import

    return {"items": vuln_import.batches(conn, limit)}


@app.get("/api/vulns/by-asset")
def vulns_by_asset(
    asset_serial: str,
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
    ...
    return vuln_import.by_asset(conn, asset_serial)


@app.get("/api/vulns/diff")
def vulns_diff(
    new_id: int, old_id: int,
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """兩批次的差異：新增／已修復／無法判定（這次沒掃到那台）三類。"""
    import vuln_import

    return vuln_import.diff(conn, new_id, old_id)


# ---------------------------------------------------------------------------
# 組態檢核（選單 9-2）。使用者 2026-09-21：「先做 AIX 那份，弱掃以後再改善」。
#
# 吃的是各平台檢核腳本產出的純文字檔（Linux 61 條／AIX 98 條／Windows 286 條）。
# 平台一律讀表頭的「平台:」，**不看檔名**——檔名可以被改。
# ---------------------------------------------------------------------------


# ---------------------------------------------------------------------------
# AD 人員名單（B-23）。關聯鍵是**員編**：主機帳號 gecos 開頭就是員編，
# AD 的 SamAccountName／employeeID 也是。姓名會改、會同名，不能當鍵。
#
# ⚠️ 這是全體員工個資：檔案只暫存在記憶體解析後就丟，**不落磁碟**。
# ---------------------------------------------------------------------------


@app.post("/api/ad/import")
def ad_import_endpoint(
    file: UploadFile = File(...),
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """匯入 AD 人員名單（.xlsx／.csv）。整份取代——AD 名單是全量快照。"""
    import ad_import
    ...
    try:
        rows = ad_import.rows_from_bytes(blob, file.filename)
        return ad_import.import_rows(conn, rows, file.filename,
                                     session["username"] if session else None)
    except Exception as exc:  # noqa: BLE001
        raise HTTPException(400, f"解析失敗：{exc}") from exc


# ===== 收集帳號遷移（2026-09-24 統一成 webit3sc）=====
# 設計見 AI/收集帳號遷移設計_2026-09-24.md。這三支**全部唯讀**：
# 探測只跑 `id`，不切換任何一台、不刪任何帳號。

@app.get("/api/collect-account/migration")
def collect_account_migration_summary(
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """遷移看板：四個數字＋分母講清楚是哪些。"""
    ...
    return cam.summary(conn)


@app.get("/api/collect-account/migration/detail")
def collect_account_migration_detail(
    state: str | None = None,
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    ...
    return {"items": cam.detail(conn, state)}


@app.post("/api/collect-account/migration/probe")
def collect_account_migration_probe(
    asset_serial: str | None = None,
    limit: int = 200,
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """探測：對每台用新帳號與舊帳號各試一次 `id`。**唯讀。**

    分批（`limit`）是刻意的：三千台 × 2 次 SSH 不可以一次打完，
    而且中途被中斷時**已經測完的要留著**（每台獨立寫入，不用整批 transaction）。
    """
    import collect_account_migration as cam
    import manage_state
    ...
    for r in rows:
        try:
            plat = manage_state.collect_platform_for(conn, r["ip"], r["asset_serial"])
            out.append(cam.probe_one(conn, r["asset_serial"], r["ip"], plat))
        except Exception as exc:  # noqa: BLE001 - 單台失敗不中斷整批
            out.append({"asset_serial": r["asset_serial"], "ip": r["ip"],
                        "error": str(exc)[:200]})
    return {"probed": len(out), "items": out, "summary": cam.summary(conn)}



@app.post("/api/collect-account/migration/switch")
def collect_account_migration_switch(
    asset_serial: str,
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    ...
    ⚠️ 切換前會**重新探測這一台**，只有當下仍然是「可切換」才切——
    看板上的狀態是上一次探測的結果，可能已經過期，
    拿過期的狀態去切那台會立刻收不到（2026-09-23 就是這個形狀）。
    """
    import collect_account_migration as cam

    try:
        return cam.switch_one(conn, asset_serial, by=(session["username"] if session else None))
    except ValueError as exc:
        raise HTTPException(400, str(exc))


@app.post("/api/collect-account/migration/revert")
def collect_account_migration_revert(
    asset_serial: str,
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """把這一台改回沿用全域設定。**還原路徑一定要有**——
    切了才發現收不到時要能立刻退回去，而不是等人去改資料庫。"""
    import collect_account_migration as cam

    return cam.revert_one(conn, asset_serial, by=(session["username"] if session else None))


@app.get("/api/collect-account/migration/history")
def collect_account_migration_history(
    asset_serial: str | None = None,
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    ...
    公司那八台 AIX 現在是假成功，遷移後數字會暴增。
    """
    import collect_account_migration as cam

    return {"items": cam.switch_history(conn, asset_serial)}


@app.get("/api/collect-account/migration/removal-plan")
def collect_account_migration_removal_plan(
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """要移除舊帳號的清單。**唯讀，只是列出來。**"""
    ...
    return cam.removal_plan(conn)


@app.get("/api/collect-account/migration/removal-script")
def collect_account_migration_removal_script(
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """移除舊帳號的指令清單（純文字）。

    ⚠️ **系統不代為執行。** 刪帳號要 root，而收集帳號是唯讀非 root；
    為了一次性的遷移給常駐收集身分刪帳號的權力並不划算——那條權限會一直留著，
    卻只用得到一次。所以產出純文字讓管理員看過再自己跑，
    ...
    return PlainTextResponse(cam.removal_script(conn))



@app.get("/api/ad/summary")
def ad_summary_endpoint(
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """匯入概況。每個數字都要點得進去，所以下面三支是對應的下鑽端點。"""
    import ad_import

    return ad_import.summary(conn)


@app.get("/api/ad/unmatched-managers")
def ad_unmatched_managers(
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """主管 DN 對不到名單裡任何一列的人。

    「沒有主管」跟「對不上」是兩件事——前者不用處理，後者是名單有缺。
    """
    import ad_import

    return {"items": ad_import.unmatched_managers(conn)}


@app.get("/api/ad/unmatched-accounts")
def ad_unmatched_accounts(
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """主機帳號對不到人的兩種情況，分開回——原因與處理方式不同。"""
    import ad_import

    return ad_import.unmatched_accounts(conn)


@app.get("/api/ad/disabled-with-account")
def ad_disabled_with_account(
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """AD 已停用（離職）但主機上還留著帳號——**這是稽核發現，不是雜訊**。"""
    import ad_import

    return {"items": ad_import.disabled_with_host_account(conn)}


@app.post("/api/config-audit/import")
def config_audit_import(
    file: UploadFile = File(...),
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    ...
    截斷的檔案會被擋下來（腳本宣稱的條數跟實際對不起來）——少查的條目
    如果照收，畫面上會變成「那些都沒問題」，那是最危險的誤讀。
    """
    import config_audit
    ...
    if not blob:
        raise HTTPException(400, "檔案是空的")
    try:
        return config_audit.import_report(
            conn, blob, file.filename,
            session["username"] if session else None)
    except Exception as exc:  # noqa: BLE001
        raise HTTPException(400, f"解析失敗：{exc}") from exc
...
def config_audit_list(
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """清單：每台一列，取最新一份。同時回總覽數字供上方卡片用。"""
    import config_audit

    return {"items": config_audit.runs(conn), "summary": config_audit.summary(conn)}


@app.get("/api/config-audit/history")
def config_audit_history(
    limit: int = 50,
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """匯入紀錄，含失敗的——匯過但壞掉要看得見，不然沒人知道。"""
    import config_audit

    return {"items": config_audit.history(conn, limit)}


@app.get("/api/config-audit/unmatched")
def config_audit_unmatched(
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """有檢核結果、卻對不回資產清冊的主機——這本身就是發現。"""
    import config_audit

    return {"items": config_audit.unmatched(conn)}


@app.get("/api/config-audit/export")
def config_audit_export(
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """匯出給 DYN 分析：吐回各機器最新一份的**原始檔全文**，不是重組的。"""
    import config_audit
    from fastapi.responses import PlainTextResponse

    text, have, missing = config_audit.export_status(conn)
    if not text.strip():
        if missing:
            raise HTTPException(404,
                f"有 {missing} 份檢核結果，但都是在系統開始保存原始檔之前匯入的，"
                "沒有原文可以吐給 DYN。把那幾個 .txt 重新匯入一次就可以了"
                "（不用重跑檢核腳本）。")
        raise HTTPException(404, "還沒有任何檢核結果可以匯出")
    return PlainTextResponse(text, headers={
        "Content-Disposition": 'attachment; filename="config_audit_export.txt"'})


@app.get("/api/config-audit/by-asset")
def config_audit_by_asset(
    asset_serial: str,
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    ...
    路由要放在 /{run_id} **前面**——不然 `by-asset` 會被當成 run_id 吃掉，
    回 422。這種撞路由的錯，畫面只會顯示「載入失敗」，很難查。
    """
    import config_audit

    return config_audit.by_asset(conn, asset_serial)


@app.get("/api/config-audit/{run_id}")
def config_audit_detail(
    run_id: int,
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    ...
    d = config_audit.detail(conn, run_id)
    if not d:
        raise HTTPException(404, "查無這份檢核結果")
    return d


@app.get("/api/vulns/unmatched")
def vulns_unmatched(
    ...
    return {"items": vuln_import.unmatched_hosts(conn, batch_id)}
...
def eos_summary(
    ...
    rows = conn.execute(
        "SELECT asset_serial, hostname, ip, os, device_model, asset_status, asset_name, asset_purpose "
        "FROM hardware"
    ).fetchall()
    ...
    # [口徑] 2026-09-20：磚塊與每一列的「資產數」以前逐筆算（同一台多筆登記會被算好幾次）。
    # 畫面寫「台」就要逐台——狀態統計與每一列改成收集正典 machine_key，最後再換成數量。
    import system_stats as _ss

    def bump_m(d, k, mk):
        d.setdefault(k, set()).add(mk)
    ...
    for r in rows:
        if (r["asset_status"] or "").strip() in manage_state.RETIRED_STATUS:
            continue
        mk = _ss.machine_key(r["hostname"], r["ip"], r["asset_serial"])
        if r["os"]:
            os_info = normalize.normalize_os(r["os"], conn, r["device_model"])
            ...
            hit = eos.lookup_os_eos(canonical)
            status = eos.eos_status(hit["eos_date"]) if hit else "unknown"
            bump_m(by_status[bucket], status, mk)
            family, linux_distro = normalize.os_family(os_info["product"], canonical)
            # [2026-09-20 使用者拍板：「類別要一樣，改第二」] 作業系統分頁的第二層改用
            # 全站正典的 OS 類型（pipeline._os_type，AIX 與 IBM i 分開、含 OpenShift 節點等），
            # 不再用 EOS 自己那套（Linux／Windows／AIX/Unix／VMware）。第一層分頁不動。
            if bucket == "host_os":
                try:
                    import pipeline as _pl

                    family = _pl._os_type(canonical, None) or family
                except Exception:  # noqa: BLE001 - 分類失敗就退回原本的 family，不讓整頁掛掉
                    pass
    ...
    def sort_items(d: dict) -> list[dict]:
        ...
        for x in d.values():                      # 台數＝去重後的機器數（見 bump_m）
            if "_m" in x:
                x["count"] = len(x.pop("_m"))
        return sorted(d.values(), key=lambda x: (order.get(x["status"], 9), -x["count"]))

    def st(d: dict) -> dict:                      # 狀態統計：set → 台數
        return {k: len(v) if isinstance(v, set) else v for k, v in d.items()}

    import pipeline as _pl2

    return {
        # 第二層分類的順序（跟漏斗、OS×環境同一份），前端照這個排
        "os_order": list(_pl2.OS_ORDER),
        "host_os": {"by_status": st(by_status["host_os"]), "items": sort_items(_merge_patch_versions(items["host_os"]))},
        "firmware": {"by_status": st(by_status["firmware"]), "items": sort_items(_merge_patch_versions(items["firmware"]))},
        "software": {"by_status": st(by_status["software"]), "items": sort_items(_merge_patch_versions(items["software"]))},
        "insufficient": {"by_status": st(by_status["insufficient"]), "items": sort_items(_merge_patch_versions(items["insufficient"]))},
        "other": {"by_status": st(by_status["other"]), "items": sort_items(_merge_patch_versions(items["other"]))},
        "hardware": {"by_status": st(hw_by_status), "items": sort_items(hw_items)},
    }
...
def _merge_patch_versions(items_by_canonical: dict) -> dict:
    ...
    for (base_name, _eos_date, _source_url, _confidence), group in groups.items():
        ...
        merged[base_name] = {
            **group[0],
            "name": base_name,
            # [2026-09-20] 台數是 machine_key 的集合（見 eos_summary.bump_m）：合併版本時要**取聯集**，
            # 不可以相加——同一台同時有 16.12 和 16.12.07 兩筆登記，相加會把它算兩次
            "_m": set().union(*[g.get("_m") or set() for g in group]),
...
# 起因：一台在 CIA 上有 5 筆登記、用途都是「…DB」，使用者問「看得出來為什麼放一起嗎?」
# 系統沒有地方記「為什麼放一起」——那是人才知道的事。規則見 asset_tags 模組說明。

class AssetTagBody(BaseModel):
    tag: str
    note: str = ""
    source: str = "manual"      # manual＝人自己打的；suggested＝採用系統建議


@app.get("/api/assets/{asset_serial}/tags")
def asset_tags_get(
    asset_serial: str,
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """這台的標籤＋系統建議（建議一定附依據，而且不會自己存進資料）。"""
    import asset_tags

    if asset_tags.machine_of(conn, asset_serial) is None:
        raise HTTPException(404, f"查無資產序號 {asset_serial}")
    return {
        "tags": asset_tags.list_tags(conn, asset_serial),
        "suggestions": asset_tags.suggest(conn, asset_serial),
        "known_tags": asset_tags.all_tags(conn),      # 給自動完成用：別人已經用過哪些字
        "registrations": len(asset_tags.rows_of_machine(conn, asset_serial)),
    }


@app.post("/api/assets/{asset_serial}/tags")
def asset_tags_add(
    asset_serial: str,
    body: AssetTagBody,
    session: sqlite3.Row = Depends(require_auth),
    ...
    """貼一個標籤（同名再貼＝更新備註）。標籤掛在「台」上，不是掛在這一筆登記。"""
    import asset_tags

    try:
        return asset_tags.add_tag(conn, asset_serial, body.tag, body.note,
                                  session["username"] if session else None, body.source)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc


@app.delete("/api/assets/{asset_serial}/tags/{tag}")
def asset_tags_remove(
    asset_serial: str,
    tag: str,
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
    ...
    try:
        return asset_tags.remove_tag(conn, asset_serial, tag)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc


@app.get("/api/asset-tags")
def asset_tags_all(
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    ...
    return {"items": asset_tags.all_tags(conn)}
...
@app.post("/api/data-quality/person-merge/preview")
def data_quality_person_merge_preview(
    body: PersonMergeBody,
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """人名合併「預覽」：把打錯的名字併到正確名字前，先看會動到哪些欄位、各幾筆（不寫入）。
    ⚠️ 路由要排在 /api/data-quality/{dim_key} 之前。"""
    import data_quality
    try:
        return data_quality.preview_person_merge(conn, body.from_name, body.to_name)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))


@app.post("/api/data-quality/person-merge")
def data_quality_person_merge_apply(
    body: PersonMergeBody,
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """人名合併「執行」：預覽確認後才呼叫，實際把錯名併成正確名。回各欄改了幾筆。"""
    import data_quality
    try:
        return data_quality.apply_person_merge(conn, body.from_name, body.to_name)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def _can_edit_nav(session: sqlite3.Row, conn: sqlite3.Connection) -> bool:
    ...
    # 先問「登入是不是停用」再去讀 session：停用時 session 可能是 None，
    # 先讀就直接 TypeError（2026-09-20 寫匯入測試時踩到）
    if auth_disabled(conn):
        return True
    return bool(session is not None and session["username"] == "admin")
...
def _nav_layout_of(conn) -> list | None:
    row = conn.execute("SELECT value FROM app_settings WHERE key = ?",
                       (NAV_LAYOUT_SETTING,)).fetchone()
...
def _validate_nav_layout(layout) -> list:
    """匯入前先擋明顯不對的東西——選單壞掉等於整套系統找不到路。

    只驗結構（群組要有名稱、items 是路由字串或 {to,label}），不驗路由存不存在：
    認不得的路由前端本來就會略過，擋掉反而讓跨版本搬設定失敗。
    """
    if not isinstance(layout, list) or not layout:
        raise ValueError("格式不對：最外層要是一個群組陣列")
    for g in layout:
        if not isinstance(g, dict) or not str(g.get("label") or "").strip():
            raise ValueError("格式不對：每個群組要有 label")
        items = g.get("items")
        if not isinstance(items, list):
            raise ValueError(f"格式不對：群組「{g.get('label')}」的 items 要是陣列")
        for it in items:
            if isinstance(it, str):
                continue
            if isinstance(it, dict) and str(it.get("to") or "").strip():
                continue
            raise ValueError(f"格式不對：群組「{g.get('label')}」裡有不認得的項目 {it!r}")
    return layout


@app.get("/api/nav-layout/export")
def export_nav_layout(
    session: sqlite3.Row = Depends(require_auth), conn: sqlite3.Connection = Depends(get_db)
):
    """把這台的選單設定匯出成一個檔（拿去另一台匯入）。

    2026-09-20 使用者：221 跟公司機選單順序不一樣，問「我要抓什麼回來才會同步」。
    選單順序存在各台自己的資料庫、不隨 patch 走，所以需要一個單獨搬它的方法
    （整庫備份雖然含這份設定，但為了搬選單去覆蓋整個資料庫太超過）。
    """
    layout = _nav_layout_of(conn)
    payload = {
        "kind": "webit3-nav-layout",
        "version": 1,
        "exported_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "source": os.environ.get("WEBIT_SITE") or socket.gethostname(),
        # null＝這台用內建預設；匯入端照樣吃，代表「把對方也恢復成預設」
        "layout": layout,
    }
    return Response(
        content=json.dumps(payload, ensure_ascii=False, indent=2),
        media_type="application/json; charset=utf-8",
        headers={"Content-Disposition": 'attachment; filename="nav_layout.json"'},
    )


class NavLayoutImportBody(BaseModel):
    kind: str | None = None
    version: int | None = None
    layout: list | None = None


@app.post("/api/nav-layout/import")
def import_nav_layout(
    body: NavLayoutImportBody,
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """匯入另一台匯出的選單設定（限管理員）。layout 是 null＝改用內建預設。"""
    if not _can_edit_nav(session, conn):
        raise HTTPException(403, "只有管理員（admin 帳號）能改選單順序")
    if body.kind and body.kind != "webit3-nav-layout":
        raise HTTPException(400, f"這不是選單設定檔（kind={body.kind}）")
    before = _nav_layout_of(conn)
    if body.layout:
        try:
            layout = _validate_nav_layout(body.layout)
        except ValueError as exc:
            raise HTTPException(400, str(exc)) from exc
        conn.execute(
            "INSERT INTO app_settings (key, value) VALUES (?, ?) "
            "ON CONFLICT(key) DO UPDATE SET value = excluded.value",
            (NAV_LAYOUT_SETTING, json.dumps(layout, ensure_ascii=False)))
        groups, items = len(layout), sum(len(g.get("items") or []) for g in layout)
        note = f"匯入選單設定（{groups} 群、{items} 項）"
    else:
        conn.execute("DELETE FROM app_settings WHERE key = ?", (NAV_LAYOUT_SETTING,))
        groups = items = 0
        note = "匯入選單設定：改用內建預設"
    conn.commit()
    try:
        activity.log(conn, username=session["username"], ip=None, method="POST",
                     path="/api/nav-layout/import", status=200, duration_ms=0, action=note)
    except Exception:  # noqa: BLE001
        pass
    return {"ok": True, "groups": groups, "items": items,
            "replaced": bool(before), "note": note}
...
@app.get("/api/version")
def version_endpoint(conn: sqlite3.Connection = Depends(get_db)):
    ...
    info = {"version": "?", "git_commit": None, "dirty": None,
            ...
            #
            # ⚠ 2026-09-21：這裡本來沒帶 conn，所以 auth_disabled() 只看得到環境變數，
            # 看不到存在 app_settings 的那個設定。221 實際上 auth_disabled='1'（登入是
            # 關的、未登入也擈得走全部資產），這支卻一直回 false——**防忘記關的
            # 那條橫幅從來沒出現過**。保險本身壞掉比沒保險更危险，
            # 因為看到畫面沒橫幅的人會以為登入是開的。
            # hasattr 防的是「不透過 HTTP、直接呼叫這支函式」的情況
            # （FastAPI 的 Depends 預設值不會被解開）。不加的話那條路徑會
            # AttributeError，而那支是免登入端點，炋了畫面連版號都拿不到。
            "auth_disabled": auth_disabled(conn if hasattr(conn, "execute") else None)}
...
# ===== 主機自我檢查（批次 1：免提權即時健檢，2026-09-21）=====
# 值班驗證 WhatsUp Gold 告警是誤報還是真故障：輸入 IP → 唯讀收 磁碟/記憶體/負載/
# port/服務 → 紅黃綠＋原始值當證據。只打已知目標、並發有上限、只支援 Linux（批次 1）。
_HEALTH_CONCURRENCY_DEFAULT = 8
_HEALTH_CONCURRENCY_MAX = 16          # 再高就像掃描/暴力，會惹 SOC（沿用批次納管的克制）
_HEALTH_MAX_TARGETS = 200            # 一次上限；整段要跑先由前端二次確認再送這批
...
@app.get("/api/health-check/targets")
def health_check_targets(
    cidr: str,
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """某網段裡「已登記」的 IP（給『整段健檢』的二次確認用）。
    只回資產庫有的 IP——健檢只打已知目標，不展開整段去探未知位址（那才像掃描）。"""
    try:
        net = ipaddress.ip_network(cidr.strip(), strict=False)
    except ValueError:
        raise HTTPException(400, "網段格式不對（例：10.99.1.0/24）")
    hits = []
    for r in conn.execute(
        "SELECT DISTINCT ip, hostname FROM hardware WHERE ip IS NOT NULL AND TRIM(ip) <> ''"
    ).fetchall():
        try:
            if ipaddress.ip_address(r["ip"].strip()) in net:
                hits.append({"ip": r["ip"].strip(), "hostname": r["hostname"]})
        except ValueError:
            continue                                 # ip 欄不是合法位址（範本/KVM），略過
    hits.sort(key=lambda x: ipaddress.ip_address(x["ip"]))
    return {"cidr": str(net), "count": len(hits), "items": hits}
...
def health_check_run(
    ...
    """對一批已知 IP 並發健檢，回逐台結果＋摘要。Linux／AIX 走 SSH，Windows 走 WinRM。"""
    import collect_log
    import health_probe
    import manage_state

    def _clean_host(s: str) -> str:
        # 剝掉 http://、路徑、user@、:port——值班常貼網址，整串拿去 SSH 會噴假的「解析失敗」
        s = (s or "").strip()
        if "://" in s:
            s = s.split("://", 1)[1]
        ...
        if ":" in s and s.rsplit(":", 1)[1].isdigit():
            s = s.rsplit(":", 1)[0]
        return s.strip()
    ips = [h for h in (_clean_host(ip) for ip in (body.ips or [])) if h]
    if not ips:
        raise HTTPException(400, "沒有要檢查的 IP")
    if len(ips) > _HEALTH_MAX_TARGETS:
        raise HTTPException(
            400, f"一次最多 {_HEALTH_MAX_TARGETS} 台，這次 {len(ips)} 台——請分批")
    conc = body.concurrency or _HEALTH_CONCURRENCY_DEFAULT
    conc = max(1, min(conc, _HEALTH_CONCURRENCY_MAX))

    def _platform_of(ip: str) -> str:
        try:
            # 用 collect_platform_for（會先去 hardware 查 os）而不是 collect_platform_of：
            # 後者在沒有 os 字串時只剩埠號可猜，AIX 只開 22 -> 一律猜成 linux，
            # 然後拿 /proc 那組指令去問 AIX，整台白檢查。
            return manage_state.collect_platform_for(conn, ip)
        except Exception:  # noqa: BLE001 - 查不到平台就當 linux 試
            return "linux"

    def _creds_of(ip: str):
        """Windows 走 WinRM 要帳密（Linux／AIX 用金鑰，不會走到這裡）。

        沿用既有的收集憑證，**不另外設一組健檢專用的**——多一組就是第二份真相，
        改了一邊另一邊還是舊的，而症狀只是「某些機器查不到」。
        查不到就回 None，由 health_probe 顯示「沒有可用的 WinRM 收集憑證」，
        **不可以顯示成連不上**：一個是去設定頁補帳號，一個是去機房。
        """
        import credential_store
        ...
        try:
            ...
            if not name:
                return None
            return credential_store.get_for_use(conn, name)
        except Exception:  # noqa: BLE001 - 取不到憑證就當沒有，錯誤訊息由上層講
            return None

    items = health_probe.check_batch(ips, platform_of=_platform_of, concurrency=conc,
                                     creds_of=_creds_of)
    # 批次 2：跟最近一份 baseline 比（方向性 diff：消失 port＝紅、新增＝提示）。沒有基準回 None。
    import health_baseline
    for r in items:
        try:
            r["diff"] = health_baseline.diff(health_baseline.latest_baseline(conn, r["ip"]), r)
        except Exception:  # noqa: BLE001 - 比對失敗不影響即時健檢本體
            r["diff"] = None
    summary = {
        "total": len(items),
        "red": sum(1 for r in items if r["overall"] == "red"),
        "yellow": sum(1 for r in items if r["overall"] == "yellow"),
        "green": sum(1 for r in items if r["overall"] == "green"),
        "skipped": sum(1 for r in items if r["overall"] == "skipped"),
        # **主詞不同的兩件事，名字就要不同。**
        # unreachable＝我們真的探測過、那台沒有回應（關於那台機器的宣稱）。
        # not_probed＝我們根本沒去成：沒有憑證／沒有金鑰／平台不支援
        #            （關於我們自己的事實，是我們的待辦，不是那台的狀態）。
        # 2026-09-23：混在一起時，值班會跑去機房看一台好好的機器，
        # 而正確的下一步是走三步到設定頁補一個帳號。
        "unreachable": sum(1 for r in items
                           if not r["reachable"] and r["overall"] == "red"
                           and not r.get("not_probed")),
        "not_probed": sum(1 for r in items if r.get("not_probed")),
        # 「查完了沒」跟「查出來好不好」是兩件事，摘要要分開給——
        # 值班要能一眼把「檢查完整且綠」跟「還沒查完」分開看（2026-09-23）。
        #
        # **只算「連得到卻沒查完」**：連不上的那台什麼都沒查到，但它已經算在
        # unreachable 也顯示成紅色，再算一次只是把真正要看的那批稀釋掉。
        # 「連不上」有自己的名字（unreachable），**同一個名字不可以在兩個地方
        # 是兩個意思**——前端因此直接用這個數字，不自己再算一份。
        "incomplete": sum(1 for r in items
                          if r.get("reachable") and r.get("complete") is False),
    }
    collect_log.record(
        conn, "health_check", None, summary["red"] == 0,
        f"健檢 {summary['total']} 台：紅 {summary['red']}、黃 {summary['yellow']}、"
        f"綠 {summary['green']}（連不上 {summary['unreachable']}）",
        actor=session["username"] if session else None,
        extra={"summary": summary, "concurrency": conc})
    return {"items": items, "summary": summary, "concurrency": conc}
...
def health_check_history(
    ...
    """某台的健康基準快照序列（批次 2）：看 port 數／燈號隨時間的變化點。"""
    import health_baseline

    return {"ip": ip, "items": health_baseline.history(conn, ip)}


@app.post("/api/health-check/baseline/run")
def health_check_baseline_run(
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """手動立刻收一份週基準（批次 2）：平常週六排程自動跑；這顆給第一次鋪底／值班想立刻更新基準用。"""
    import health_baseline

    return health_baseline.run_weekly_baseline(conn)
...
@app.get("/api/accounts/privileged-trend")
def accounts_privileged_trend(
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """特權帳號趨勢：每次採集後存的一筆現值，累積成時間序列（2026-09-19）。
    資料是採集後才開始累積的——點數少代表剛開始收，不是沒有特權帳號。"""
    import account_inventory

    return {"items": account_inventory.privileged_trend(conn)}
...
# ===== B-19 弱點到期自動寄信 =====
# 使用者拍板：每週一、每月 1 號 08:00 寄；門檻 14/30/60 天；收件人＝弱點報告上的
# 負責人（查 AD 名單拿信箱）；月報主管收副本。**SMTP 每一個值都要能在畫面上改。**


class VulnNotifySettings(BaseModel):
    enabled: bool = False
    smtp_host: str | None = None
    smtp_port: int | None = None
    mail_from: str | None = None
    send_time: str | None = None          # "08:00"
    buckets: list[int] | None = None      # 預設 14/30/60
    fallback_to: str | None = None        # 找不到負責人時寄給誰
    manager_cc: bool = True               # 月報給主管副本
    subject_prefix: str | None = None     # 主旨前綴（收信端做規則分類用）
    always_cc: str | None = None          # 一律副本給（窗口，逗號分隔）
    # 收件範圍：先選單位、再挑人。**兩個都空＝全部通知**（見 vuln_notify.in_scope）
    only_depts: list[str] | None = None
    only_people: list[str] | None = None


def _vn_public(cfg: dict) -> dict:
    """回給畫面的設定。**含 pass/secret 的欄位一律不回**——目前公司是免認證
    內部 relay 所以沒有密碼欄，但這種東西要在「還沒有」的時候就擋住，
    不是等有人加了密碼欄之後才想到 API 會把它吐出去。"""
    return {k: v for k, v in cfg.items()
            if not any(w in k.lower() for w in ("pass", "secret", "token"))}


@app.get("/api/vuln-notify/settings")
def vuln_notify_get_settings(
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """寄信設定＋**還缺什麼**。畫面要能一眼看出為什麼現在不會寄。"""
    import vuln_notify

    try:
        cfg = vuln_notify.load_settings(conn)
        corrupt = None
    except vuln_notify.SettingsCorrupt as exc:
        # 壞掉跟沒設定要分開：沒設定是去填，壞掉是去看誰把它寫壞了。
        cfg, corrupt = {}, str(exc)
    h, m = vuln_notify.send_time_of(cfg)
    return {
        "settings": _vn_public(cfg),
        "corrupt": corrupt,
        "missing": vuln_notify.check_settings(cfg),
        "ad_ready": vuln_notify.ad_ready(conn),
        "send_time": f"{h:02d}:{m:02d}",
        "buckets": list(vuln_notify.buckets_of(cfg)),
        "last_runs": vuln_notify.last_runs(conn, 10),
    }


@app.put("/api/vuln-notify/settings")
def vuln_notify_put_settings(
    body: VulnNotifySettings,
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """存設定。**缺必填卻按啟用時直接擋下並指名缺哪一項**——
    讓它存成「啟用但不會寄」的狀態，畫面會顯示開著、實際一封都不寄，最難查。"""
    import vuln_notify

    # 只濾掉 None（沒給），**空清單／空字串要留著**——
    # 否則使用者把「只通知架構部」清掉之後，舊設定還在 DB 裡繼續生效，
    # 畫面顯示「全部」但實際只寄給架構部。
    cfg = {k: v for k, v in body.model_dump().items() if v is not None}
    missing = vuln_notify.check_settings(cfg)
    if cfg.get("enabled") and missing:
        raise HTTPException(400, f"要啟用就得先填：{'、'.join(missing)}")
    vuln_notify.save_settings(conn, cfg)
    return {"ok": True, "settings": _vn_public(cfg), "missing": missing}


@app.get("/api/vuln-notify/recipients")
def vuln_notify_recipients(
    depts: str | None = None,
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """可以挑的單位與人（來源是 AD 名單）。depts 用逗號分隔，給了就只回那幾個單位的人。"""
    import vuln_notify

    picked = [d.strip() for d in (depts or "").split(",") if d.strip()]
    return {"departments": vuln_notify.departments(conn),
            "people": vuln_notify.people_of(conn, picked or None)}


@app.post("/api/vuln-notify/preview")
def vuln_notify_preview(
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """預覽會寄給誰、幾筆、誰沒人收。**不寄、也不動 last_bucket。**

    固定用月報（週報的超集）而不是看今天是不是寄送日：人會在平日按這顆，
    回「今天不是寄送日」等於預覽在需要它的時候不能用。
    """
    import vuln_notify

    return vuln_notify.run(conn, schedule="monthly", dry_run=True)


@app.post("/api/vuln-notify/run")
def vuln_notify_run_now(
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """立刻寄一期（月報內容）。第一次鋪底、或排程當天出事要補寄時用。

    記成 schedule='manual'：**不可以佔掉當天排程那一期的位置**，
    否則手動補一次就會讓當天的自動班表被當成做過了。
    """
    import vuln_notify
    from datetime import datetime as _dt

    now = _dt.now()
    r = vuln_notify.run(conn, now.date(), "monthly")
    vuln_notify.mark_run(conn, now.strftime("%Y-%m-%d"), "manual", r, now)
    return r


@app.get("/api/vuln-notify/logs")
def vuln_notify_logs(
    limit: int = 200,
    session: sqlite3.Row = Depends(require_auth),
    conn: sqlite3.Connection = Depends(get_db),
):
    """寄送紀錄：那天寄到哪去了、誰沒收到、SMTP 回了什麼。"""
    import vuln_notify

    return {"items": vuln_notify.logs(conn, min(max(limit, 1), 1000)),
            "runs": vuln_notify.last_runs(conn, 50)}
