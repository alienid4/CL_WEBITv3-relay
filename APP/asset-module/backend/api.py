...
def list_assets(
    ...
    if source == "registered":
        query += (" AND asset_serial NOT LIKE 'DYN-%' AND asset_serial NOT LIKE 'VC-%' "
                  "AND asset_serial NOT LIKE 'AUTO-%'")
    elif source == "offbook":
        query += (" AND (asset_serial LIKE 'DYN-%' OR asset_serial LIKE 'VC-%' "
                  "OR asset_serial LIKE 'AUTO-%')")
...
def collect_dispatch_run(
    ...
    collect_log.record(
        ...
        f"探測 {out.get('total', 0)} 台：已納管 {out.get('collected', 0)}、"
        f"要人工處理 {out.get('needs_action', 0)}、不納管 {out.get('not_onboardable', 0)}、"
        f"沒有回應（只能匯入）{bs.get('import_only', 0)}",
...
def update_asset(
    ...
    if "asset_status" in cleaned:
        old_status = conn.execute("SELECT asset_status FROM hardware WHERE asset_serial = ?",
                                  (asset_serial,)).fetchone()[0]
...
def backup_dump_endpoint(
    ...
    if recipient is None:
        raise HTTPException(
            400,
            "還沒設定收件人公鑰，無法匯出。匯出檔會離開這台機器，一律加密——"
            "請先到系統設定貼上收件端（221）的公鑰。"
            "公鑰在收件端的系統設定產生，不需要保密，可以直接複製貼上。")
...
def export_accounts(
    ...
    ws = wb.active
    if fmt == "standard":
        rows, summary = account_export.standard_rows(conn)
        ws.title = "帳號盤點"
        cols = account_export.STANDARD_COLUMNS
        ws.append(cols)
        for r in rows:
            ws.append([r[c] for c in cols])
    else:
        rows, cols = account_export.full_rows(conn)
        ws.title = "帳號全匯出"
        ws.append(cols)
        for r in rows:
