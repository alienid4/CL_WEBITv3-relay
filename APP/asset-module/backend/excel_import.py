...
def import_excel(xlsx_path: Path, conn: sqlite3.Connection) -> dict[str, Any]:
    ...
    summary: dict[str, Any] = {"sheets": {}, "errors": [], "example_rows": 0}
    from import_templates import is_example

    # 三個分頁一個都沒有 = 這不是 CIA 資產清冊（最常見：把 RVTools 或對照表放錯列）。
    # 以前會回 200 加三行「找不到分頁」，畫面看起來像「匯入完成 0 筆」。
    if not any(s in wb.sheetnames for s in SHEET_CONFIG):
        raise ValueError(f"匯錯檔了？這不是 CIA 資產清冊：要有「{'／'.join(SHEET_CONFIG)}」分頁，"
                         f"這份檔案的分頁是：{wb.sheetnames}")
    ...
    for sheet_name, cfg in SHEET_CONFIG.items():
        ...
        for record in rows:
            if is_example(record.get("asset_serial")):
                summary["example_rows"] += 1   # 範例檔的範例列
                continue
