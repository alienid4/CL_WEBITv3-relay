...
STAGES = [
    ...
    },
    {
        ...
        "label": "非納管設備（已豁免）",
        "tone": "muted",
        "why": "人工標記為不納管並填了原因，或是 OCP 節點（RHCOS）——"
               "系統區唯讀、本機帳號重佈就消失，要建帳號得改 MachineConfig",
        "next": "不需處理。人工豁免的可在資產頁取消；"
                "OCP 節點第一期不納管（使用者 2026-09-21 拍板），第二期改向叢集 API 取資料",
        "action": "",
    },
    {
...
def summarize(conn) -> dict:
    ...
    for r in conn.execute(
            "SELECT asset_serial, hostname, ip, os, device_model, asset_name, asset_purpose, api_id, "
            "hw_serial, environment, physical_location, "
    ...
    # 「為什麼它重複」要講得出來（2026-09-20 使用者：「我看不出來為什麼他重複」）。
    # 畫面只顯示「重複 2」等於沒說；這裡逐台比對那幾筆的差別，給一句人看得懂的理由。
    for it in items:
        it["dup_reason"] = _dup_reason(hw, it.get("dup_serials") or [])
...
def _dup_reason(hw: dict, serials: list[str]) -> str:
    """同一台為什麼有好幾筆登記——用那幾筆的差別講白話。

    CIA 清冊是「一個服務／VIP 一筆」，所以多筆**多半不是錯**。分四種：
    帳外來源另外補的、掛不同業務系統、同系統但用途／VIP 分列、欄位幾乎一樣（才是疑似真重複）。
    """
    import system_report

    rows = [hw[s] for s in serials if s in hw]
    if len(rows) < 2:
        return ""
    off = [r["asset_serial"] for r in rows if system_report.is_off_book(r["asset_serial"])]
    cia = [r for r in rows if not system_report.is_off_book(r["asset_serial"])]
    parts = []
    if off:
        parts.append(f"其中 {len(off)} 筆是帳外來源自動補的（{'、'.join(off[:3])}）")
    systems = {(r["api_id"] or "").strip() for r in cia if (r["api_id"] or "").strip()}
    if len(systems) > 1:
        parts.append(f"CIA 上掛了 {len(systems)} 個業務系統（{'、'.join(sorted(systems)[:4])}）")
    elif len(cia) > 1:
        names = {((r["asset_name"] or "") + "／" + (r["asset_purpose"] or "")).strip("／") for r in cia}
        if len(names) > 1:
            parts.append("同一個系統，但名稱／用途分列")
        else:
            parts.append(f"{len(cia)} 筆 CIA 登記的欄位幾乎一樣——疑似真重複，可到資產查詢逐欄比對")
    return "；".join(parts)
