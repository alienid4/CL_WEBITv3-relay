...
CREATE TABLE IF NOT EXISTS hardware (
    ...
    -- 這筆是從哪個管理端匯進來的（RVTools vInfo 的 `VI SDK Server` 欄）。
    --
    -- ⚠️ 名字叫 vi_sdk_server 不叫 vcenter，是刻意的：**RVTools 也能直連單台 ESXi**，
    -- 這欄記的是「RVTools 當時連的是誰」，不保證是 vCenter。叫 vcenter 會讓讀的人
    -- 以為系統確認過那是一座 VC，那是我們沒查證的事。
    --
    -- NULL 有三種完全不同的原因，**不可混為一談**（畫面要分開講）：
    --   1. 這台不是從 vCenter/RVTools 來的（手動建、Excel 匯入、掃描發現）
    --   2. 那份 RVTools 匯出沒有 VI SDK Server 欄（舊版 RVTools）
    --   3. 匯入時間早於這個功能（2026-08-28 以前匯的 2000 多台都是這種）
    -- 只顯示空白會讓人以為「查過了，沒有」——那是第 4 種意思，而且是假的。
    vi_sdk_server TEXT,
    ...
    -- 業務用途分類（部門報告頁B/頁C 的核心交易／非核心分色依據）。
    --
    -- ⚠️ 為什麼掛在**每一台**上，不是掛在業務系統（api_id）上：
    -- 2026-08-26 拿管理員的《系統盤點(全環境)Data》驗證，155 個 api_id 裡有 **88 個
    -- 橫跨多種分類**（例如 N-070 正式環境的機器就散在 6 個不同分類）。分類是逐台的
    -- 人工判斷，不是業務系統的屬性——做成 api_id 對照表根本表達不了這件事。
    --
    -- ⚠️ 這是**人填的業務欄位**，跟用途/保管者同一類：RVTools／CIA／dynassets 匯入
    -- 只更新機器事實（主機名/IP/OS），**不可覆蓋這欄**。否則使用者辛苦分好的類，
    -- 下次匯入就被洗掉（FACT_FIELDS 的既有原則，見各匯入模組）。
...
CREATE TABLE IF NOT EXISTS sessions (
    ...
    -- 2026-08-26「誰在線上」用。**在線 ≠ session 沒過期**：TTL 有好幾小時，
    -- 開著分頁去吃飯的人 session 還在，但他不在用。所以在線的定義是
    -- 「last_seen_at 在最近 N 分鐘內」，而 last_seen_at 由 require_auth 每次
    -- 請求更新（60 秒內只寫一次，見 activity.touch_session 的節流說明）。
    last_seen_at TEXT,
    last_ip TEXT,
    user_agent TEXT
);
...
-- ===== 操作紀錄（誰、什麼時候、做了什麼）=====
-- 2026-08-26 使用者要求：「我要知道誰在用，LOG 紀錄也要」。
--
-- **只記會改東西的請求（非 GET）與登入相關事件**，不記 GET。理由：GET 佔九成以上
-- 流量而稽核價值低，全記會讓這張表一天長好幾萬列，真正要查「誰改了這台」的時候
-- 反而被淹掉。（讀取類真的要留痕的地方已經各自有專屬表：doc_download_audit、
-- credential_use_audit。）
--
-- ⚠️ **不記 request body**。body 裡可能有密碼、真實主機名、人員姓名電話——
-- 把它們複製一份進另一張表只是多開一個外洩面，稽核要的是「誰做了什麼動作」，
-- 不是「他打了什麼字」。要看改了什麼值，看的是資料本身的 updated_at 與既有的
-- 各種 audit 表。
CREATE TABLE IF NOT EXISTS activity_log (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    at TEXT NOT NULL,                       -- 本地時間（決策 T6）
    username TEXT,                          -- 未登入時為 NULL（例如登入失敗）
    ip TEXT,
    method TEXT NOT NULL,
    path TEXT NOT NULL,
    status INTEGER,                         -- HTTP 狀態；失敗的動作也要留，才看得出誰在試什麼
    duration_ms INTEGER,
    action TEXT                             -- login / logout / login_failed / change
);
CREATE INDEX IF NOT EXISTS idx_activity_at ON activity_log(at);
CREATE INDEX IF NOT EXISTS idx_activity_user ON activity_log(username, at);
...
CREATE TABLE IF NOT EXISTS connections (
    ...
    scan_time TEXT,                        -- 這個網段自己的每日排程時間 HH:MM；NULL＝跟全域排程一起掃（2026-09-12）
    last_scan_at TEXT,                     -- 這個網段上次被「自己的排程」掃的時間（判當天是否已掃）
...
INSERT OR IGNORE INTO feature_flags (module_key, label, enabled) VALUES
    ...
    ('adopt', '納入管理', 1),
    ('pipeline', '納管漏斗', 1),
    -- 2026-08-26 在線人數與操作紀錄。**預設開啟**——這跟其他模組不同：
    -- 它不是「多一個功能頁」，是「這台系統有沒有在留痕」。預設關掉的話，
    -- 出事要查的時候才發現沒開，那份 log 就永遠補不回來了。
    ('activity', '在線人數與操作紀錄', 1),
...
-- 名稱是拿同一個 api_id 底下 MIN(asset_name) 湊的——那是「隨便挑一台機器的名字
-- 當系統名」，猜對是運氣。帳號盤點要交出去的 Excel 第一欄就是 system_id + system。
--
-- 為什麼 AP 部門／負責人放這裡而不是掛在機器上：使用者提供的範例裡，這四欄在
-- 同一個系統的每一列都重複同一組值——那是業務系統的屬性，不是機器的屬性。
-- 掛在機器上會有 N 份副本，改一次要改 N 台。
-- 月報每月凍結的快照。跨月比較都用這份，不要拿「現在的資料」去比上個月——
-- 資料每天在補，那樣比出來的差異分不清是「真的變好」還是「資料變全」。
--
-- 同月重跑**覆蓋**（使用者 2026-09-08 決策 A）：月報是當月結算，中途重產是正常的。
-- 但每次覆蓋都更新 generated_at/by 並累加 regenerated_count——
-- 「這份數字什麼時候、誰產的、重產過幾次」查得到。
--
-- 只存彙總不存明細：明細是當下的事實，下個月再看已經不同，而且會讓這張表無限長大。
CREATE TABLE IF NOT EXISTS monthly_report_snapshot (
    year_month        TEXT PRIMARY KEY,   -- 2026-09
    generated_at      TEXT NOT NULL,
    generated_by      TEXT NOT NULL,
    summary_json      TEXT NOT NULL,
    regenerated_count INTEGER DEFAULT 1
);

CREATE TABLE IF NOT EXISTS business_system (
    api_id          TEXT PRIMARY KEY,   -- 對到 hardware.api_id（使用者：system_id 就是 AP_ID）
    name            TEXT,               -- 系統名稱（中文）
    ap_department   TEXT,               -- AP 部門：通知 AP 做盤點用的
    ap_owner        TEXT,               -- AP 負責人
    -- 同一個 api_id 在來源檔出現多組不同名字時：不靜默取一個（會猜錯又看不出來），
    -- needs_review=1 標「多來源·待確認」，name_candidates 存所有候選（｜分隔）供人日後挑。
    needs_review    INTEGER DEFAULT 0,
    name_candidates TEXT,
    -- 系統類別（原字串照存，例如「S1-第一類系統軟體（核心）」）。來源是使用者給的 APID
    -- 分級表，由「系統類別對照表」匯入**只更新這一欄**——不經過整列覆寫的對照表匯入，
    -- 否則 AP 部門／負責人會被那份沒有這兩欄的檔案清空。沒有值＝未分級。
    sys_class       TEXT,               -- 分級表匯入的（整份取代，程式判）
    sys_class_override TEXT,             -- 稽核人員手動改的分級；有值蓋過 sys_class，匯入不碰
    updated_at      TEXT DEFAULT (datetime('now','localtime'))
);
...
-- ===== 軟體盤點（2026-09-11）=====
-- 表名刻意不叫 software：那張是 CIA 清冊匯入的「系統軟體」分頁，意思完全不同。
-- 一列＝一台主機上的一個套件版本。key 含版本：kernel 這種會多版本並存。
CREATE TABLE IF NOT EXISTS host_package (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    ip TEXT NOT NULL,
    asset_serial TEXT,
    name TEXT NOT NULL,
    version TEXT NOT NULL DEFAULT '',
    arch TEXT NOT NULL DEFAULT '',          -- x86_64 / noarch / x64 / x86（Windows 64/32 位元機碼）
    vendor TEXT,
    installed_at TEXT,                      -- 機器講的安裝時間；講不出來留 NULL，不猜
    source TEXT NOT NULL,                   -- ssh_rpm / winrm_registry
    first_seen TEXT,
    last_seen TEXT,
    gone_at TEXT,                           -- 移除不刪，只標時間（同 host_service）
    UNIQUE(ip, name, arch, version)
);
CREATE INDEX IF NOT EXISTS idx_host_package_name ON host_package(name);
CREATE INDEX IF NOT EXISTS idx_host_package_ip ON host_package(ip);

-- 新裝／移除。每台第一次盤點不寫（那是「本來就在」，不是「剛裝」）。
CREATE TABLE IF NOT EXISTS package_change (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    ip TEXT NOT NULL,
    asset_serial TEXT,
    name TEXT NOT NULL,
    ...
    detected_at TEXT NOT NULL,              -- 哪一輪盤點發現的（不是實際安裝時間）
    run_id INTEGER
);
CREATE INDEX IF NOT EXISTS idx_package_change_when ON package_change(detected_at);

CREATE TABLE IF NOT EXISTS software_collect_runs (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    trigger TEXT,                           -- manual / schedule
    status TEXT DEFAULT 'running',          -- running / ok / failed
    host_count INTEGER,                     -- 試著收幾台
    ok_count INTEGER,                       -- 收到幾台
    package_count INTEGER,
    failed_count INTEGER,
    unsupported_count INTEGER,              -- 沒有 rpm 的機器（Debian 類），不算失敗也不算沒軟體
    error TEXT,
    started_at TEXT,
    finished_at TEXT
);
...
CREATE TABLE IF NOT EXISTS host_account (
    ...
    kind TEXT,                              -- human / service / default（程式判的，決定規則適用範圍）
    kind_override TEXT,                     -- 稽核人員手動改的類型；有值就蓋過 kind，收集不覆寫（同 note）
...
-- 特權帳號趨勢快照（2026-09-19）：host_account 只有 first_seen/last_seen/gone_at，
-- 沒有「每次採集當下有幾個特權帳號」的時間序列。這張表每次採集完存一筆現值，
-- 往後才累積得出趨勢（稽核最看「特權帳號是變多還變少」）。定義與帳號儀表板一致：
-- privileged = is_sudoer OR uid=0 OR kind='mgmt'（單一來源，見 account_inventory 快照函式）。
CREATE TABLE IF NOT EXISTS account_priv_snapshot (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    taken_at TEXT NOT NULL,
    run_id INTEGER,                          -- 對應 account_collect_runs.id（哪次採集後拍的）
    total_accounts INTEGER,
    privileged INTEGER,                      -- is_sudoer OR uid=0 OR kind='mgmt'
    sudoers INTEGER,                         -- is_sudoer=1
    uid0 INTEGER,                            -- uid=0（等同 root）
    nopasswd INTEGER,                        -- sudo_nopasswd=1（免密碼 sudo）
    mgmt INTEGER,                            -- kind='mgmt'（標準管理帳號）
    human INTEGER                            -- kind='human'（真人帳號）
);
CREATE INDEX IF NOT EXISTS idx_account_priv_snapshot_at ON account_priv_snapshot(taken_at);

-- 主機自我檢查「健康基準」快照（批次 2，2026-09-22）：每週六排程對已納管主機收一份，
-- 事發時的即時健檢跟最近一份比（方向性 diff：消失＝紅、新增＝提示）。留多份看序列變化點。
-- 只存穩態＋數值供比對；完整原始輸出即時看即可，不落 DB（見 health_baseline.py）。
CREATE TABLE IF NOT EXISTS health_snapshot (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    ip TEXT NOT NULL,
    taken_at TEXT NOT NULL,
    source TEXT,                    -- weekly（排程）／manual（值班手動釘）
    overall TEXT,
    reachable INTEGER,
    ports_json TEXT,               -- 監聽 port 清單（JSON 數字陣列），diff 用
    disks_json TEXT,               -- {掛載點: 使用率%}（JSON），看跨基準的漲幅
    mem_pct REAL,
    load_per REAL
);
CREATE INDEX IF NOT EXISTS idx_health_snapshot_ip ON health_snapshot(ip, taken_at);
...
-- 收集這一跳的 SSH 事實（2026-09-22 B-25）。**一台一列，最新的蓋掉舊的。**
--
-- 為什麼要獨立一張表而不是塞進 host_spec：這裡記的是「收集本身發生什麼事」，
-- 不是「收到什麼」。收集全失敗的時候 host_spec 根本不會有列 --
-- 而那正是最需要看到診斷的時候。
--
-- 存的是事實不是推論：離開碼、stderr 原文、指令原文。
-- 不存金鑰內容（只存路徑），不存密碼（本來就沒有，走金鑰）。
CREATE TABLE IF NOT EXISTS collect_ssh_trace (
    asset_serial TEXT PRIMARY KEY,
    ip TEXT,
    account TEXT,                -- 用哪個帳號連（AIX 是 8 字元的那個）
    key_path TEXT,               -- 用哪把金鑰（只存路徑）
    connected INTEGER,           -- 1=連得上 0=連不上 NULL=這輪沒試
    transport_error TEXT,        -- SSH 那層的原始訊息（Permission denied / timed out…）
    cmd_total INTEGER,
    cmd_ok INTEGER,              -- 離開碼 0 且有輸出
    cmd_empty INTEGER,           -- 離開碼 0 但輸出是空的（指令跑了、真的沒東西）
    cmd_failed INTEGER,          -- 離開碼非 0（指令不存在／被擋／語法錯）
    samples TEXT,                -- JSON：前幾條指令的原文、離開碼、輸出長度、stderr
    collected_at TEXT
);
...
CREATE TABLE IF NOT EXISTS network_segment (
    ...
    environment TEXT,                       -- 正規化後的環境：測試／正式
                                            -- 2026-08-26 起**優先用檔案裡的「環境別」欄**
                                            -- （UAT／PROD），空白才退回從 category 的
                                            -- UAT- 前綴推。使用者這版的檔案把 UAT 放在
                                            -- 獨立欄位、使用類別純寫 SERVER，只看前綴
                                            -- 會把 UAT 網段全部標成正式。
    environment_raw TEXT,                   -- 檔案裡的原值（UAT／PROD／看不懂的字串）
                                            -- 保留原值：認不出來時畫面要顯示得出「他填了什麼」，
                                            -- 不然人不知道要去改哪一格
    scan_excluded INTEGER DEFAULT 0,        -- 弱掃說明寫「建議排除掃描」
    scan_note TEXT,                         -- 弱掃說明原文（含伺服器/IOT 區段的細節）
    expanded_from TEXT,                     -- 這列是從原檔哪一格「多段寫法」展開出來的
                                            -- （例如 10.99.121.0/24----10.99.127.0/24 展開成 7 列）。
                                            -- 一定要留：畫面上必須看得出「原檔沒有這一列，是系統
                                            -- 拆出來的」，否則人拿系統的清單去跟 Excel 對帳會對不起來，
                                            -- 而且不知道要回頭改哪一格
    vlan TEXT,                              -- VLAN ID。存字串不存整數：原檔有「2001」也有
                                            -- 「2001,2002」「N/A」這種寫法，硬轉整數會讓
                                            -- 那幾列整個匯不進來
    remark TEXT,                            -- 註解（原檔的自由文字欄）
...
CREATE TABLE IF NOT EXISTS collect_dispatch_run (
    ...
    started_at TEXT,
    finished_at TEXT,
    -- 自動分批（2026-09-16 使用者：「你有分批嗎?」「你應該幫我自動分」）。
    -- 一次貼 1270 台以前直接被拒絕、叫人自己切；現在系統自己切成幾批依序跑，
    -- 這三欄讓畫面看得出「跑到第幾批、做完幾台」，而不是只能盯著轉圈。
    batch_total INTEGER,
...
-- ===== 一鍵批次自動納管（2026-09-06）=====
-- 每台試密碼→佈金鑰(納管)→(可選)統一密碼成 A。背景執行（500 台不能卡畫面），
-- 結果持久化，畫面輪詢＋隔天回來還看得到上次跑到哪。
-- ⚠️ 這兩張表**絕不存密碼**：只存「用第幾組(password_index)」與結果，不存任何密碼字串。
CREATE TABLE IF NOT EXISTS batch_onboard_run (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    triggered_by TEXT,                      -- 操作者帳號（稽核用）
    target_count INTEGER,
    unify INTEGER DEFAULT 0,                -- 這次有沒有勾「統一密碼成 A」
    status TEXT DEFAULT 'running',          -- running / ok / failed
    error TEXT,
    started_at TEXT,
    finished_at TEXT,
    targets_json TEXT                       -- 這批的目標清單[{ip,environment}]，供中斷後算「還剩哪幾台」＋繼續
);

CREATE TABLE IF NOT EXISTS batch_onboard_result (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    run_id INTEGER NOT NULL,
    ip TEXT NOT NULL,
    environment TEXT,          -- 這台當時的環境別（測試才會被自動跑）
    login_ok INTEGER,
    password_index INTEGER,    -- 用第幾組密碼（0=A/1=B）；NULL=都不對。**不是密碼本身**
    platform TEXT,
    onboarded INTEGER,
    password_unified INTEGER,  -- 有沒有把密碼統一成 A
    fail_stage TEXT,           -- connect / execute / unify（分清楚下一步）
    fail_reason TEXT,          -- 給人看的原因，不含密碼
    created_at TEXT DEFAULT (datetime('now','localtime'))
);
CREATE INDEX IF NOT EXISTS idx_batch_onboard_result_run ON batch_onboard_result(run_id);
...
-- SAN switch 收集結果（Brocade）。2026-09-13：機房搬遷盤點需要 WWPN/zone/port/fabric，
-- 靠 SSH 唯讀指令收集後解析存這裡。⚠️ 只存「結果」——帳密當場打、只在記憶體、不落任何欄位。
-- 一台 switch 一列（以管理 IP 為 key），data_json 放解析後的完整結果（WWPN↔別名↔port↔zone）。
CREATE TABLE IF NOT EXISTS san_switch (
    ip           TEXT PRIMARY KEY,      -- 管理 IP（納管/收集對象）
    switch_name  TEXT,
    switch_wwn   TEXT,
    zoning_cfg   TEXT,                  -- 作用中的 zone config 名
    vendor       TEXT,                  -- brocade
    port_count   INTEGER,
    zone_count   INTEGER,
    wwpn_count   INTEGER,
    data_json    TEXT,                  -- san_parse.build_inventory() 的完整結果
    raw_json     TEXT,                  -- 各唯讀指令的原始輸出，日後可重判讀不必再連 switch
    collected_at TEXT,
    collected_by TEXT
);

-- HMC（IBM Hardware Management Console）收集結果。2026-09-13：Power／AIX／IBM i 的 frame
-- 與 LPAR（CPU/記憶體/序號/韌體）從 HMC 唯讀指令收集後存這裡。⚠️ 只存結果、不存帳密。
CREATE TABLE IF NOT EXISTS hmc_console (
    ip           TEXT PRIMARY KEY,
    hostname     TEXT,
    hmc_version  TEXT,
    system_count INTEGER,
    lpar_count   INTEGER,
    data_json    TEXT,                 -- hmc_parse.build_inventory() 的完整結果
    collected_at TEXT,
    collected_by TEXT
);

-- 主機硬體規格（已納管主機用唯讀指令撈的 CPU/記憶體/Storage/型號/序號）。2026-09-13。
-- 一台一列（以資產序號為 key）。給搬遷盤點表把 CPU/RAM/Storage 那幾欄自動填上。
CREATE TABLE IF NOT EXISTS host_spec (
    asset_serial TEXT PRIMARY KEY,
    ip           TEXT,
    platform     TEXT,
    cpu          TEXT,
    cores        TEXT,
    mem_mb       INTEGER,
    storage      TEXT,               -- 一行摘要（相容用）
    storage_json TEXT,               -- 依 VG／掛載點分組：{"by":"vg","items":[{name,size_gb,free_gb}],"total_gb":N}
    psu_json     TEXT,               -- 實體機 PSU：{"count":N,"watts":[750,750]}；VM/查不到為 NULL
    spec_json    TEXT,               -- 其餘規格：CPU 詳情/kernel/uptime/GPU/RAID/缺哪些工具 tools_missing
    model        TEXT,
    serial       TEXT,
    vendor       TEXT,
    raw_json     TEXT,
...
-- 網卡明細（納管後收集）。2026-09-14 機房搬遷盤點。一台多列（每張 NIC 一列）。
-- 每欄的來源記在 sources(JSON)：{"mac":"collect","vlan":"field",...}——之後可自動只補空格＋四色。
-- 自然鍵＝(資產序號, nic_name)；MAC 為次要對帳鍵（見程式）。speed 以 Mb/s 存。
CREATE TABLE IF NOT EXISTS asset_nic (
    ...
    gateway       TEXT,
    vlan          TEXT,
    bond          TEXT,               -- bond/team 主介面或成員
    bond_mode     TEXT,
    speed_cur     INTEGER,            -- 目前協商速率 Mb/s
    speed_max     INTEGER,            -- 額定/最大速率 Mb/s（卡的能力）
    downgraded    INTEGER DEFAULT 0,  -- speed_cur < speed_max（10G 卡跑 1G）
    state         TEXT,               -- up/down
    driver        TEXT,               -- 網卡驅動（sysfs，免裝工具）
    model         TEXT,               -- 網卡型號/晶片（lspci，有裝才有）
    sources       TEXT,               -- 每欄來源 JSON
    collected_at  TEXT,
    UNIQUE(asset_serial, nic_name)
);

-- FC HBA 明細（納管後收集）。一台多列（每個 fc_host 一列）。
-- 主機端收 wwpn/wwnn/速率/link；Fabric/SAN switch/zone/target 由 SAN 收集反查補（下一刀）。
-- 自然鍵＝(資產序號, hba_name)；WWPN 為次要對帳鍵。speed 以 Gbit 存。
CREATE TABLE IF NOT EXISTS asset_hba (
    id            INTEGER PRIMARY KEY,
    asset_serial  TEXT NOT NULL,
    hba_name      TEXT,
    wwpn          TEXT,
    wwnn          TEXT,
    speed_cur     INTEGER,            -- 目前 Gbit
    speed_max     INTEGER,            -- 額定/最大 Gbit
    downgraded    INTEGER DEFAULT 0,
    port_state    TEXT,               -- Online/Linkdown/fcstat failed
    fabric        TEXT,               -- 以下由 SAN 反查補（下一刀）
    san_switch    TEXT,
    san_port      TEXT,
    zone          TEXT,
    target_wwpn   TEXT,
