...
def _dup_key(a: dict):
    """同一台的判準：主機名＋IP **都相同**才算。跟資產查詢頁的重複清單、
    納管漏斗用同一個保守定義——只有 IP 相同可能是 DHCP 回收給了別台，
    只有主機名相同可能是換網段（搬遷）。少一邊就不收。"""
    host = (a.get("hostname") or "").strip().lower()
    ip = (a.get("ip") or "").strip()
    # 0.0.0.0 是「沒 IP」的佔位，不是真的共用 IP——兩台都填 0.0.0.0 不代表同一台
    # （2026-09-17 verify_numbers 抓到：一台 Cisco 交換器兩筆都 0.0.0.0＋同主機名被誤併成一台，
    #  害 classify_assets 台數 4366 跟分佈統計 collapse_machines 的 4367 差 1）。對齊 system_stats.machine_key。
    # [B-03] 直接用全站同一把尺 machine_key（含網域處理：ap02.sit.xx.com.tw 與 ap02 同一台）；
    # 以前自己算 (host, ip)，machine_key 一改就會跟分佈統計的台數對不上
    import system_stats
    if not (host and ip and ip != "0.0.0.0"):
        return None
    return system_stats.machine_key(host, ip, None)


def _collapse_duplicates(assets: list[dict]) -> list[dict]:
    """同一台機器被登記成多筆時收成一台（2026-09-17）。

    ## 為什麼

    使用者拿這個月的平台表（合計 6084）跟上個月的報告（合計 3654）對照，問
    「這數值不對…是不是計算錯誤」。查證：`classify_assets` 是逐筆掃 `hardware`，
    **一列算一台**，而同一台機器會被 CIA／dynassets／RVTools 各登記一筆。

    證據（使用者自己的兩張畫面）：平台表寫「總計 8,085 台」，同一份資料的納管漏斗
    寫「共 5,405 台（已收起 2,681 筆重複登記）」——差額正好是重複登記數。
    納管漏斗 v1.188.0 已改成照主機算，**報告這邊漏掉了**，兩邊講的不是同一件事。

    ## 留哪一筆

    留**資料最完整**的那筆（有 OS、有型號、非退役優先）——留到空白那筆會讓這台
    掉進「需確認」，等於憑空生出一個待辦。其餘序號掛在 `dup_serials`，下鑽看得到。

    ⚠️ **不合併資料、不刪任何列**：只在統計與清單上收成一台。要留哪一筆是人的決定
    （資產查詢頁的重複清單才是處理入口）。
    """
    ...
    for a in assets:
        ...
        if key is None:
            a["dup_count"] = 1
            a["dup_serials"] = [a["asset_serial"]] if a.get("asset_serial") else []
            out.append(a)
            continue
        prev = kept.get(key)
        if prev is None:
            ...
            out.append(a)
            continue
        if a.get("asset_serial"):
            prev["dup_serials"].append(a["asset_serial"])
        prev["dup_count"] = len(prev["dup_serials"]) or prev["dup_count"] + 1
        if _completeness(a) > _completeness(prev):
            # 換成資料比較完整的那筆當代表，但保留已經累積的重複資訊
            dup_count, dup_serials = prev["dup_count"], prev["dup_serials"]
            idx = out.index(prev)
            a["dup_count"], a["dup_serials"] = dup_count, dup_serials
            out[idx] = a
            kept[key] = a
    return out
...
def _completeness(a: dict) -> tuple:
    """哪一筆「資料比較完整」。非退役優先——報廢那筆的 OS 常常是舊的。"""
    return (
        0 if a.get("retired") else 1,
        1 if a.get("os_canonical") else 0,
        1 if a.get("device_model") else 0,
        1 if a.get("environment") else 0,
        1 if a.get("location") else 0,
    )
...
def platform_lifecycle(conn: sqlite3.Connection, detail: bool = False) -> dict[str, Any]:
    ...
    retired = 0
    # 收起了幾筆重複登記——數字變動要講得出原因，不能靜默少掉兩千多台
    duplicates_merged = sum((a.get("dup_count") or 1) - 1 for a in assets)
    ...
    result = {
        ...
        # 2026-09-17：照主機算（同主機名＋IP 收成一台），跟納管漏斗同一個口徑。
        # 以前照筆數算，同一台被 CIA／dynassets／RVTools 各登記一筆就重複計算。
...
def drill_platform(conn: sqlite3.Connection, platform: str | None = None,
                   ...
                   retired: bool = False, os_canonical: str | None = None,
    ...
    registered_only=True → 只算列入本報告的（a["platform"] 有值）。表1 上方的
    狀態摘要卡（已EOS／一年內EOS…）是把各平台列同一欄加總來的，那個加總只算
    列入報告的平台；若不設這個旗標、直接用 status 下鑽，會把「不列入本報告」的
    那批（網路設備等，也各有 eos_status）也撈進來，筆數就比卡片上的數字多——
    違反「加總＝下鑽同一份口徑」。所以摘要卡下鑽一律帶這個旗標。
    """
    ...
    for a in items:
        ...
        if bucket is not None:
            ...
            continue
        elif registered_only and a["platform"] is None:
            continue
...
def off_book_sql(col: str = "asset_serial") -> str:
    """SQL 版「帳外」判定（DYN-/VC-/AUTO- 前綴）。跟 is_off_book 同義、同一份前綴。
    以前 api.py 把 `LIKE 'DYN-%' OR ...` 直接寫死在查詢裡，前綴一改就漂走——要在 SQL
    裡判帳外就呼叫這支。回傳的是「是帳外」的條件；要「排除帳外」就 NOT(...)。

    ⚠️ 用 GLOB 不用 LIKE：SQLite 的 LIKE 預設**不分大小寫**，但正典 is_off_book 用
    Python startswith（**分**大小寫）。GLOB 分大小寫，兩邊才會逐筆一致
    （test_canonical_sql 會擋）。"""
    return "(" + " OR ".join(f"{col} GLOB '{p}*'" for p in OFF_BOOK_PREFIXES) + ")"
