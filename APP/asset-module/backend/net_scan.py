...
# 需要什麼就探什麼（2026-09-15 使用者，正式區開通申請）：22/5985 收集用、445/3389 判 Windows。
# 80/443/8000/3000/8443 只是線索、161 是 UDP 服務（TCP 探它沒意義）→ 拿掉，申請書與程式一致。
PROBE_PORTS = (22, 445, 3389, 5985)


def _probe_host(ip: str, ports=PROBE_PORTS, timeout: float = 0.6, icmp=True) -> list[int] | None:
    """回傳該 IP 有回應的 open port 清單；主機沒回應回 None（=掃不到）。
    被拒(closed)也算主機活著，但這裡只收 open port 當佐證；只要有任一 port 非逾時就視為活著。

    2026-09-16 使用者拿 `ping` 的輸出問「怎麼會判失聯?」——那台 Redhat ping 得通，
    但 22/445/3389/5985 全關，只探 TCP 就會判定它不存在，接著被算成失聯。
    **所以 TCP 全逾時之後補一次 ICMP**：會回 ICMP 就是活著（回空清單＝活著但沒有開放埠）。

    ICMP 只在 TCP 完全沒反應時才送，正常有服務的機器不會多花這一次。
    """
    open_ports: list[int] = []
    alive = False
    for port in ports:
    ...
    if alive:
        return open_ports
    # TCP 全逾時 → 補一次 ICMP。ping 不通才真的當作掃不到。
    if icmp and host_ping_alive(ip):
        return []
    return None


def host_ping_alive(ip: str, timeout: float = 1.0) -> bool:
    """送一個 ICMP 封包看有沒有回。跑不起來（沒裝 ping／沒權限）一律回 False，
    不要因為工具缺席就把機器說成活著——那會製造假資料。"""
    import os
    import subprocess

    cmd = (["ping", "-n", "1", "-w", str(int(timeout * 1000)), ip] if os.name == "nt"
           else ["ping", "-n", "-c", "1", "-W", str(max(int(timeout), 1)), ip])
    try:
        return subprocess.run(cmd, capture_output=True, timeout=timeout + 3).returncode == 0
