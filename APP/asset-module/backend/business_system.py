...
def list_all(conn: sqlite3.Connection) -> list[dict]:
    ...
    asset_count 是「台」不是「筆」：一台掛多 VIP／多筆登記在同一個 api_id 底下只算一台，
    跟全站台數口徑一致（走正典 system_stats.machine_key；含網域正規化，SQL 無法忠實複製
    那段正則，所以在 Python 算）。
    """
    import system_stats
    from collections import defaultdict
    hosts_of: dict[str, set] = defaultdict(set)
    for r in conn.execute(
        "SELECT api_id, hostname, ip, asset_serial FROM hardware "
        "WHERE api_id IS NOT NULL AND TRIM(api_id) != ''"
    ):
        hosts_of[r["api_id"]].add(
    ...
    return [{**dict(r), "asset_count": len(hosts_of.get(r["api_id"], ()))} for r in rows]
