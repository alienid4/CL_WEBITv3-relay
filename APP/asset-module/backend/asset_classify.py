...
def platform_of_row(row, guesses: dict, conn=None) -> str:
    """單列資產的平台大類。沿用 manage_state 的判準，避免兩邊分岔成兩套規則。

    優先序：hardware.os（收到的真 OS，用 normalize_os() 那套判斷）> 掃描 os_guess
    （推測）> device_model 關鍵字（見 manage_state.platform_of_from_os()）。
    """
    import manage_state

    guess_os = guesses.get(row["ip"], (None, None))[0] if row["ip"] else None
    return manage_state.platform_of_from_os(row["os"], row["device_model"], conn, guess_os)
...
#: Linux 的細分（沿用原本平台判斷的發行版分類）
LINUX_DETAILS = ("RHEL", "CentOS", "Debian", "Oracle Linux", "Linux(其他)")


def classify_all(conn) -> dict[str, dict]:
    """全部資產的 OS 類型（大類）＋ Linux 細分＋角色，key 是 asset_serial。

    2026-09-18 使用者：「我們平台不是還有，這部分是要對應嗎？」——資產查詢的「平台」
    跟漏斗的「OS 類型」是兩套規則（網路設備一邊 666 筆、一邊 581 台，還各有對方沒有的類）。
    改成兩層：
    - **os_type（大類）**＝漏斗那套 pipeline._os_type，並套用人工指定（os_override）——只有一套
    - **os_detail（細分）**＝只有 Linux 才有：原本平台判斷的發行版（RHEL／CentOS…）；
      原本判斷給不出發行版的歸「Linux(其他)」
    - platform 保留原值，給舊連結（?platform=）與相容用
    """
    import os_override
    import pipeline
    ...
    out = {}
    for r in conn.execute(
        "SELECT asset_serial, ip, hostname, asset_name, os, device_model FROM hardware"
    ).fetchall():
        plat = platform_of_row(r, guesses, conn)
        auto = pipeline._os_type(r["os"], None, r["device_model"], (r["hostname"], r["asset_name"]))
        os_type = os_override.resolve(ov_map, r["hostname"], r["ip"], r["asset_serial"], auto)[0]
        detail = None
        if os_type == pipeline.OS_LINUX:
            detail = plat if plat in LINUX_DETAILS else "Linux(其他)"
        out[r["asset_serial"]] = {
            "platform": plat,
            "os_type": os_type,
            "os_detail": detail,
