...
def _is_cidr(target: str) -> bool:
    ...
    `http://10.99.18.35:3001`，裡面也有 '/'，於是一個 HTTP 網址被當成 CIDR
...
def scan_targets(conn, only_ids: set[int] | None = None) -> list[NetworkSweepSource]:
    """回傳要掃的網段來源。only_ids 給定時只回那些 connection（各網段自訂排程用）；
    不給＝全部（手動掃／全域排程）。fallback 到本機網段只在「全掃且一個來源都沒有」時才發生。"""
    ...
    if not sources and only_ids is None:
        sub = _local_subnet()
