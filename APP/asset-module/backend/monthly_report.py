...
@_section("資產總覽", "—", "hardware（照主機去重算台，跟報表/漏斗同口徑）")
def _assets(conn) -> dict:
    # ⚠️ 台數要「照主機去重」，不是 COUNT(*)——2026-09-17 verify_numbers 抓到：原本 COUNT(*)
    # 把 4785 筆（含 419 筆重複登記）當「總台數」餵進 AI 月報，實際台數是 4367。掛筆數當台數，
    # 主管月報就多算了 419 台。改用 classify_assets（跟頁A/B、漏斗同一份去重）。
    import system_report as sr
    import manage_state
    ca = sr.classify_assets(conn)
    total = len(ca)
    rows_total = _one(conn, "SELECT COUNT(*) FROM hardware") or 0
    hwvm = {r["asset_serial"]: (r["is_vm"], r["device_model"])
            for r in conn.execute("SELECT asset_serial, is_vm, device_model FROM hardware")}
    by_env: dict[str, int] = {}
    vm = 0
    for a in ca:
        env = a.get("environment") or "(未填)"
        by_env[env] = by_env.get(env, 0) + 1
        isv, dm = hwvm.get(a["asset_serial"], (None, a.get("device_model")))
        if manage_state.is_vm_value(isv, dm):
            vm += 1
    by_env = dict(sorted(by_env.items(), key=lambda kv: -kv[1]))
    return {"總台數": total, "登記筆數": rows_total, "依環境": by_env,
            "虛實": {"VM": vm, "實體/未標": total - vm}, "_has_data": total > 0}
