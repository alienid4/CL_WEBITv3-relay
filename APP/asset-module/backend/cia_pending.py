    ...
    try:
        for s in uniq:
            ...
            conn.execute("UPDATE hardware SET asset_status = ?, manual_updated_at = ? WHERE asset_serial = ?",
                         (status, now, s))
            # [2026-09-20 使用者拍板 A] 整台下線連帳外（DYN-/VC-/AUTO-，系統自己從 DY／vCenter
            # 產出來的登記）一起改——否則同一台一筆停用、一筆使用中，會永遠掛在「登記矛盾」。
            # 但「CIA 待異動」只對 CIA 那幾筆記：帳外根本不在公司清冊上，沒有「回頭改清冊」這件事，
            # 記了只會在待辦裡製造永遠做不完的項目。
            import system_report as _sr

            if not _sr.is_off_book(s):
