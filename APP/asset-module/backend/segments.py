...
def import_segments(conn: sqlite3.Connection, path: Path) -> dict:
    ...
    **整批取代不是 upsert**：Excel 是這份清單的唯一真相，段被刪掉就該從系統消失。
    ...
    conn.commit()
    # 自動維護掃描範圍：重匯後新符合規則的段自動納入、不再符合的停用，不用人回來重勾
    # （這正是規則式範圍存在的理由——填掉「新段沒人勾→整批假失聯」的坑）。
    try:
        import scan_scope
        scan_scope.apply_policy(conn)
    except Exception as exc:  # noqa: BLE001 - 同步失敗不該讓匯入整個失敗，但要留痕
        import logging
        logging.getLogger(__name__).warning("匯入後自動同步掃描範圍失敗：%s", exc)
    return {
