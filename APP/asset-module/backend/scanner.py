...
def run_scan(sources: list[ScanSource], conn: sqlite3.Connection) -> dict:
    ...
    # 先記「這次打算涵蓋哪些網段」（2026-09-16）。有這份紀錄才分得出
    # 「掃了但這台沒回應（失聯）」與「這段根本沒掃（未涵蓋）」——
    # 以前兩者都被算成失聯，使用者 ping 得通的機器也被判死（見 scan_scope 的說明）。
    scan_scope.record_coverage(conn, scan_stamp, sources)
    ...
    for source in sources:
        try:
            ...
            # 掃描失敗不等於掃過了：標 ok=0，那段裡的機器不該被判成失聯
            scan_scope.mark_failed(conn, scan_stamp, source.name)
            conn.execute(
