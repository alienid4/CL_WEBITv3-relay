...
_COLUMN_CANDIDATES: dict[str, tuple[str, ...]] = {
    ...
    "cluster": ("Cluster",),
    # 這份匯出是從哪個管理端抓的。RVTools 每一列都會帶這欄（vInfo 分頁），
    # 值是它當時連線的目標——**可能是 vCenter，也可能是單台 ESXi**，所以欄位名不叫
    # vcenter。上面已經警告過不可用 "VI SDK UUID"（那是伺服器的 UUID）；
    # 這裡要的是 Server 那一欄，兩者只差一個字，抓錯就會存進一串沒人看得懂的 UUID。
    "vi_sdk_server": ("VI SDK Server",),
...
FACT_FIELDS = ("hostname", "ip", "os", "vm_uuid", "is_vm", "vi_sdk_server")
...
def parse_rvtools(xlsx_path: Path) -> list[dict[str, Any]]:
    ...
    try:
        ...
        # 原始表頭（保留原文，供「整列原樣存」用）。2026-09-08 使用者定案：
        # 「連存錢哪一天會用到都不知道，不要只存你自己想知道的」——vInfo 除了挑進
        # hardware 的核心欄位，其餘欄位也要一起留（存進 source_record 的 raw），
        # 之後哪個欄位要用再從 payload 挑，不預先丟棄。
        raw_headers = [
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            ...
            # 整列原始全欄（原文表頭 → 值），一欄不漏。之後要用哪欄再從 payload 挑。
            raw = {}
            ...
            rec = {
                ...
                # 這份匯出是從哪個管理端抓的（可能是 vCenter 也可能是單台 ESXi）。
                # 舊版 RVTools 沒有這欄 -> None，畫面要能分辨"檔案沒這欄"與"這台不是匯進來的"。
                "vi_sdk_server": get("vi_sdk_server"),
                # vInfo 這一列的原始全欄（一欄不漏），存進 source_record.payload。
                # identity.resolve / FACT_FIELDS 只讀上面挑好的鍵，不會碰到 raw。
                "raw": raw,
            }
            # 範例檔的範例列（VM 名稱以「範例」開頭）不算一台
            from import_templates import is_example
            if is_example(rec["vm_name"]):
                continue
            # 整列連個名字都沒有就不算一台
...
def import_rvtools(xlsx_path: Path, conn: sqlite3.Connection) -> dict[str, Any]:
    ...
    errors: list[str] = []
    # 同一台 VM 這次匯進來的管理端跟上次不一樣（跨 vCenter vMotion、或兩座 VC 都看得到
    # 同一台）。**不能靜默覆蓋**：人會以為它一直在新那座，而「什麼時候搬的」正是
    # 事故調查最想知道的事。後匯的值還是會蓋上去（它比較新），但要在匯入結果講出來。
    vc_moved: list[dict[str, str]] = []
        ...
        try:
            ...
            if match.status == identity.MATCHED:
                existing = conn.execute(
                    "SELECT asset_serial, vi_sdk_server FROM hardware WHERE id = ?",
                    (match.hardware_id,)
                ).fetchone()
                if existing is None:
                    errors.append(f"{rec.get('vm_name') or rec.get('hostname')}：對到的資產已不存在，略過")
                    continue
                old_vc, new_vc = existing["vi_sdk_server"], rec.get("vi_sdk_server")
                if old_vc and new_vc and old_vc != new_vc:
                    vc_moved.append({
                        "asset_serial": existing["asset_serial"],
                        "name": rec.get("vm_name") or rec.get("hostname") or "",
                        "from": old_vc, "to": new_vc,
                    })
    ...
    # 這份匯出是從哪個管理端抓的。正常情況只會有一個值（一份 RVTools 匯出＝連一個
    # 管理端），出現兩個以上代表這份檔被人合併過——那會讓「這台在哪座 VC」失去意義，
    # 要講出來而不是安靜地各存各的。
    servers = sorted({r["vi_sdk_server"] for r in records if r.get("vi_sdk_server")})
    return {
        ...
        # 來源管理端（RVTools 的 VI SDK Server）。空清單＝這份匯出沒有那一欄
        # （舊版 RVTools），**不是**「查過沒有 vCenter」——畫面要分開講。
        "vi_sdk_servers": servers,
        # 換了管理端的 VM。不是錯誤，是要人知道的事實（跨 VC vMotion）。
        "vc_moved": vc_moved,
    }
