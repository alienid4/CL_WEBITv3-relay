...
def _migrate(conn: sqlite3.Connection) -> None:
    ...
    # 這一台的收集帳號（NULL ＝ 沿用全域設定）。2026-09-24 收集帳號統一成 webit3sc
    # 的遷移需要它：沒有這一欄就只能全域一次翻，而全域一次翻＝ 09-23 那個
    # 「好好的機器變成 0/9」的機隊放大版。驗過的那一台才寫新帳號，其餘不受影響。
    if "collect_account" not in cols:
        conn.execute("ALTER TABLE hardware ADD COLUMN collect_account TEXT")
    ...
    # san_switch 補 raw_json（2026-09-13：既有庫的表是 v1.144 建的、沒這欄；存原始輸出用）
    try:
        san_cols = {r[1] for r in conn.execute("PRAGMA table_info(san_switch)")}
        if san_cols and "raw_json" not in san_cols:
            conn.execute("ALTER TABLE san_switch ADD COLUMN raw_json TEXT")
    except sqlite3.OperationalError:
        pass

    # 收集分派：自動分批的進度欄（2026-09-16「你應該幫我自動分」）。既有庫沒這三欄。
    try:
        d_cols = {r[1] for r in conn.execute("PRAGMA table_info(collect_dispatch_run)")}
        if d_cols:
            for col, decl in (("batch_total", "INTEGER"), ("batch_done", "INTEGER DEFAULT 0"),
                              ("done_count", "INTEGER DEFAULT 0")):
                if col not in d_cols:
                    conn.execute(f"ALTER TABLE collect_dispatch_run ADD COLUMN {col} {decl}")
    except sqlite3.OperationalError:
        pass
    ...
    # 業務系統對照表：同一 api_id 多組名字要標「多來源·待確認」，不靜默蓋（既有 DB 補欄）
    bs_cols = {r[1] for r in conn.execute("PRAGMA table_info(business_system)")}
    if "needs_review" not in bs_cols:
        conn.execute("ALTER TABLE business_system ADD COLUMN needs_review INTEGER DEFAULT 0")
    if "name_candidates" not in bs_cols:
        conn.execute("ALTER TABLE business_system ADD COLUMN name_candidates TEXT")
    # 系統類別（第一／二／三類，2026-09-10）：公司機與 221 都是既有 DB，要補欄
    if "sys_class" not in bs_cols:
        conn.execute("ALTER TABLE business_system ADD COLUMN sys_class TEXT")
    # 系統分級的手動覆寫（2026-09-11）：分級表是整份取代匯入，會把手動分級沖掉；
    # 存成獨立欄、匯入永不碰它，顯示時覆寫優先（同 host_account.kind_override）。
    if "sys_class_override" not in bs_cols:
        conn.execute("ALTER TABLE business_system ADD COLUMN sys_class_override TEXT")

    # 主機規格：Storage 依 VG／掛載點分組、PSU、其餘規格 spec_json（2026-09-14；既有 DB 補欄）
    try:
        hs_cols = {r[1] for r in conn.execute("PRAGMA table_info(host_spec)")}
        if hs_cols:
            for col in ("storage_json", "psu_json", "spec_json"):
                if col not in hs_cols:
                    conn.execute(f"ALTER TABLE host_spec ADD COLUMN {col} TEXT")
    except sqlite3.Error:
        pass
    # NIC 明細補欄（driver/model；asset_nic 是 v1.167 才建，既有 DB 要補）
    try:
        nic_cols = {r[1] for r in conn.execute("PRAGMA table_info(asset_nic)")}
        if nic_cols:
            for col in ("driver", "model"):
                if col not in nic_cols:
                    conn.execute(f"ALTER TABLE asset_nic ADD COLUMN {col} TEXT")
    except sqlite3.Error:
        pass
    # 批次納管：存目標清單，供中斷後算「還剩哪幾台」＋繼續（2026-09-17；既有 DB 補欄）
    try:
        bo_cols = {r[1] for r in conn.execute("PRAGMA table_info(batch_onboard_run)")}
        if bo_cols and "targets_json" not in bo_cols:
            conn.execute("ALTER TABLE batch_onboard_run ADD COLUMN targets_json TEXT")
    except sqlite3.Error:
        pass
    ...
    # 各網段自己的排程時間（2026-09-12）：既有 DB 補欄。NULL＝跟全域排程一起掃。
    for col in ("scan_time", "last_scan_at"):
    ...
    if acct_tables:
        ...
        # kind_override＝稽核人員手動改的帳號類型（程式判錯時蓋過 kind，收集永不覆寫，同 note）。
        for col in ("login_source", "os_family", "os_id", "os_version", "note", "kind_override"):
...
def set_connection_scan_time(conn: sqlite3.Connection, connection_id: int, scan_time: str | None) -> None:
    """設這個網段自己的每日排程時間（HH:MM）；None／空＝清掉、回到跟全域排程一起掃。"""
    t = (scan_time or "").strip() or None
    conn.execute(
        "UPDATE connections SET scan_time = ?, updated_at = datetime('now','localtime') WHERE id = ?",
        (t, connection_id),
    )
    conn.commit()


def mark_connection_scanned(conn: sqlite3.Connection, connection_id: int, when: str) -> None:
    """記錄這個網段被自己的排程掃過的時間（判當天是否已掃，避免同一天重複觸發）。"""
    conn.execute("UPDATE connections SET last_scan_at = ? WHERE id = ?", (when, connection_id))
    conn.commit()
