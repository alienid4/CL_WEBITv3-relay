...
def collect(runner, host: str, platform: str, is_vm: bool = False) -> dict:
    ...
    if platform == "aix":
        ...
        spec = {
            ...
            "model": p.get("model"), "serial": p.get("serial"), "vendor": "IBM",
        }
...
def _platform_of(conn, ip: str, os_val: str | None) -> str:
    """AIX 用 os 欄位判（埠指紋分不出 AIX 與 Linux，兩者都 SSH 22）；Windows 用指紋；其餘 linux。"""
    if os_val and "aix" in os_val.lower():
        return "aix"
    import service_inventory
    return "windows" if service_inventory._platform_for(conn, ip) == "windows" else "linux"
...
def run_collection(conn, trigger: str = "manual", only_serial: str | None = None) -> dict:
    ...
    q = ("SELECT asset_serial, ip, os, is_vm, device_model FROM hardware "
    ...
    for t in targets:
        ...
        platform = _platform_of(conn, ip, t["os"])
        if platform == "windows":
        ...
        try:
            runner = manage_state._runner_for(ip, manage_state.COLLECTOR_KEY_DEFAULT, account=account)
