...
def eos_hosts(conn) -> list[dict]:
    ...
    for r in rows:
        ...
        if not (os_hit or {}).get("eos") and not (hw_hit or {}).get("eos"):
            continue
        out.append({
            ...
            "os_eos": (os_hit or {}).get("eos"),
            "os_status": eos.eos_status((os_hit or {}).get("eos")) if os_hit else None,
            "hw_eos": (hw_hit or {}).get("eos"),
            "hw_status": eos.eos_status((hw_hit or {}).get("eos")) if hw_hit else None,
        })
    return out
