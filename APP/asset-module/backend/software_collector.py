...
# AIX 的軟體有兩套並存，**只收一套就會少一半**：
#   · 原生 fileset（`lslpp`）——bos.*、openssh.base 這些，AIX 自己的安裝單位
#   · AIX Toolbox 的 rpm（`rpm -qa`）——openssl、python 之類，不是每台都有
# 兩套都吐同一種 PKG 行格式，共用 Linux 那支解析器。
#
# ksh88 注意：沒有 `command -v`，要用 `whence`；`lslpp` 一定存在（OS 內建），
# 所以 SRC= 直接寫 lslpp，rpm 只是加分。
#
# `lslpp -Lc` 是冒號分隔：欄1=Package Name、欄2=Fileset、欄3=Level、欄5=State。
# 取 Fileset 當名稱（那才是實際安裝單位），Package Name 放進「廠商／產品」欄
# 讓人看得出它屬於哪個產品。第一行以 # 開頭是表頭，跳掉。
AIX_CMD = (
    "echo 'SRC=lslpp'; "
    "lslpp -Lc 2>/dev/null | awk -F: '"
    "/^#/ {next} "
    "NF >= 3 && $2 != \"\" {printf \"PKG\\t%s\\t%s\\t%s\\t%s\\t%s\\n\", $2, $3, \"\", \"\", $1}'; "
    "if whence rpm >/dev/null 2>&1; then "
    "LC_ALL=C rpm -qa --qf 'PKG\\t%{NAME}\\t%{VERSION}-%{RELEASE}\\t%{ARCH}\\t%{INSTALLTIME}\\t%{VENDOR}\\n' "
    "2>/dev/null | grep -v '^PKG\tgpg-pubkey\t'; fi"
)

PLATFORM_CMD = {"linux": LINUX_CMD, "aix": AIX_CMD}
...
def collect(runner, host: str, platform: str = "linux") -> dict:
    """用 SSH runner 收套件清單。回傳 {source, packages}；判定交給寫入層。

    ⚠️ 2026-09-22 B-24：以前這支沒有 platform 參數，**一律跑 Linux 那條**。
    AIX 上 `command -v` 不是 ksh88 的內建指令，整條命令直接報錯，
    於是 `SRC=none` -> 寫入層當成「這台沒有 rpm，不支援」。
    使用者看到的是「軟體 0」，而真相是 AIX 有 `lslpp`，我們從來沒問過它。
    """
    cmd = PLATFORM_CMD.get(platform)
    if cmd is None:
        # 認不得的平台就講認不得，不要拿 Linux 的指令去碰運氣
        return {"source": None, "packages": [],
                "error": f"沒有 {platform} 的軟體盤點指令"}
    source, packages = parse_packages(runner(host, cmd))
