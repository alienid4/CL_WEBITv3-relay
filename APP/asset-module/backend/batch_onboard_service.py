...
def start(conn, ips: list[str], username: str, passwords: list[str],
    ...
    now = _now_local()
    # 存這批的目標清單：中斷後（process 重啟）才算得出「還剩哪幾台」並讓人繼續。只存 ip/環境，無密碼。
    import json as _json
    targets_json = _json.dumps(
        [{"ip": t["ip"], "environment": t.get("environment")} for t in run_targets],
        ensure_ascii=False)
    cur = conn.execute(
        "INSERT INTO batch_onboard_run (triggered_by, target_count, unify, status, started_at, targets_json) "
        "VALUES (?,?,?,?,?,?)",
        (triggered_by, len(run_targets), 1 if unify_password else 0, "running", now, targets_json))
...
def status(conn) -> dict:
    ...
    # 中斷判定：DB 說 running，但這個 process 沒有在跑（重啟/當掉）→ 是「已停下」不是「還在等」。
    interrupted = (run["status"] == "running" and not is_running())
    # 還剩哪幾台（存的目標清單扣掉已有結果的）——給「繼續剩下的」用。
    remaining: list[dict] = []
    if interrupted:
        done_ips = {r["ip"] for r in results}
        try:
            import json as _json
            targets = _json.loads(run["targets_json"]) if ("targets_json" in run.keys() and run["targets_json"]) else []
        except (ValueError, TypeError):
            targets = []
        remaining = [t for t in targets if t.get("ip") not in done_ips]
    return {
        "running": is_running(),
