...
#: Storage／SAN 的判斷只有這一條，分佈統計（系統名）與漏斗 OS 類型（OS／設備機型）共用。
#: 2026-09-18 使用者：HP SAN Switch、IBM Storage、EMC Storage、Synology「還是分到 storage/san」
#: ——加入 synology／nas（加之前在 221 量過：沒有任何業務系統的分群因此改變）。
STORAGE_RE = re.compile(r"san\s*switch|storage|儲存|\bemc\b|netapp|\bsan\b|synology|\bnas\b", re.I)

_INFRA_GROUP_RULES = (
    (GROUP_STORAGE, STORAGE_RE),
...
def _system_classes(conn) -> dict[str, dict]:
    ...
    try:
        ...
        return {}          # 舊 DB 還沒有 sys_class 欄（見 docstring）→ 當全部未分級，不讓整頁掛掉
    out: dict[str, dict] = {}
    for r in rows:
...
#: [B-03] 「短名.多段.英文頂級網域」才算帶網域（例：ap02.sit.example.com.tw → ap02）。
#: ⚠️ 不可以「見點就砍」：交換器堆疊成員「…-254.254-1」「…-254.254-2」共用管理 IP 但是不同設備，
#: 點後面不是英文頂級網域，必須維持原名。這條判斷式跟 AI/B驗收.py 的 B-03b／c 同一條。
DOMAIN_RE = re.compile(r"^([^.]+)\.((?:[a-z0-9-]+\.)+[a-z]{2,})$", re.I)


def strip_domain(hostname: str | None) -> str:
    """去掉真的網域（只有符合 DOMAIN_RE 才去）；其餘原樣回傳（不改大小寫）。"""
    h = (hostname or "").strip()
    m = DOMAIN_RE.match(h)
    return m.group(1) if m else h


def machine_key(hostname: str | None, ip: str | None, asset_serial: str | None) -> str:
    """「同一台機器」的鍵：主機名（不分大小寫）＋IP；缺任一個（或 IP 是 0.0.0.0 佔位）
    就退回資產序號，當成獨立的一台。

    為什麼要有（2026-09-14 使用者拍板「台數（筆數）」）：CIA 清冊是「每個服務／VIP 一筆」，
    一台 VM 可能登記 12 筆。用筆數算台數，同一台在同一個系統底下會被算成好幾台。
    缺主機名或 IP 的無法判斷是不是同一台，寧可各算一台（台數偏多）也不要亂併。
    """
    # [B-03] 帶網域與不帶網域的同名同 IP 是同一台（221 上 3 組被拆成兩台）
    h = strip_domain(hostname).lower()
    i = (ip or "").strip()
    if h and i and i != "0.0.0.0":
        return f"{h}|{i}"
    return f"sn:{asset_serial}"


def collapse_machines(rows) -> dict[str, list]:
    """把資產列依 machine_key 分組；每組依序號排，第一筆當代表（環境／機房取它的）。

    代表列的選法固定（最小序號），同一份資料每次算出來一樣，各張表的合計才對得起來。
    同一台在不同筆填了不同環境是資料矛盾，那要去修資料，這裡不猜哪筆對。
    """
    out: dict[str, list] = {}
    for r in rows:
        out.setdefault(machine_key(r["hostname"], r["ip"], r["asset_serial"]), []).append(r)
    for group in out.values():
        group.sort(key=lambda x: x["asset_serial"] or "")
    return out


def _machine_role(group: list, services: dict, onboarded: set, default_ap: bool) -> str:
    """一台機器（可能好幾筆登記）的角色：把每筆的用途／名稱、每筆的服務證據合起來判。"""
    fields: list = []
    svc: list[str] = []
    for r in group:
        fields += [r["asset_purpose"], r["asset_name"]]
        svc += services.get(r["asset_serial"]) or []
    rep = group[0]
    fields += [rep["device_model"], rep["os"]]
    return guess_role(*fields, onboarded=(rep["ip"] in onboarded),
                      services=svc or None, default_ap=default_ap)


def mapping_diagnosis(conn) -> dict:
    """資產的 AP ID 跟業務系統對照表對不起來時，**說得出是哪一種原因**。

    2026-09-14 公司機：對照表有 169 筆、資產 4,155 台，分佈統計卻「涵蓋 0 台」。
    對不上有好幾種完全不同的原因，要做的事也不同，只給一個 0 會讓人只能猜：
      · 對照表是空的 → 去匯對照表
      · 資產的 AP ID 全空白 → CIA 匯入時沒對到 API ID 欄
      · 只差空白／大小寫 → 寫法問題
      · 兩邊代碼長得完全不一樣 → 多半是對照表匯入時抓錯欄
    附兩邊的代碼樣本，人一眼就看得出是哪一種。
    """
    bs = [str(r[0]) for r in conn.execute("SELECT api_id FROM business_system").fetchall()]
    hw = conn.execute(
        "SELECT api_id, COUNT(*) AS n FROM hardware "
        "WHERE api_id IS NOT NULL AND length(trim(api_id)) > 0 "
        "GROUP BY api_id ORDER BY n DESC").fetchall()
    hw_rows = sum(r[1] for r in hw)
    total = conn.execute("SELECT COUNT(*) FROM hardware").fetchone()[0]
    bs_exact = set(bs)
    bs_norm = {b.strip().upper() for b in bs}
    matched_exact = sum(r[1] for r in hw if r[0] in bs_exact)
    matched_norm = sum(r[1] for r in hw if str(r[0]).strip().upper() in bs_norm)
    if not bs:
        reason = "業務系統對照表是空的，請先到「資料匯入」匯入業務系統對照表。"
    elif not hw_rows:
        reason = "資產的 AP ID 欄全部空白——CIA 匯入時可能沒對到「API ID」欄。"
    elif matched_exact == 0 and matched_norm > 0:
        reason = "代碼只差在前後空白或大小寫，兩邊寫法不一致。"
    elif matched_exact == 0:
        reason = ("兩邊的代碼完全對不起來（看下面的樣本）。最常見是業務系統對照表匯入時抓錯欄——"
                  "代碼欄應該是 N-xxx／I-xxx 這種 AP ID。")
    else:
        reason = None
    return {
        "business_system_rows": len(bs),
        "assets_total": total,
        "assets_with_api_id": hw_rows,
        "matched_rows": matched_exact,
        "matched_rows_if_normalized": matched_norm,
        "reason": reason,
        "sample_table_codes": sorted(bs)[:8],
        "sample_asset_codes": [str(r[0]) for r in hw[:8]],
    }
...
def by_system(conn, limit: int | None = 10, kind: str | None = None) -> dict:
    ...
    """
    rows = conn.execute("""
        SELECT h.asset_serial, h.hostname, h.ip, h.environment AS env_raw,
               b.api_id, b.name, b.ap_department
        FROM hardware h
    ...
    for r in rows:
        if kind is not None and system_kind(r["api_id"]) != kind:
            continue
        per_sys.setdefault(r["api_id"], []).append(r)

    systems: dict[str, dict] = {}
    for api, sys_rows in per_sys.items():
        m = sys_rows[0]
        cinfo = classes.get(api) or {}
        s = systems[api] = {
            "api_id": api, "name": m["name"], "ap_department": m["ap_department"],
            "kind": system_kind(api),
            # 基礎設施再分群（網路設備／Storage／其他大型系統／未分群）。
            # AP 系統那邊用不到，但一律帶著，畫面自己決定要不要用。
            "group": infra_group(api, m["name"]),
            "sys_class": cinfo.get("cls", CLASS_UNRATED),
            # 第一類裡還分「核心」與否——原檔有這個資訊，不丟
            "sys_class_core": cinfo.get("core", False),
            "sys_class_override": cinfo.get("override"),
            "sys_class_computed": cinfo.get("computed", CLASS_UNRATED),
            # total／by_env 是**台數**（同一台多筆只算一台）；rows 是登記筆數。
            # 兩個都給——使用者 2026-09-14：資料一筆不少，畫面顯示「台數（筆數）」。
            "total": 0, "rows": len(sys_rows), "by_env": {},
        }
        for group in collapse_machines(sys_rows).values():
            env = normalize_env(group[0]["env_raw"])
            s["total"] += 1
            s["by_env"][env] = s["by_env"].get(env, 0) + 1
    ...
    ordered = sorted(systems.values(), key=lambda x: (
        ...
        -x["total"],
        x["api_id"]))
    all_rows = conn.execute("SELECT asset_serial, hostname, ip FROM hardware").fetchall()
    total_assets = len(collapse_machines(all_rows))
    # 十項盤點第 5、6 條（2026-09-18）：「共 N 台」要拆得開——公司機分佈統計寫「共 5,405 台」、
    # 首頁「在管 3,377」，沒有任何地方說差在哪；「全庫對不上業務系統 1,300」裡 1,142 是帳外
    # （本來就沒有 AP ID）、約 141 是退役，真正在管卻沒填 AP ID 的只有 17 台（221）。
    # 拆法沿用報表列印來源對照表（host_sources.summary），不另算一套。
    import host_sources as _hs_mod
    import manage_state as _ms_mod
    _hs = _hs_mod.summary(_hs_mod.rows(conn))
    import system_report as _sr_mod
    _live_cia = [r for r in conn.execute(
        "SELECT asset_serial, hostname, ip, api_id, asset_status FROM hardware").fetchall()
        if (r["asset_status"] or "").strip() not in _ms_mod.RETIRED_STATUS
        and not _sr_mod.is_off_book(r["asset_serial"])]
    _unmapped_active = sum(1 for g in collapse_machines(_live_cia).values()
                           if not any((x["api_id"] or "").strip() for x in g))
    # mapped 一律算「對得上業務系統的全部台數」，不受 kind 篩選影響——
    # 不然切到基礎設施分頁時，「對不上 N 台」會突然暴增，看的人會以為資料壞了。
    # 台數要**去重後**再算：同一台掛兩個系統，在各系統各算一台，但全庫只算一台。
    all_mapped = len(collapse_machines(rows))
    shown_rows = [r for sys_rows in per_sys.values() for r in sys_rows]
    mapped = len(collapse_machines(shown_rows))
    ...
    return {
        ...
        # 全庫＝在管＋退役＋帳外（互斥窮盡，加總＝total_assets）
        "partition": {"managed": _hs["cia_hosts"], "retired": _hs["retired_hosts"],
                      "offbook": _hs["offbook_only_hosts"]},
        # 在管（使用中 CIA）卻沒填 AP ID 的台數——真正要回頭補的
        "unmapped_active_cia": _unmapped_active,
        # 筆數（CIA 登記的列數）——跟台數並列，一筆都沒有少
        "mapped_rows": len(shown_rows),
        "total_rows": len(all_rows),
        # 一台都對不上時才算（便宜但沒必要每次都跑）；畫面用它講出原因
        "mapping": mapping_diagnosis(conn) if all_mapped == 0 and all_rows else None,
    }
...
def drilldown(conn, api_id: str, env: str | None = None) -> dict:
    ...
    counted = 0
    row_count = 0
    # 一台機器算一台（見 machine_key）；環境／機房取代表列，角色把每筆合起來判
    for group in collapse_machines(rows).values():
        rep = group[0]
        env_name = normalize_env(rep["environment"])
        if env is not None and env_name != env:
            continue
        counted += 1
        row_count += len(group)
        loc = normalize_location(rep["physical_location"])
        slot = by_loc.setdefault(loc, {"location": loc, "total": 0, "by_env": {}})
        slot["total"] += 1
        slot["by_env"][env_name] = slot["by_env"].get(env_name, 0) + 1
        role = _machine_role(group, services, onboarded, system_kind(api_id) == KIND_AP)
        roles[role] += 1
...
def assets_in_cell(conn, api_id: str, env: str | None = None,
    ...
    for group in collapse_machines(rows).values():
        ...
        role_name = _machine_role(group, services, onboarded, system_kind(api_id) == KIND_AP)
        if role is not None and role_name != role:
            continue
        purposes: list[str] = []
        for g in group:
            pv = (g["asset_purpose"] or "").strip()
            if pv and pv not in purposes:
                purposes.append(pv)
        out.append({
            "asset_serial": r["asset_serial"],
            # 同一台的每一筆登記（一台一列，底下附全部序號，一筆都不藏）
            "serials": [g["asset_serial"] for g in group],
            "row_count": len(group),
            "hostname": r["hostname"],
            "ip": r["ip"],
            "os": r["os"],
...
def role_coverage(conn) -> dict:
    ...
    all_rows = conn.execute(
        "SELECT asset_serial, hostname, ip, api_id, asset_purpose, asset_name, device_model, os "
        "FROM hardware").fetchall()
    for group in collapse_machines(all_rows).values():
        is_on = group[0]["ip"] in onboarded
        # 只有「掛在業務系統底下」的才套「不是 DB 就是 AP」。
        # 沒有 api_id 的機器連屬於哪個系統都不知道，不能替它假設角色。
        default_ap = any((g["api_id"] or "").strip() and system_kind(g["api_id"]) == KIND_AP
                         for g in group)
        role = _machine_role(group, services, onboarded, default_ap)
        roles[role] += 1
