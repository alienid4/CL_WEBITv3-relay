...
#: 只有 Windows 會開的埠。445 不算——Linux 的 Samba 也開 445。
...
def onboard_method(os_guess: str | None = None, open_ports=None,
    ...
    # Windows 專屬埠（3389 RDP／5985 WinRM）證據力**蓋過通用 OpenSSH banner**
    # （2026-09-17 使用者：「同時有 3389 和 22 port 的，應該要判斷為它是 Windows，
    # 可能是 Windows 上架的 OpenSSH 服務，不該把它列為 Linux」）。
    #
    # 他是對的：Windows Server 2019 以後內建 OpenSSH，banner 是
    # `SSH-2.0-OpenSSH_9.7`——跟 Linux **一模一樣**，分不出來。
    # 原本只要 banner 解析得出 OS 就回「Linux・確定」，於是 10.99.18.39
    # （22/445/3389/5985）被說成「確定是 Linux」，拿 Linux 腳本去打它一定失敗，
    # 而且失敗訊息長得像帳號問題（webit3scan 進不去），害人去查錯方向。
    #
    # 只有 banner 自己寫 for_Windows 才算 Windows 的直接證據；
    # 通用 OpenSSH banner **排除不掉 Windows**，這時 3389／5985 說了算。
    win_ports = sorted(p for p in WINDOWS_ONLY_PORTS if p in ports)
    if banner and os_from_banner(banner):
        from_banner = os_from_banner(banner)
        if "Windows" in from_banner:
            return {"method": ONBOARD_WINDOWS, "confidence": "confirmed",
                    "evidence": f"服務 banner：{banner[:80]}",
                    "hint": ONBOARD_HINT[ONBOARD_WINDOWS]}
        if win_ports:
            return {"method": ONBOARD_WINDOWS, "confidence": "likely",
                    "evidence": (f"開了 {'、'.join(map(str, win_ports))}（只有 Windows 會開），"
                                 f"22 埠的 banner「{banner[:40]}」是通用 OpenSSH——"
                                 f"Windows 內建的 OpenSSH banner 跟 Linux 一樣，分不出來"),
                    "hint": ONBOARD_HINT[ONBOARD_WINDOWS]}
        # 通用 OpenSSH／Dropbear banner **不能說「確定」**（2026-09-17）：
        # 使用者對一台 AIX 按一鍵納管，畫面寫「實測研判：Linux（SSH-2.0-OpenSSH_9.7，確定）」
        # ——但 AIX 的 banner 跟 Linux 一模一樣，Windows 內建的 OpenSSH 也是。
        # 「確定」會讓人照著選，選錯平台腳本一定失敗。
        # 只有 banner 自己講出 OS 名字（Ubuntu／Debian／for_Windows…）才算確定。
        generic = ("openssh" in banner.lower() or "dropbear" in banner.lower())             and not any(k in banner.lower() for k in
                        ("ubuntu", "debian", "centos", "rhel", "red hat", "suse", "for_windows"))
        if generic:
            return {"method": ONBOARD_LINUX, "confidence": "likely",
                    "evidence": (f"服務 banner：{banner[:60]}——這是通用 OpenSSH 字串，"
                                 f"**分不出 Linux／AIX**（AIX 的 banner 一模一樣）。"
                                 f"這台若是 AIX 請自己改選，選錯平台腳本一定失敗"),
                    "hint": ONBOARD_HINT[ONBOARD_LINUX]}
        return {"method": ONBOARD_LINUX, "confidence": "confirmed",
                "evidence": f"服務 banner：{banner[:80]}", "hint": ONBOARD_HINT[ONBOARD_LINUX]}
    ...
    if "Windows" in text or win_ports:
        return {"method": ONBOARD_WINDOWS, "confidence": "likely",
