...
0 ${CRON_HOUR} * * * root cd ${WORK} && python3 scan_segments.py --segments segments.txt --out ./out --window '${WINDOW}' --limit 0 >> ${WORK}/scan.log 2>&1
CRONEOF
chmod 644 "$CRON_FILE"
say "✓ 已寫入 $CRON_FILE"
...
if python3 scan_segments.py --segments /tmp/_one_seg.txt --out "$TEST_OUT" \
     --now --limit 1 --pause 0 2>&1 | sed 's/^/    /'; then
